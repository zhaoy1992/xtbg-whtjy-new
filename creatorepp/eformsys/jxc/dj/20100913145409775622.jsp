...
//初始化
function init(){
    statusValue = getStatusByWft();//批注状态 
    if(eflag == 0){ //从查询页面或申请页面来
        if(avermentState == "未申请"){
         } else if(tflag == 1){
            document.getElementById('tr_id').style.display = "none";
        } else if(avermentState == "已申请" && tp_status == 0){
            findStateInfo();//进入初始化听证信息的界面
...
function toSpecialApply(){
       ...
       var tempURL = "20101016201431000833.jsp?wft_id=" + wft_id + "&avermentState=" + avermentState + "&tp_status=" + tp_status + "&temp=1";
...
function saveEndInfo(wft_id){
    ...
    vo.SPA_TERMINATION_REASONS = "听证已结束！";//强制终止理由
    return vo;
}
...
function validate(){
    ...
    if(flag){
        ...
        CHECK_TYPE : "required",
        ARGUMENT : {placeholders : ["听证结论"]}
        }, true);
    }
    
    if(flag){
        var time1 = findListenApplyTime().split(" ")[0];//听证申请时间
         var time2 = $('state_time').value;//听证开始时间
    ...
    if(flag){
        var time1 = $('state_time').value;//听证开始时间
    ...
    if(flag){
        if(${'state_time'}.value > ${'state_men'}.value){
            alert("听证开始时间不能大于听证结束时间");
            flag = false;
        }
    }
    
    if(flag){
        var time1 = $('state_men').value;//听证结束时间
         var time2 = getNowDate();//当前服务器时间
      ...
      <TR>
         <TD>
            <TABLE class=Ctable cellSpacing=0 cellPadding=0 width="100%" border=0>
               <TBODY>
                  <TR>
                     <TD class=taobox colSpan=4>听证登记</TD>
                  </TR>
                  <TR id=tr_id>
                     <TD class=c2 width="15%">听证程序申请</TD>
                     <TD colSpan=3><BUTTON class=button id=apply style="LEFT: 0px; POSITION: static; TOP: 0px" onmovestart=moveStart() dropstyle="否" controltype="button" onclick='bill_onclick("toSpecialApply()")'>听证申请</BUTTON></TD>
                  </TR>
                  <TR>
                     <TD class=c2 width="15%">听证开始时间<FONT color=red>*</FONT></TD>
                     ...
                     <TD class=c2 width="15%">听证结束时间<FONT color=red>*</FONT></TD>
                     <TD><INPUT class=input_out id=state_men style="LEFT: 0px; WIDTH: 100%; POSITION: static; TOP: 0px; HEIGHT: 30px" onmovestart=moveStart() readOnly controltype="text" onclick='bill_onclick("SelectDate()")'></TD>
                  </TR>
                  <TR>
                     <TD class=c2 width="15%">听证记录<FONT color=red>*</FONT></TD>
                  ...
                  <TR>
                     <TD class=c2 width="15%">听证结论<FONT color=red>*</FONT></TD>
