...
function init(){
    ...
    var sql = PunishMent.initRMD(piId);
    dataset1.Open(sql);
...
function addRMD(){
    ...
    var url = "20100910095124046567.jsp?piId="+piId
    var features = "dialogWidth:800px;dialogHeight:550px;scroll:no;status:no";
    var flag = window.showModalDialog(url,"",features);
    if(flag){
        reloadExtGrid();   
    }
}
...
function delRMD(){
    ...
    if(ids.length==0){    
       ...
       if(!confirm("确定要删除选择的违法行为吗？")){
             return;
       }
       var rv = PunishMent.delRMDByRMDId(ids);
       if(rv){
          alert("删除数据信息成功！");
       }else{
           alert("删除数据信息失败！");
...
**/
cm.setHidden(1,true);
cm.setHidden(2,true);ExtGrid1.render();
