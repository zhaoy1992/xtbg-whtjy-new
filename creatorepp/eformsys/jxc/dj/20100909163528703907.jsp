...
function calculateTime(){
    var happenTime = $('SPA_DATE').value;//申请时间
...
function unableApply(){
    ...
    */
    ...
    var rv = SpecialApply.deleteApply(wft_id, "陈述申辩");
    if(IsSpace(rv)){
        alert("取消成功!");
        $('UnableApply').style.display = "none";//隐藏取消申请按钮
         $('saveResult').style.display = "none";//隐藏登记结果按钮
...
function toAvermentPage(){
   ...
   var tempURL = "20100909161026046925.jsp?djbh=222&wft_id=" + wft_id + "&eflag=0&tflag=1";
   ...
   if(obj.pic == 1){//已暂存
       ...
       $('saveApply').style.display = "none";//隐藏提交按钮
        ...
        $('saveResult').style.display = "none";//隐藏登记结果按钮
        //查询申请信息
        var sql = SpecialApply.findSpecialApply(wft_id, "陈述申辩");
...
function validate(){
    ...
    if(flag){
        var tempTime = findListenApplyTime().split(" ")[0];//听证申请时间
    ...
    if(flag){
        var time1 = findSignInTime().split(" ")[0];//告知回执签收时间
         var time2 = $('SPA_DATE').value;//陈述申请时间
    ...
    if(flag){
        var time1 = $('SPA_DATE').value;//陈述申请时间
      ...
      <TR>
         <TD>
            <TABLE class=Ctable cellSpacing=0 cellPadding=0 width="100%" border=0>
               <TBODY>
                  <TR>
                     <TD class=taobox colSpan=4>陈述申辩申请</TD>
                  </TR>
                  <TR>
                     <TD class=c2 width="15%">申请时间<FONT color=red>*</FONT></TD>
                     <TD width="30%"><INPUT class=input_out id=SPA_DATE onblur=checkEndTime() style="LEFT: 0px; WIDTH: 100%; POSITION: static; TOP: 0px; HEIGHT: 20px" onmovestart=moveStart() readOnly controltype="text" onclick='bill_onclick("SelectDate()")'></TD>
                  ...
                  <TR>
                     ...
                     <TD width="30%"><INPUT id=SPA_END_DATE style="LEFT: 0px; WIDTH: 100%; POSITION: static; TOP: 0px; HEIGHT: 20px" onmovestart=moveStart() readOnly controltype="text"></TD>
