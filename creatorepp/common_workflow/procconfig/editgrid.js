...
Ext.onReady(function(){
	...
	var ActivityProperties = Ext.data.Record.create([
	    ...
	    processConfigManager.isSubFlowAct(process_all_name,activityId,function(data){
	    	if(!data){//非子流程活动
	    	    ...
	    	    processConfigManager.getSubFlowMgrName(process_all_name,activityId,function(data){
	    	    	if(data != ""){
	    	    		window.showModalDialog("shark_content.jsp?id="+module_id+"&name="+module_name+"&processMgrName="+data+"&type=subflow",window,"dialogWidth:700px");
	    	    	}
	    	    });
	    	}
	    });
...
//
function savaProcessData(process_config_id, module_id,process_all_name){
