...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/commonListServlet?classNameId=doneWorkList',  //commonListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['实例id','业务id','表单','办件编号',  '标题','业务类型 ', '发起人 ', '发起时间  ', '处理环节 ','状态', '办件历史 ','委托人','环节实例id','上一环节执行人','追回','环节执行顺序','cc_form_instanceid'],
		...
		gridComplete: function() { // var idss=jQuery("#gridTable").jqGrid('getColNames');
			...
			for (var i = 0; i < ids.length; i++) {
				...
				var zhuihuibt = "";//追回按钮
				if(model.actInsId != '' && model.act_index == '1' && model.status_name != '一般办结' && model.status_name != '已终止'&& model.lastact_userid === loginUserId){//上一环节执行人必须是当前登录者
					zhuihuibt = "<input id='zhuihui' type='button' class='but_y_01' onclick='getbackFlow("+cl+")' value = '追回' />";
				}
...
var toUserParamJosn = function() {
	var str = "{'username':'"+loginUserName+"',"+
			  "'busitype_code':'"+$('#busitype_code').val()+"',"+
			   "'ins_name':'"+$('#ins_name').val()+"','startSlTime':'"+$('#startSlTime').val()+"','endSlTime':'"+$('#endSlTime').val()+"','status_name':'"+jQuery("#status_name").val()+"'}";
				return descape(escape(str));
}

function getbackFlow(id){
	var model = jQuery("#gridTable").jqGrid('getRowData', id);
	var _actusername = loginUserName;
	var _actInsid = model.actInsId;
	var _busiId = model.cc_form_instanceid;
	var _insid = model.ins_id
	var url = "getbackflowajax.jsp";
	params = {actUsername:_actusername,actInsId:_actInsid,busiId:_busiId,insid:_insid};
...
function handle(id){ 
    ...
    var url ="situation.jsp?action_form="+ model.action_form + "&ins_id="+model.ins_id+"&busi_id="+model.busi_id+"&list_type=gongwen&operType=yiban&act_name="+model.act_name+"&isdone=true";
				...
				<tr>
					<td>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabs_search_ctable">
							  <tr>
									...
									<td class="cx_title_td"><input id="ins_name" name="ins_name" type="text" class="input_cx_title_283" /></td>
					...
					<td>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabs_search_ctable" id="hiddenArea" style="display:none">
							...
							<%if(CacheCommonManageProxy.getBooleanSystemParam("SelectState")){ %>
							</tr>
							<tr>
								<th class="input_cx_title_th">状态：</th>
								<td  class="cx_title_td" colspan="3" >
								<select  style="width: 132px" id="status_name" name="status_name">
									<option value="00">--请选择--</option>
									<option value="01">在办</option>
									<option value="02">一般办结</option>
									<option value="05">已终止</option>
								</select>
								</td>
							</td>
							</tr>
