...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/commonListServlet?classNameId=toBeWorkList',  //commonListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['实例id','业务id','环节实例id','ccformid','流程定义ID','办件编号 ', '标题 ','业务类型 ',  '发起人 ', '发起时间 ', '当前环节 '],
...
function handle(id){   
    ...
    var url = "../../../resources/plug/workflow/jsp/commonflowstart.jsp?ins_id="+model.ins_id+"&busi_id="+model.busi_id+"&actInsId="+model.actInsId+"&cc_form_instanceid="+model.cc_form_instanceid+"&def_id="+model.def_id+"&list_type=tobe_work&operType=nosend&actionPage=gongwen";
