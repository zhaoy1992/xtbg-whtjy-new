...
function Load(){
  ...
  //  webform.WebOffice.AppendTools("13","-",0);
    ...
    if(<%=ShowMenu%>=="0"){
    	webform.WebOffice.DisableMenu("套红(&O)");
    }
   // webform.WebOffice.DisableMenu("文件(&F)");
    webform.WebOffice.DisableMenu("编辑(&E)");
    webform.WebOffice.DisableMenu("视图(&V)");
    webform.WebOffice.DisableMenu("插入(&I)");
    webform.WebOffice.DisableMenu("格式(&O)");
    webform.WebOffice.DisableMenu("工具(&T)");
    webform.WebOffice.DisableMenu("表格(&A)");
   ...
   //webform.WebOffice.AppendMenu("主菜单","-");
    ...
    //webform.WebOffice.AppendMenu("20","-");
    webform.WebOffice.AppendMenu("21","套红(&O)");
    webform.WebOffice.AppendMenu("7","-");
    webform.WebOffice.AppendMenu("16","打印文档(&P)"); 
    webform.WebOffice.AppendMenu("8","-");
    //webform.WebOffice.AppendMenu("5","签名盖章(&Q)");
    //webform.WebOffice.AppendMenu("9","-");
    webform.WebOffice.AppendMenu("1","打开本地文件(&L)");
    webform.WebOffice.AppendMenu("2","保存本地文件(&S)");
    webform.WebOffice.AppendMenu("3","保存远程文件(&U)");
    webform.WebOffice.AppendMenu("10","-");
   ...
   StatusMsg(webform.WebOffice.Status);					//状态信息
    ...
    if(typeof(window.parent.docheckafter) == "function"){
...
<body bgcolor="#ffffff" onload="Load()" onunload="UnLoad()">  <!--引导和退出iWebOffice ../../../../iWebOffice2006/DocumentSave.jsp-->
  <form name="webform" id="webform" method="post" action="iwebOffice2009hh.jsp?submit_flag=1" onsubmit="return SaveDocument();">  <!--保存iWebOffice后提交表单信息-->
    ...
    <div id="webofficDiv" >
		 <OBJECT  id="WebOffice" height="100%" width="100%" classid="clsid:8B23EA28-2009-402F-92C4-59BE0E063499" codebase="../../../resources/plug/iWebOffice2009/iWebOffice2009.cab#version=10,5,0,0"></OBJECT>
	</div>
	</form>
