...
$(function() {
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/commonListServlet?classNameId=priManageList',  //commonListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
...
var toUserParamJosn = function() {
	var str = "{'loginUserID':'" + loginUserID + "','version':'" + $("#userReName").val() + "'}";
	return descape(str);
}

var addFlow = function() {
	JqueryDialog.Open1('添加权限', 'priManageAdd.jsp', 450, 250, true, true, true);
}
...
var editFlowInfo = function(id) {
	...
	JqueryDialog.Open1('修改权限', "priManageModify.jsp?leadID="+model.leadID, 450, 250, true, true, true);
}
/*
 * 删除
 */
var deleteFlowInfo = function() {
	...
	if (IsSpace(ids)) {
		...
		if (!confirm("确定要删除流程信息吗？")) {
			return;
		}
		var url = "priManageDeletedo.jsp";
		$("#flowids").val(ids);
