	...
	$.groupConfigTreeXtbg = function() {
		...
		this.initXtbg = function(){
			...
			$.each(this.treeList, function(key, val) {
				if(val.isGroup){
				    if("U5" == val.type){   //此判断用于过滤掉常用分组树   jia.tong  2013-08-16  
			...
			$(butOk).click(function() {
				NewTreeSubmitXtbg(THIS.NEWTREE,THIS.NEWTREE.TYPE);
				$(root).remove();
				lock.remove();
			});
			$(butR).click(
...
function NewTreeSubmitXtbg(newTree, type) {
	...
	var span = {
			...
			dataType:"text",
			data : '{"type":"'+type+'","treejson":"'+listObj+'"}',
			classID : 'find',
