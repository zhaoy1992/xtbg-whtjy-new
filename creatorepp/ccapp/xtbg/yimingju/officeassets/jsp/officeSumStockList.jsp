...
var xiugai = "";//修改按钮字段
if('<%=userArea%>' == '421005') {
	xiugai = "<input class='but_y_01' id=add value='修改' type='button' onclick='updateStock()'/>";
}
$(function() {
...
//tab2
	$("#gridTable").jqGrid({
		...
		<%if("421005".equals(userArea) || "421006".equals(userArea)) { %>
			colNames: ['库存ID', '品名', '类别',' 规格','供货厂商','单价(元)','数量','备注'],		
		<%} else { %>
			colNames: ['库存ID', '品名', '类别',' 规格','供货厂商','售后电话','单价(元)','数量 '],
		<%}%>
		...
		toolbar:[true, "top" ,"<input class='but_y_01' id=add value='入库' type='button' onclick='addLevel()' value=''/><input class='but_y_01' value='删除' id=del type='button' onclick='deleteStockLevel()'/><input class='but_y_01' value='库存明细' id=del type='button' onclick='stockLevel()'/>"+xiugai],
		multiselect: true,
...
var toUserParamJosn = function() {
		var str = "{'con_name':'"+$('#con_name').val()+"','suppliers':'"+$('#suppliers').val()+"'}";
...
var updateLevel = function(id) {
	...
	openWindows("windowId","当前物品的库存信息","../../officeassets/jsp/SeeStcokConSumDetailed.jsp?widowsid=windowId&stock_Id=" + model.stock_id,'officeSumStockList',false,window);
}
...
var addLevel = function(id) {
	openWindows("windowId","易耗品入库","../../officeassets/jsp/addOfficeConSumStock.jsp?widowsid=windowId",'officeEqStockList',false,window);
}
/*
	 函数:  stockLevel
	 说明:  修改缓急程度
...
var stockLevel = function(id) {
	var model = jQuery("#gridTable").jqGrid('getRowData', id);
	openWindows("windowId","库存明细","../../officeassets/jsp/SeeStcokConSumDetailed.jsp?widowsid=windowId&apply_id=" + model.apply_id,'PurchaseApply',false,window);
}
...
var updateStock = function(){
	...
	ids = idsArrTemp.join();
	if (count>=2){
		alert("只能选择一条记录修改,请重新选择...");
		return ;
	}
	if (IsSpace(ids)) {
		alert("请选择要修改的记录！");
	} else {
		openWindows("windowId","易耗品入库","../../officeassets/jsp/addOfficeConSumStock.jsp?widowsid=windowId&stock_id="+ids,'officeEqStockList',false,window);
	}
}
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabs_search_ctable">
							<tr>
								...
								<td width="296"  class="cx_title_td">
									<input class="input_cx_title_283" id="con_name" name="con_name" type="text" />
								</td>
...
<form id="delForm2" name="delForm2" method="post" action="addOfficeConSumStockDo.jsp">
