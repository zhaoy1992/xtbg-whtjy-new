...
//tab2
	$("#gridTable").jqGrid({
		...
		colNames: ['库存ID','类别ID', '设备名称', '类别',' 设备编号','型号','购买日期','供货厂商','售后电话','单价'],
...
var addLevel = function() {
	...
	if (IsSpace(ids)) {
		...
		for (var i = 0; i < selectedIds.length; i++) {
			...
			var equipment_name = model.equipment_name;
			var type_id = model.type_id;
			var type_name = model.type_name;
			var equipment_no = model.equipment_no;
			var equipment_model = model.equipment_model;
			var stock_data = model.stock_data;
			var equipment_price = model.equipment_price;
			if (stock_ids.indexOf(stock_id) >= 0){
				continue;
					...
					$(firstTr).find("#equipment_name").val(equipment_name);
					$(firstTr).find("#type_id").val(type_id);
					$(firstTr).find("#type_name").val(type_name);
					$(firstTr).find("#equipment_no").val(equipment_no);
					$(firstTr).find("#equipment_model").val(equipment_model);
					$(firstTr).find("#equipment_price").val(equipment_price);
