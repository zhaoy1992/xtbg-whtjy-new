...
<%
	...
	String infotypeHtml = DictDropDownList.buildSelect(sql,"license_number", "" ,"",true,""); // 车辆下拉框的值
%>
...
$(function() {
	...
	*/
	...
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/commonListServlet?classNameId=carManageMileagerecordYmjList',  //commonListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		{
			label:"行驶里程记录ID",
			name: 'mileagerecord_id',
			index: 'mileagerecord_id',
			hidden: true
		},
		{
			label:"车牌号码",
...
var toSearchParamJson = function() {
	var str = "{'license_number':'"+$('#license_number').val()+"'}";
...
返回值: 无
*/
var delInfo = function(ci) {
	var model = jQuery("#gridTable").jqGrid('getRowData', ci);
	var okF = function(){	
		$("#id").val(model.mileagerecord_id);
...
var addInfo = function() {
	 openWindows('addMileagerecordInfo_main_page','新增','addMileagerecordInfo.jsp?opt=add&subId=addMileagerecordInfo_main_page','addMileagerecordInfo_main_page',false,window);
}
/*
	 函数:  udateInfo
	 说明:  修改程序界面
	 ...
	 返回值: 无
*/
var udateInfo = function(ci) {
	var model = jQuery("#gridTable").jqGrid('getRowData', ci);
		openWindows('updMileagerecordInfo_main_page','查看信息', 'addMileagerecordInfo.jsp?opt=upd&subId=updMileagerecordInfo_main_page&id='+model.mileagerecord_id ,'updMileagerecordInfo_main_page',false,window);
	}
				...
				<tr>
					<td>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabs_search_ctable">
							<tr>
								<th class="input_cx_title_th">车牌号码：</th>
...
<!-- 查询列表区 --></div>
<form id="delForm1" name="delForm1" method="post" action="addMileagerecord.jsp">
