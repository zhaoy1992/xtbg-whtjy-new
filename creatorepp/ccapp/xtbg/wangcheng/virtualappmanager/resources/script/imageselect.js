  ...
  var methods = {
      init: function(options){
          ...
          if(options.lock == 'height'){
            header.append('<img style="height:' + (options.height-60) + 'px;width:"' + (options.width-0) + 'px />');
          }else{
          ...
          $options.each(function(i,el){
                ...
                dropdown.append('<img style="width:30%" onclick="jQuery(\'select[name=' + selectName + ']\').val(\''+ $(el).val() + '\').ImageSelect(\'close\').ImageSelect(\'update\',{src:\''+ $(el).text() + '\'});" src="' + $(el).text() + '"/>');
          });
