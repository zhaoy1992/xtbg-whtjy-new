<%@page import="com.chinacreator.pcbase.util.SqlUtil"%>
...
String path = request.getContextPath();
	...
	String backchooseuserids =request.getParameter("backchooseuserids");
	String showLevel="";
	if(StringUtilForAndroid.isBlank(request.getParameter("showLevel"))){
		showLevel="0";
	}else{
	...
	String theRootOrgid = SqlUtil.SqlToField("select org_id from td_sm_organization where parent_id='0'");
%>
	<TITLE> 人员树</TITLE>
	<!-- 引入jQuery -->
<script type="text/javascript" src="../../../resources/plug/jquery-ui-1.8.17.custom/js/jquery-1.7.1.min.js"></script>
	...
	/*  $(function(){
		 var perponid = $(parent.document).find("#uid").val();
		//初始人员id
			$("#perponid").val(perponid); 
			 var perponname = $(parent.document).find("#uName").val();
				//初始人员姓名
					$("#perponname").val(perponname); 
		...
		var setting = {
		    ...
		    */
			...
			async: {
				enable: true,
				url:'<%= path %>' + "/orgUserTreeServlet",
				autoParam:["item_id", "name=item_id", "level=lv"],
				otherParam:{"toporgId":"<%=theRootOrgid%>","backchooseuserids":"<%=backchooseuserids%>","showLevel":"<%=showLevel%>"}
				//dataFilter: filter
			},
			callback: {
		...
		*/
		...
		//-->
		function onclick() {
		/* //	alert(treeNode.name+""+treeNode.org_id);
		orgdept = $("#uservalue").val()+treeNode.name+',';
		orgdeptId = $("#uservalueId").val()+treeNode.item_id+','; */
        ...
        // parent.document.getElementById('textareaid').value = "aa";
        //alert(paren t.document.getElementById('textareaid').value);
		...
		$("#uservalue").val(returnValName);
		$("#uservalueId").val(returnValId);
		//文本域赋值
	   $(parent.document).find("#textareaid2").val(returnValName); */
		}
...
<table border="0" cellpadding="0" cellspacing="0" class="ztree_tcbox">
  ...
  <tr>
  <td><!-- <input id="perponid" type="hidden"/><input id="perponname" type="hidden"/> --><input type="hidden" id="uservalue"/> <input type="hidden" id="uservalueId"/></td>
  </tr>
 <tr>
    <td class="ztree_tcbotton">
    <input id="showorgs" name="showorgs" type="button" value="确定" class="cButton" onclick="OK()"/>
    </td>
  </tr> 
</table>
</BODY>
