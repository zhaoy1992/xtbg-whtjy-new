...
<script language="javascript" type="text/javascript">
  	...
  	*/
  	function changeSelect(action_defid,selectAct){
  		...
  		//-------省院班子成员请假 流程-------
  		//1.院长审核
  		if(!IsSpace(action_defid)&&action_defid=="yldqj_wp1_act5"){
  			selectAct.find("option[value=yldqj_wp1_act3]").attr("selected","selected").change();
  			selectAct.find("option[value=yldqj_wp1_act1]").remove();
  		} else if(!IsSpace(action_defid)&&action_defid=="yldqj_wp1_act3"){//办公室备案
  			selectAct.find("option[value=yldqj_wp1_act4]").attr("selected","selected").change();
  			selectAct.find("option[value=yldqj_wp1_act5]").remove();
  		} 
  	}
