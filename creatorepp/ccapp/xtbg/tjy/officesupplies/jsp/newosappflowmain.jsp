...
String busitype_type_code=StringUtil.deNull("yhpcg");//获取办公用品类型分类
String busitype_type_code2=StringUtil.deNull("wply");//获取办公用品类型分类
...
flowinfobean.setBusitype_type_code(busitype_type_code);
flowinfobean2.setBusitype_type_code(busitype_type_code2);
List<FlowInfoBean>  flowInfoBeanList  = flowInfoConfigServiceIfc.getFlowBaseInfoList(flowinfobean);
List<FlowInfoBean>  flowInfoBeanList2  = flowInfoConfigServiceIfc.getFlowBaseInfoList(flowinfobean2);
//TODO 此处应该做成配置
    ...
    <td><table border="0" cellspacing="0" cellpadding="0" style="margin-top:20px;">
      ...
      <tr>	
      		<%for(int i=num; i < flowInfoBeanList.size();i++){
      			...
      			if("uuid_521".equals(flowInfoBeanList.get(i).getBusitype_code())){
      				continue;
      			}
      			String busitype_code = flowInfoBean.getBusitype_code();
      ...
      <tr>
      		<%
      			for(int i=num; i < flowInfoBeanList.size();i++){
      				String busitype_code = flowInfoBeanList.get(i).getBusitype_code();
      				if(busitype_code.equals("uuid_520")){
      		%>
    	  ...
    	  <%	int num2 =0;
    		while(flowInfoBeanList2.size()>num2){ %>
      <tr>	
      		<%for(int i=num2; i < flowInfoBeanList2.size();i++){
      			FlowInfoBean flowInfoBean = flowInfoBeanList2.get(i);
      			String url = path+flowInfoBean.getFrom_path()+"?busiTypeCode="+flowInfoBean.getBusitype_code();
      			if("2".equals(flowInfoBean.getFlow_tyle())){
      				url = url+"&template_id="+flowInfoBean.getTemplate_id()+"&Template=word"+ flowInfoBean.getTemplate_id();
      			}
      ...
      <tr>
      		<%for(int i=num2; i < flowInfoBeanList2.size();i++){%>
 				<td height="40" align="center" class="font2"><%=flowInfoBeanList2.get(i).getFlow_name() %></td>      		
 			<%
 				num2++;
	 			if(i%5==4){//换行处理
	 				 break;
