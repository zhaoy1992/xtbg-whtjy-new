...
<%
	...
	if(rolesNames.indexOf("办公用品管理员")!=-1){
		StrRoles = "rkUser";
	}
%>
...
var afterSubmitFn=function(){
	try{
		window.location.href="<%=path%>/ccapp/xtbg/tjy/officesupplies/jsp/osnomoveobjectmainlist.jsp";
...
function exportFile(){
    ...
    html.push("<input type='hidden' name='excelImplClass' value='osNomoveDaoExcelImpl' /> ");//导入导出的实现类，要提前在spring中或orcleConfig中配置好
    html.push("<input type='hidden' name='excelTemplateFilePath' value='bgypTZ_excelTemplateFilePath' /> ");//导出的excel模版路径，要提前在系统properties中配置好
    html.push('</form>');
    html.push("</div>");
    //加载到body，提交后再删除
...
var updateLevel = function(id) {
	...
	var url = "<%=path%>/ccapp/xtbg/tjy/officesupplies/jsp/osupdatenomoveobjectinfo.jsp?windowsId=windowIds&t_ha_id="+model.t_ha_id;
	openAlertWindows('windowIds',url,'固定资产修改',1024,768,'25%','25%');
}

var havainfo = function(){
	var url = "<%=path%>/ccapp/xtbg/tjy/officesupplies/jsp/osnomoveobjectlist.jsp";
	window.location.href = url;
}

function addLevel(id){   
	var path ="<%=path%>/ccapp/xtbg/tjy/officesupplies/jsp/osnomoveobjectinfo.jsp?windowsId=windowIds1";
