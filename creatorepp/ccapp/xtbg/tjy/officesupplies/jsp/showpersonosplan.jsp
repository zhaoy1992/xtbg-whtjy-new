...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/pubListServlet?classNameId=com.chinacreator.xtbg.tjy.officesupplies.list.PlandoneDeWorkList',  //pubListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['','实例id','业务id','表单','办件编号 ', '标题  ','申购部室','业务类别','业务类型 ', '申请人 ', '申请起止时间 ', '当前环节 ',
		            <%if("uuid_520".equals(busitype_codes)){%> 
		          	 '审批状态',
		         	<%}%>
		...
		//申购审批查询
		<% } else if("uuid_521".equals(busitype_codes)){%>
		...
		//易耗领用查询
		<%} else if("uuid_620".equals(busitype_codes)){%>
		...
		//固定资产查询
		<%} else if("uuid_621".equals(busitype_codes)){%>
...
var toUserParamJosn = function() {
	...
	var str = "{'username':'"+loginUserName+"',"+
			  ...
			  "'ins_name':'"+$('#ins_name').val()+"','startSlTime':'"+
			  $('#startSlTime').val()+"','endSlTime':'"+
			  $('#endSlTime').val()+"','userId':'<%=userId%>'}";
...
function exportSgSPFile(){
	...
	if (IsSpace(ids)) {
	    ...
	    html.push("<input type='hidden' name='excelImplClass' value='osexaminationdaoexcelimp' /> ");//导入导出的实现类，要提前在spring中或orcleConfig中配置好
	    html.push("<input type='hidden' name='excelTemplateFilePath' value='bgypsgspjh_excelTemplateFilePath' /> ");//导出的excel模版路径，要提前在系统properties中配置好
	    html.push('</form>');
	    html.push("</div>");
	    //加载到body，提交后再删除
	    jQuery(html.join("")).appendTo("body");
...
function exportLysqFile(){
	...
	if (IsSpace(ids)) {
	    ...
	    html.push("<input type='hidden' name='busi_id' value='"+ids+"' /> ");
	    html.push("<input type='hidden' name='excelImplClass' value='oshavalydaoexcelimpl' /> ");//导入导出的实现类，要提前在spring中或orcleConfig中配置好
	    html.push("<input type='hidden' name='excelTemplateFilePath' value='bgyplydj_excelTemplateFilePath' /> ");//导出的excel模版路径，要提前在系统properties中配置好
	    html.push('</form>');
	    html.push("</div>");
	    //加载到body，提交后再删除
	    jQuery(html.join("")).appendTo("body");
...
var exportGdZcFile= function(){
	...
	if (IsSpace(ids)) {
	    ...
	    html.push("<input type='hidden' name='busi_id' value='"+ids+"' /> ");
	    html.push("<input type='hidden' name='excelImplClass' value='osnomovedaoexceelimpl' /> ");//导入导出的实现类，要提前在spring中或orcleConfig中配置好
	    html.push("<input type='hidden' name='excelTemplateFilePath' value='bggdzcly_excelTemplateFilePath' /> ");//导出的excel模版路径，要提前在系统properties中配置好
	    html.push('</form>');
	    html.push("</div>");
	    //加载到body，提交后再删除
	    jQuery(html.join("")).appendTo("body");
