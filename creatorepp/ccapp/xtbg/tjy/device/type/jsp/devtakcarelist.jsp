...
<%
	...
	String GET_LIST_CLASS1="com.chinacreator.xtbg.tjy.device.list.DevTakcareList";
	String path = request.getContextPath();
%>
<title>设备维修和保养记录界面</title>
<!-- 公共部分开始 -->
<!-- 引入jQuery -->
...
//tab2
	$("#gridTable").jqGrid({
		...
		colNames: ['主键ID','设备id', '设备编号 ','设备名称', '规格型号','投入使用日期','维修保养内容','维修验收情况','维修单位/人','设备管理员','日期','状态'],
		...
		{
			name: 'tac_checkcontent',
			index: 'tac_checkcontent',
			hidden:true
		},
		{
			name: 'tac_maintenance',
			index: 'tac_maintenance',
			width: 70
		},
		{
			name: 'devmanagename',
...
var addLevel = function() {
 	var pageName ="新增设备维修和保养记录";
	openAlertWindows('windowId',"../../../tjy/device/type/jsp/devtakcareinfo.jsp?windowsId=windowId&actionName=insert",pageName,600,340,'25%','25%');
}
...
var updateLevel = function(id) {
	var model = jQuery("#gridTable").jqGrid('getRowData', id);
	var pageName ="修改设备维修和保养记录";
	openAlertWindows('windowId','../../../tjy/device/type/jsp/devtakcareinfo.jsp?windowsId=windowId&actionName=update&tac_id='+ model.tac_id,pageName,600,340,'25%','25%');
}
...
var delLevel = function(id) {
	...
	$(selectedIds).each(function () {
		...
		if(!IsSpace(model.tac_id)){
			idsArrTemp.push(model.tac_id);
		}
	});
					...
					<td>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabs_search_ctable" id="hiddenArea" style="display:none">
							...
							<tr>
								...
								<td>
									<input type="text" id="dev_type" class="input_cx_title_283" >
								</td>
							</tr>
						</table>
							<!-- 隐藏查询 end-->
...
<form id="delForm2" name="delForm2" method="post" action="devtakcareinfodo.jsp">
