...
<%
	...
	String GET_LIST_CLASS1="com.chinacreator.xtbg.tjy.officesupplies.list.OfficeDetailList";
	...
	String name = StringUtil.deNull(request.getParameter("name"));
...
//tab2
	$("#gridTable").jqGrid({
		...
		colNames: ['主键ID', '标识卡编号','名称', '规格/型号','金额','领取时间','领用人','责任人','存放位置','资产用途','报废处理情况','备注'],
...
var addLevel = function() {
	...
	openAlertWindows('windowId',"../../../tjy/officesupplies/jsp/officedetailinfo.jsp?windowsId=windowId&actionName=insert&org_id=<%=org_id%>",pageName,550,520,'25%','25%');
}
...
var updateLevel = function(id) {
	...
	openAlertWindows('windowId','../../../tjy/officesupplies/jsp/officedetailinfo.jsp?windowsId=windowId&actionName=update&org_id=<%=org_id%>&id='+ model.id,pageName,550,520,'25%','25%');
}
...
var importExcel = function(){
	...
	var url = '${pageContext.request.contextPath}/ccapp/oa/resources/util/excel/excelimport.jsp?excelImplClass=officedetailDaoExcelImpl&excelTemplateFilePath=officedetail_excelTemplateFilePath&title='+title+'&remark='+remark;
...
function exportFile(){
    ...
    html.push("<input type='hidden' name='excelImplClass' value='officedetailDaoExcelImpl' /> ");//导入导出的实现类，要提前在spring中或orcleConfig中配置好
    html.push("<input type='hidden' name='excelTemplateFilePath' value='officedetail_excelTemplateFilePath' /> ");//导出的excel模版路径，要提前在系统properties中配置好
    html.push('</form>');
    html.push("</div>");
    //加载到body，提交后再删除
