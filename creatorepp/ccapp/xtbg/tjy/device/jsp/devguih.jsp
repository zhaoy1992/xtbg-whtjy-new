...
<%
	...
	String lingyong = StringUtil.deNull(devborrowInfo.get("t_lyorg_mc"));
	//System.out.println("lingyong="+lingyong);
	//根据设备编号查询出 配件信息
...
<title>设备借用归还界面</title>
	...
	//环节ID
	var action_defid="sbjygh_wp1_act4";
	...
	*/
	  function openDevAppInfo(){
    	var path ="<%=path%>/ccapp/xtbg/tjy/device/jsp/devInfoList.jsp?windowsId=windowIds1&dev_stockflag=2,3&dev_state=1&frompage=devguih";
	...
	返回值: 无
	*/
	function setDevInfo(id,dev_code,dev_name,dev_stockflag,type,make_users,technical_parameters,arrive_time,custos_userid,custos_username,jieyong_orgmc,jieyonguser,t_lydate,guihid,user_orgname){
		//设备标题
		jQuery("#TA_OA_DEVBORROW__T_TITLE").val("【"+dev_name+"】:【"+type+"】");
		//设备ID
		jQuery("#TA_OA_DEVBORROW__T_SB_ID").val(id);
		//设备编号
		jQuery("#dev_code").val(dev_code);
		//设备名称
		jQuery("#dev_name").val(dev_name);   
		//设备状态
		if(dev_stockflag=="1"){
			dev_stockflag="在库";
		}
		jQuery("#dev_stockflag").val(dev_stockflag);
		jQuery("#TA_OA_DEVBORROW__T_LYORG_MC").val(jieyong_orgmc);
		jQuery("#TA_OA_DEVBORROW__T_JRE_MC").val(jieyonguser);
		
		jQuery("#TA_OA_DEVBORROW__T_LYDATE").val(t_lydate);
		jQuery("#dis_TA_OA_DEVBORROW__T_LYDATE").val(t_lydate);
		jQuery("#guihid").val(guihid);
		resultDevInfoItem(id);
	}
...
<form id="form1" name="form1" action="<%=path%>/ccapp/xtbg/tjy/device/jsp/devinfodo.jsp" method="post" >
	...
	<input type="hidden" id="guihid"  name="guihid"   /> 
