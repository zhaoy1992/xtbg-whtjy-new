...
<%
	...
	String sql = "select i.busitype_code, i.busitype_name from OA_FLOW_BUSITYPE i where i.is_valid = 'Y' and i.busitype_type_code in('sbzj') ";
	String str = DictDropDownList.buildSelect(sql,"busitype_code","","",true,"");
	
	String busitype_codes = DaoUtil.sqlToField("select WMSYS.WM_CONCAT( i.busitype_code) from OA_FLOW_BUSITYPE i where i.is_valid = 'Y'  and i.busitype_type_code in('sbzj')");
%>
...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/pubListServlet?classNameId=com.chinacreator.xtbg.tjy.device.list.ToBeDeWorkList',  //pubListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['实例id','业务id','环节实例id','ccformid','流程定义ID','办件编号 ', '公文标题 ','业务类型 ','业务类型CODE',  '发起人 ', '发起时间 ', '当前环节 ', '表单','模板ID'],
