...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/pubListServlet?classNameId=com.chinacreator.xtbg.tjy.processing.list.DoDeWorkList',  //pubListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['实例id','业务id','环节实例id','ccformid','流程实例ID','活动定义ID','活动类型编号 ','流程ID','办件编号 ',  '标题  ','业务类别', '业务类型  ','发起人 ', '发起时间  ', '当前环节 ', '办件历史 ','代理人','表单','业务类型CODE','模板ID','是否代理'],
		...
		gridComplete: function() { // var idss=jQuery("#gridTable").jqGrid('getColNames');
			...
			for (var i = 0; i < ids.length; i++) {
				...
				<%}%>
				if(1==num){
					parent.window.countTotaltProcess($("#gridTable").jqGrid('getGridParam','records'));
				}
				num++;
			}
		},
