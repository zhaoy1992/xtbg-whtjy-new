...
<%
	...
	StringBuffer sql = new StringBuffer(" select oftype_id,oftype_name from ta_oa_osupplietype t  where 1=1 and t.oftype_flag = '1' ");
	String infotypeHtml= DictDropDownList.buildSelect(sql.toString(),"oftype_id","","",true,"","width:286px;");
...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/pubListServlet?classNameId=com.chinacreator.xtbg.tjy.detectionsupplies.list.OsHavaLingYongobjectList',  //pubListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['物品领用id','物品ID','物品名称','物品类型ID','物品类型名称','设备编号','规格ID','规格名称 ','领用人ID','领用人','领用ORGID','领用处室','领用时间'],
...
var toUserParamJosn = function() {
		 var str = "{'t_ha_iswhat':'1','ofustype_name':'"+jQuery("#ofustype_name").val()
		 		   + "','oftype_id':'" + jQuery("#oftype_id").val()
		 		   + "','t_ha_sbno':'" + jQuery("#t_ha_sbno").val()
				...
				<tr>
					<td>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabs_search_ctable">
							<tr>
								<th class="input_cx_title_th">设备名称：</th>
					...
					<td>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabs_search_ctable" id="hiddenArea" style="display:none">
							<!-- 正规 4列 -->
							<tr>
								<th class="input_cx_title_th">设备类别：</th>
								<td  class="cx_title_td">
									<%=infotypeHtml %>
								</td>
								<th class="input_cx_title_th">设备编号：</th>
								<td  class="cx_title_td">
									<input id="t_ha_sbno" name="t_ha_sbno" type="text" class="input_cx_title_283" />
								</td>
							</tr> 
							<tr>
