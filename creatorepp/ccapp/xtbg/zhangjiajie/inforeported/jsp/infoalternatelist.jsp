...
$(function() {
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/commonListServlet?classNameId=infoReportAlternateList',  //commonListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['报送id', '标题' , '备用时间', '录入者', '上报单位', '审核状态'],
...
var editPerson = function(id) {
	...
	var url = "infoviewforalternate.jsp?submittedinfoId="+model.submittedinfo_id;
	location.href = url;
}
...
<table width="100%" class="cQueryTableArea" id="queryTable">
	<tr>
	  ...
	  <input class="cText_out" id="submittedinfo_title" name="submittedinfo_title" type="text" /></td>
