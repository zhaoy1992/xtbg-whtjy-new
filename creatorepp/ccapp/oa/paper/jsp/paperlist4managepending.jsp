...
<%
	...
	String p_rep_title_selectHtml = DictDropDownList
			.buildSelect4DictType(
					p_rep_title_dictType,
					"p_rep_type_id",
					"",
					"",
					true,
					"",
					"",
					"class='validate[required] select_240' style='width: 138px;height:24px;'",
					"");
	//信息状态flag
	String strWhere = " and (d.dictdata_value='"
			+ PaperConstants.STATUS_WAIT_PUBLISH
			+ "' or d.dictdata_value='"
			+ PaperConstants.STATUS_PUBLISHED
			+ "' or d.dictdata_value='" + PaperConstants.STATUS_UNDONE
			+ "') ";
	String sql = "select d.dictdata_value, d.dictdata_name, d.dictdata_id from oa_dict_data d, oa_dict_type t where 1=1 "
			+ strWhere
			+ " and t.dict_id = d.dict_id and d.dictdata_is_valid='1' and t.dict_is_valid='1' and t.dict_code = 'infoflag' order by d.dictdata_order desc";
	String p_flag_id_selectHtml = DictDropDownList
			.buildSelect4Sql(
					sql,
					"p_flag_id",
					"",
...
$(function() {
    ...
    $("#gridTable").jqGrid({
       ...
       //表头设置       
        ...
        prmNames : { //这是一个数组，用于设置jqGrid将要向Server传递的参数名称
       	...
       	toolbar : [ true, "top" ,"<input class='but_y_01' onMouseOver=javascript:$(this).attr('class','but_y_02'); onMouseOut=javascript:$(this).attr('class','but_y_01'); id=add value='增加' type='button' onclick='addKbmDocDetail()' value=''/>"],
       	multiselect : true, 
        onCellSelect:function (id,icol)
		{
			if(icol != 0)
			{
        		...
        		urlTmp = "&loadurl=paperlist4managepending&actionName=viewpending&p_manage_id="+model.p_manage_id;
				toNewPage2(urlTmp,"已送审");
			}
		}
    });
    ...
    var toUserParamJosn = function() {
        var str = "{'opentype':'"+opentype
        	...
        	+"','p_flag_id':'"+jQuery("#p_flag_id").val()
    	    +"','p_acc_user_name':'"+jQuery("#p_acc_user_name").val()
			...
			<tr>
				<td>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0"
						class="tabs_search_ctable" style="width: 970px;">
						<tr>
							<th width="90" class="input_cx_title_th">信息标题：</th>
							<!-- 敏感字符的问题 -->
							<td width="200" class="cx_title_td"><input type="text"
								id="p_rep_title" name="p_rep_title" class="input_cx_title_283"
								style="width: 133px;" /></td>
							<th width="90" class="input_cx_title_th">信息类型：</th>
							<td width="200" class="cx_title_td" style="padding-left:1px;"><%=p_rep_title_selectHtml%>
							</td>
							...
							<td width="200" class="cx_title_td"><%=p_flag_id_selectHtml%>
							</td>
						</tr>
						<tr>
							<th width="90" class="input_cx_title_th">上报日期：</th>
							<td width="300" class="cx_title_td" colspan=3><input
								id="begin_time" name="begin_time"
								onfocus="WdatePicker({maxDate:'#F{$dp.$D(\'end_time\')}',dateFmt:'yyyy-MM-dd'})"
								style="float: left" class="date_120 Wdate" type="text" /><span
								style="float: left;padding-left:63px;">&nbsp;至：&nbsp;</span> <input
							...
							<td width="200" class="cx_title_td"><input type="text"
								...
								id="p_rep_unit_id" name="p_rep_unit_id">
							</td>
