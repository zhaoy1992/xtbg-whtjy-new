	...
	$("#gridTable").jqGrid({
		...
		onCellSelect:function (id,iCol,c,d){
			if(iCol != -1){
				...
				setParentValue(model.p_template_id);
			}
		}
	}).navGrid('#gridPager', {
		edit: false,
...
function toParamJosn(){
	...
	var str = "{'p_template_name':'"+p_template_name+"','p_template_unitid':'<%=p_type_unit%>'}";	
	return escape(str);
}
 ...
 *调用父页面的赋值方法
 */
var setParentValue = function(p_template_id){
	
  try{
	 var parentWindow = getParentWindow("<%=windowId%>");
	 //关闭窗口
     removeAlertWindows("<%=windowId%>",true);            
	 if(parentWindow && parentWindow.replacepaperContents)
	 {
		 parentWindow.replacepaperContents(p_template_id);
	 }
  }catch(e){
      alert(e.message+"");
  }
