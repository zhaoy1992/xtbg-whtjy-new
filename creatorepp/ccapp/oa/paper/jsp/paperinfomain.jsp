		...
		$("#tabs").tabs({//每个页面只刷新一次
			select : function(event, ui) {
				...
				if ($(iframeitem).attr("id")=="frame2" && $(iframeitem).attr("src")==""){
					$(iframeitem).attr("src", "paperinfo_noread.jsp?p_manage_id=<%=p_manage_id %>");//已经审核过的
				}
			}
		});
			...
			<td>
				<div>
					<div id="tabs">
						...
						<div  id="maindiv" class="vcenter" style="height:100%;overflow-x:hidden;overflow-y:auto;">
							<div id="tabs-1">
								<iframe src="paperinfo_read.jsp?p_manage_id=<%=p_manage_id %>" id="frame1" frameborder="0"
									width="100%"></iframe>
							</div>
							<div id="tabs-2">
							...
							<div><table><tr><td height=1></td></tr></table></div>
						</div>
					</div>
				</div>
			</td>
