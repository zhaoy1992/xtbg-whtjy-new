...
<%
	...
	List<RoleType> typenamelist = (List<RoleType>)rtm.getTypeNameList();//所有角色类型
	if(typenamelist == null){
		typenamelist = new ArrayList<RoleType>();
	}
	
	String role_id = StringUtil.deNull(request.getParameter("role_id"));
	String role_name = StringUtil.deNull(Escape.unescape(DataControlUtil.replaceStr(request.getParameter("role_name"))));
	String role_type_id = StringUtil.deNull(request.getParameter("role_type_id"));
	String role_desc = StringUtil.deNull(Escape.unescape(DataControlUtil.replaceStr(request.getParameter("role_desc"))));
%>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">

<%@page import="com.chinacreator.eform.util.Escape"%>
<%@page import="com.chinacreator.xtbg.core.common.util.DataControlUtil"%><html>
<head>
	...
	function checkRoleName(){
		...
		if(role_name != ""){//角色名称验证
			$.ajax({
				...
				data: "role_name="+role_name+"&action=update"+"&role_id="+role_id,
...
<body style="overflow-y:hidden;">
	<form id="form1" name="form1" action="" method="post">
		...
		<div id="users-contain">
			<table  width="100%" border="0" cellspacing="0" cellpadding="0" >
				<tr>
					...
					<td class="content_02_box_div_table_td">
						<input type="text" name="role_name" id="role_name" onblur="checkRoleName()"
							onkeyup="checkInsert(this)" onbeforepaste="checkPaste()" class="validate[required,maxSize[50]] input_152" style="width:70%;height=23px"  value="<%=role_name %>" maxlength="50">
					...
					<td class="content_02_box_div_table_td">
						<select name="role_type" id="role_type" class="validate[required]" style="width:70%;">
							...
							<%for(RoleType bean : typenamelist) {%>
								<option value="<%=bean.getRoleTypeID() %>" <%if(role_type_id.equals(bean.getRoleTypeID())){ %> selected <%} %>>
									<%=bean.getTypeName() %>
								</option>
							<%} %>
						</select>
				...
				<tr>
					...
					<td class="content_02_box_div_table_td">
						<textarea name="role_desc" id="role_desc" onkeyup="checkInsert(this)" onbeforepaste="checkPaste()" class="validate[required,maxSize[500]] textarea_556" style="width:70%;" ><%=role_desc %></textarea>
					</td>
				</tr>
			</table>
		</div>
		...
		<div class="cue_box_foot" style="position: absolute;bottom: 0px;">
			...
			<input class="but_y_01" type="button" id="sel"  value="保存" onclick="updateRole()"/>
		</div>
	</form>
