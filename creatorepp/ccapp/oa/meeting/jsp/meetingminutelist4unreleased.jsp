...
<%
    ...
    //获得当前登录用户的部门id
    String orgId = controler.getChargeOrgId();
    //获得当前登录用户的id
...
$(function() {
    ...
    $("#gridTable").jqGrid({
        url: '<%=path%>/pubListServlet?classNameId=com.chinacreator.xtbg.core.meeting.list.MeetingMinutesList', 
    ...
    /*
      ...
      */
    var add = function() {
    	openWindows('add_meetingMinutes','新建会议纪要'
                ,'meetingminutesadd.jsp?windowId=add_meetingMinutes&flag=0'
    ...
    var update = function(id) {
        openWindows('update_meetingMinutes','更新会议纪要'
                ,'meetingminutesadd.jsp?windowId=update_meetingMinutes&flag=0&id='+id
                ,'meetingmanager',false,window);
    }
    ...
    */
     var del = function(id) {
            ...
            $(selectedIds).each(function () {
                ...
                //id不为空
                ids.push(model.m_minutes_id);  
            });
                
            if (!ids.length) {
                alert("未选中会议纪要，请重新选择要删除的会议纪要！");
            } else {
                var okF = function(){
                    //ajax方式提交数据
                    jQuery.ajax({ 
