...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%=path%>' + '/pubListServlet?classNameId=com.chinacreator.xtbg.core.notice.list.NoticeAuditList',  //pubListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
...
var doview = function(id){
	...
	var updateNotice = function(id) {
		var model = jQuery("#gridTable").jqGrid('getRowData', id);
		 openWindows('sub_main_updateannouncements','修改通知公告','../../notice/jsp/noticeauditbaseinfo.jsp?n_notice_id='+ model.n_notice_id+ '&subid=sub_main_updateannouncements','ggfs',false,window);
	}
