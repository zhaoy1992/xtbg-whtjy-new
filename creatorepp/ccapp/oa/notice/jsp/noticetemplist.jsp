...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%=path%>' + '/pubListServlet?classNameId=com.chinacreator.xtbg.core.notice.list.NoticeTempList',  //pubListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		filterNames:["n_state"],
		filterOptions:[[["--发布状态--","--发布状态--"],["未发布","未发布"],["撤销","撤销"],["已送审","已送审"],["审核不通过","审核不通过"]]],//alue text 后的值与显不
		...
		toolbar: [true,"top","<input class='but_y_01' onMouseOver=javascript:$(this).attr('class','but_y_02'); onMouseOut=javascript:$(this).attr('class','but_y_01'); id=add type='button' onclick='addNotice()' value='新增公告'/><input class='but_y_01' onMouseOver=javascript:$(this).attr('class','but_y_02'); onMouseOut=javascript:$(this).attr('class','but_y_01'); id=del type='button' value='删除' onclick='delNotice()'/>"],
		multiselect: true,
...
var doview = function(id){
	...
	var updateNotice = function(id) {
		var model = jQuery("#gridTable").jqGrid('getRowData', id);
		 openWindows('sub_main_updateannouncements','修改通知公告','../../notice/jsp/noticebaseinfo.jsp?n_notice_id='+ model.n_notice_id+ '&subid=sub_main_updateannouncements&n_state='+model.n_state,'ggfs',false,window);
	}
