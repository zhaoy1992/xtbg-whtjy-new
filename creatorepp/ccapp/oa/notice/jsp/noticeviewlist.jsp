...
<%
	...
	String n_select_type = request.getParameter("n_select_type");
%>
<title>通知接收列表</title>
<!-- 引入jQuery -->
...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%=path%>' + '/pubListServlet?classNameId=com.chinacreator.xtbg.core.notice.list.NoticeViewList',  //pubListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
...
var toAnnouncementParamJosn = function() {
	...
	+ "','n_release_time':'" + $("#n_release_time").val() + "','n_orgname':'" + $("#n_orgname").val()
	 + "','n_select_type':'" + $("#n_select_type").val() + "','n_creator_userid':'<%=accesscontroler.getUserID()%>'}";
...
var updateAnnouncement = function(id) {
	var model = jQuery("#gridTable").jqGrid('getRowData', id);
		openWindows('suxian_ggjs','查看通知公告信息','<%=path%>'+'/ccapp/oa/news/newnoticeview.jsp?isview=view&n_notice_id='
				+ model.n_notice_id+'&n_receiverid='+ model.n_receiverid+'&subid=suxian_ggjs&urls=jsy','ggjs',false,window);
	}
</script>
</head>
<body>
			...
			<tr>
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0"
					...
					<tr>
						...
						<input  type="hidden" name="n_select_type" id="n_select_type" value="<%=n_select_type %>"/>
						<input class="input_cx_title_283" type="text" id="n_orgname" name="n_orgname" /></td>
