...
date: 2013-11-06
--%>
<%@page import="com.chinacreator.xtbg.core.inspect.util.InspectConstant"%>
...
<%
    ...
    //查询参数
    String timeday=StringUtil.deNull(request.getParameter("send_time"));
	...
	String timeyear=StringUtil.deNull(request.getParameter("plan_endtime"));
	String nowday=DateUtil.getCurrentDay();
	String nowmonth=DateUtil.getCurrentMonth();
	String send_time="";
	String plan_endtime="";
	if("0".equals(inspect_state)){
    	inspect_state="";
    }
    if("0".equals(inspect_type)){
    	inspect_type="";
    }
    if("0".equals(timeday)){
    	send_time=timeyear+"-01-01";
    	plan_endtime=timeyear+"-12-31";
    }else if("1".equals(timeday)){//一天以内
    	send_time=timeyear+"-"+nowmonth+"-"+nowday;
    	plan_endtime=timeyear+"-"+nowmonth+"-"+nowday;
    }else if("2".equals(timeday)){//一周以内
    	send_time=DateUtil.getFormatCurrentAdd(-6,"yyyy-MM-dd");
    	plan_endtime=timeyear+"-"+nowmonth+"-"+nowday;
    }else if("3".equals(timeday)){//一月以内
    	send_time=timeyear+"-"+nowmonth+"-01";
    	plan_endtime=timeyear+"-"+nowmonth+"-"+DateUtil.getDaysOfCurMonth(timeyear+"-"+nowmonth);
    }
...
var inspect_type = "<%=inspect_type%>";
var inspect_state = "<%=inspect_state%>";
var plan_endtime = "<%=plan_endtime%>";
var userId = "<%=userId%>";
var path = "<%=path%>";
$(function() {
    ...
    var toUserParamJosn = function(objName,isEscape){ 
        ...
        param[objName+"inspect_state" ] = inspect_state;    
        param[objName+"inspect_name"] = jQuery("#inspect_name").val();
        param[objName+"inspect_type"] = inspect_type;
        //起始交办时间
        param[objName+"plan_endtime"] = plan_endtime;
        //用指定期限字段存放“截止交办时间”
        param[objName+"send_time"] = send_time;
        if(isEscape){
            //转码
    ...
    */
    var update = function(id,flag){
        ...
        openAlertWindows('update_inspect'
                ,path+'/ccapp/oa/inspect/jsp/inspectdetail.jsp?windowId=update_inspect'
                +"&id="+id+"&flag="+flag, 
                '更新任务'
                ,width,height,'5%','10%');        
    }
