...
<%
	...
	Map<String,String> exFieldMap = workCountDao.getListExtendFields("发文", userid);
...
$(function() {
	...
	$("#gridTable").jqGrid({
		...
		colNames: ['id','busi_id','busitype_code','action_form','模板id','主题','发文日期','拟稿单位','发文编号','拟稿人'<%=ex_columnNames%>],
		//字符串数组，用于指定各列的题头文本，与列的顺序是对应的。
		colModel: [ //最重要的数组之一，用于设定各列的参数
		 {
		  	name: 'ins_id',index: 'ins_id',hidden:true
		 },
		 {
			  name: 'busi_id',index: 'busi_id',hidden:true
		 },
		 {
			  name: 'busitype_code',index: 'busitype_code',hidden:true
		 },
		 {
			  name: 'action_form',index: 'action_form',hidden:true
		 },
		 {
			  name: 'template_id',index: 'template_id',hidden:true
		 },
		 {
   			name: 'title',index: 'title',edit:false,width: '150' ,sortable: false,editable: true
   		},
   		{
   			name: 'accept_time',index: 'accept_time',width: '80',sortable: false
    	},
    	{
   			name: 'org_name',index: 'org_name',width: '80',sortable: true
    	},
    	{
   			name: 'number_title',index: 'number_title',width: '80',sortable: false
    	},
    	{
   			name: 'accepter',index: 'accepter',width: '60',sortable: false
    	}
    	<%=ex_columns%>
		],
		...
		toolbar: [true,"top","<input type='button' class='but_y_01' title='归档' value='归档' onclick='sendToFileByInsId()'/>"],
		multiselect: true,
...
var editguestinfo = function(id) {
    ...
    +"&template_id="+model.template_id+"&Template=word"+model.template_id+"&windowsId=windowsId";
...
function openForms(url, flowname){
			...
			openAlertWindows('windowsId',url,flowname,width,height,'0%','0%');
}

/***
 * 归档
 */
function sendToFileByInsId(){
	var ids = "";
    ...
    $(selectedIds).each(function () {
        ...
        //id不为空
        ids.push(model.ins_id);  
    });
        
    if (!ids.length) {
        alert("未选中数据，请重新选择要归档的数据！");
    } else {
        var okF = function(){
            //ajax方式提交数据
            jQuery.ajax({ 
        ...
        alert("请勿重复归档，重复归档将导致重复的档案数据。点击确定进行发送归档数据？",p); 
    }
}
...
var toUserParamJosn = function() {
	...
	var str = "{'workTitle':'"+$("#workTitle").val()
			  ...
			  +"','count_type':'发文"
			  //特检院OA start 2014-06-09 只查询发文，收文，请批件 公文数据 
			  +"','busitype_type_code':'000001"
			  //特检院OA end 2014-06-09 只查询发文，收文，请批件 公文数据 
			  +"','user_id':'<%=userid%>"	
			  +"'}";
...
var configlistcolumn = function(){
	openAlertWindows('sub_configcolumn',getContextPath()+"/ccapp/oa/archive/worklistcount/jsp/gwcountlistcolumnconfig.jsp?listtype=发文&subid=sub_configcolumn",'自定义列表',800,300,'25%','25%');
}
								...
								<tr>
									...
									<!-- 隐藏查询 -->
										<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabs_search_ctable" id="hiddenArea" style="display: none">
											<tr>
												<th class="input_cx_title_th">发文日期：</th>
											...
											<tr>
												<th class="input_cx_title_th">发文编号：</th>
												<td class="cx_title_td">
