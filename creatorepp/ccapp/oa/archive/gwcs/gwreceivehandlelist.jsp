...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/pubListServlet?classNameId=com.chinacreator.xtbg.core.archive.gwcs.list.ReceiveGwHandleList',  //pubListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colModel: [ //最重要的数组之一，用于设定各列的参数
            {
    			...
    			label:"t_gwreceive_id",
    			name: 't_gwreceive_id',
    			index: 't_gwreceive_id',
    			hidden: true
    		},{
    			label:"t_ins_id",
    			name: 't_ins_id',
...
var toUserParamJosn = function() {
		var str="";
		var str = "{'t_lwbt':'"+$('#title').val()+"','t_lwbh':'"+$('#serial_number').val()+
				  "'}";
...
var handle = function(id) {
	...
	var path =  "<%=path%>/ccapp/oa/archive/gwcs/gwreceiveinfo.jsp?windowsId=windowIds&gwreceive_id="+model.gwreceive_id;
	openAlertWindows('windowIds',path,'查看',900,500,'25%','25%');

	
	<%-- var ajaxurl="gwreceivelistdo.jsp";
	var params = {'attach_id':model.attach_id,'filename':filename};
	jQuery.ajax({
		url: ajaxurl,
		type: 'POST',
		dataType:'json',
		...
		success: function(data){
			...
			if(djsn!=''){
				...
				window.open("<%=path%>/servlet/pubFileDownload?djsn="+djsn+"&openType=online&fileName="+encodeURI(filename),"_blank","width="+width+",height="+height+",left=0px,top=0px");
			}
		}
	}); --%>
