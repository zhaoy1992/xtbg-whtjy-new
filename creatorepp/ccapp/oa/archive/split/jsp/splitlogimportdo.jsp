	...
	<title>拆分文件导入jsp</title>
		...
		try {
			...
			if (err.equals("false") && !file.isMissing()) {//如果文件正确则上传
				relative_path = java.io.File.separator + "splitfile"+java.io.File.separator+new SimpleDateFormat("yyyyMMdd").format(new Date());
		...
		if ("false".equals(err)) {
			outStr = "开始拆分文件...";
			SplitLogService service = new SplitLogServiceImpl();
			outStr = service.splitFile(relative_path,fileName,Integer.parseInt(header_row),user_id,"");
		} else {
