...
function Load(){
  try{
	...
	//var is_infotype = jQuery("#is_infotype").val();//是否信息分类
	var is_chief = jQuery("#is_chief").val();//是否信息分类
	var is_nbfs = jQuery("#is_nbfs").val();//内部分送
	var is_doback = jQuery("#is_doback").val();//回退
	var is_ckls = jQuery("#is_ckls").val();//查看历史
	//var is_nbfs ="1";//内部分发
	//var is_print = "1";//是否打印
	//var is_split = "1";//是否拆分
	...
	//webform.WebOffice.ExtParam="{_DBType:\""+jQuery("#_DBType").val();"\"}";       //夏天新增：
    ...
    if(initType=="1" || initType=="2" || initType=="3"){//如果不是1，2，3那么不需要此菜单
		/*if(is_ckzw=="1"){//查看正文
	 		webform.WebOffice.AppendTools("11","查看正文",7);
	 	}
		if(is_ckmq=="1"){//公文面签
	 		webform.WebOffice.AppendTools("12","查看面签",7);
	 	}
		if(is_msg =="1"){//短信配置
	 		webform.WebOffice.AppendTools("6","短信配置",7);
	 	}*/
	 	if(is_red =="1"){//套红
	 		webform.WebOffice.AppendTools("2","套  红",7);
	 	}
		if(is_singname=="1"){//公文面签
	 		webform.WebOffice.AppendTools("13","签  名",7);
	 	}
    	/*if(is_infotype =="1"){//信息分类
    		webform.WebOffice.AppendTools("26","信息分类",7);
  	    }*/
		if(is_chief =="1"){//督办
			webform.WebOffice.AppendTools("26","督办",7);
	    }
		if(is_ckls =="1"){//督办
			webform.WebOffice.AppendTools("28","查看历史",7);
	    }
    	...
    	if(is_split =="1"){//拆分
	    	webform.WebOffice.AppendTools("24","指标拆分",7);
	    }
	    if(is_newflow =="1"){//发起新的流程(流程发起)
	    	webform.WebOffice.AppendTools("21","流程发起",7);
	    }
	    if(is_gk =="1"){//发起新的流程
	    	webform.WebOffice.AppendTools("22","信息公开",7);
	    }
	    if(is_sign =="1"){//签字
	 		webform.WebOffice.AppendTools("3","签  字",7);
	 	}
	    ...
	    if(is_ckcg=="1"){//查看草稿
	 		webform.WebOffice.AppendTools("10","查看原文",7);
	 	}
	    if(is_cklc =="1"){//分发公文
	 		webform.WebOffice.AppendTools("20","查看流程",7);
	 	}
	    //if(is_attachment =="1"){//处理附件
			webform.WebOffice.AppendTools("4","附  件",7);
			webform.WebOffice.AppendTools("23","参阅件",7);

		//}
		if(is_cd =="1"){//存档（将文件送入档案模块）
	 		webform.WebOffice.AppendTools("19","办结归档",7);
	 	}
		if(is_nbfs =="1"){//内部分送
    		webform.WebOffice.AppendTools("27","内部分送",7);
  	    }
	 	if(is_gwff =="1"){//分发公文
	 		webform.WebOffice.AppendTools("18","分发公文",7);
	 	}
	 	if(is_affix=="1"){//转公文传输
	 		webform.WebOffice.AppendTools("14","盖  章",7);
	 	}
	 	if(is_xd =="1"){//校对
	 		webform.WebOffice.AppendTools("17","校  对",7);
	 	}
	 	if(is_gwzl=="1"){//公文整理
	 		webform.WebOffice.AppendTools("9","公文整理",7);
	 	}
	 	if(is_serialnumber =="1"){//公文编号
	 		webform.WebOffice.AppendTools("5","编  号",7);
	 	}
		webform.WebOffice.AppendTools("16","后续环节",7);
		if(is_doback=="1"){
			webform.WebOffice.AppendTools("15","回  退",7);
		}
	...
	webform.WebOffice.attachEvent("OnToolsClick",function(vIndex,vCaption){ 
	    ...
	    if (vIndex==19){//存档(档案系统)
	    	doFlowSave();
	    };
	    if (vIndex==20){//查看流程
	    	showFlowHistory();
	    };
	    if (vIndex==21){//发起新的流程(流程发起)
	    	wordiwebofficeJS.doFlowInFlow.newFlow();
	    };
	    if (vIndex==22){//是否公开
	    	isPublicSel();
	    };
...
function queryWord(type){
	...
	var height = window.screen.availHeight;
	var width = window.screen.availWidth;
	var url = jQuery("#path").val()+"/ccapp/oa/archive/iwebofficequery.jsp?windowsId=windowId1&RecordID="+recordIDs+"&EditType="+EditType;
