...
var JqueryDialog = {
	...
	//Header背景色
	"cHeaderBackgroundColor"	:	"#95D1F7",
	//右上角关闭显示文本
	//"cCloseText"				:	"X 关闭",
	"cCloseText"				:	"",//yiping.huang modiy
	//鼠标移上去时的提示文字
	"cCloseTitle"				:	"关闭",
	
	//Bottom背景色
	"cBottomBackgroundColor"	:	"#E4F1FC",
	//提交按钮文本
	"cSubmitText"				:	"确 认",
	//取消按钮文本
	"cCancelText"				:	"取 消",
	...
	/// <param name="isScrolling">是否有滚动条</param>
	
	Open1:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight, isSubmitButton, isCancelButton, isDrag, isScrolling){
		JqueryDialog.init(dialogTitle, iframeSrc, iframeWidth, iframeHeight, isSubmitButton, isCancelButton, isDrag, isScrolling);
	},
	...
	/// <param name="isCancelButton">是否呈现“取消”按钮</param>
	/// <param name="isDrag">是否支持拖拽</param>
	init:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight, isSubmitButton, isCancelButton, isDrag, isScrolling){
		
		//获取客户端页面宽高
		var _client_width = document.body.clientWidth;
		var _client_height = document.documentElement.scrollHeight;
		
		if($.browser.msie && 6 == $.browser.version ){//如果是ie6的浏览器
		...
		/**
		//获取左边窗口宽高
		var _left_client_width = top.frames["perspective_main"].frames["perspective_toolbar"].document.body.clientWidth;
		var _left_client_height = top.frames["perspective_main"].frames["perspective_toolbar"].document.documentElement.scrollHeight;
		//获取中间隐藏/显示菜单栏
		var _centerhidden_client_width = top.frames["perspective_main"].frames["hidden_frame"].document.body.clientWidth;
		var _centerhidden_client_height = top.frames["perspective_main"].frames["hidden_frame"].document.documentElement.scrollHeight;
		**/
		//create shadow
...
/**全局隐藏
 * 		var leftWin = $(top.frames["perspective_main"].frames["perspective_toolbar"].document.body);
		if(typeof(leftWin.find("#jd_shadow")[0]) == "undefined"){
			leftWin.prepend("<div id='jd_shadow' >&nbsp;</div>");
			var _jd_shadowLeft = leftWin.find("#jd_shadow");
			_jd_shadowLeft.css("width", _left_client_width + "px");
			_jd_shadowLeft.css("height", _left_client_height + "px");
		}
		var centerHiddenWin = $(top.frames["perspective_main"].frames["hidden_frame"].document.body);
		if(typeof(centerHiddenWin.find("#jd_shadow")[0]) == "undefined"){
			centerHiddenWin.prepend("<div id='jd_shadow' >&nbsp;</div>");
			var _jd_shadowCenter = centerHiddenWin.find("#jd_shadow");
			_jd_shadowCenter.css("width", _centerhidden_client_width + "px");
			_jd_shadowCenter.css("height", _centerhidden_client_height + "px");
		}
		//如果判断不为空则说明存在机构树内容区域
		if(typeof(top.frames["perspective_main"].frames["perspective_content"].frames["base_navigator_container"].frames["base_navigator_content"]) != "undefined"){
			//获取中间左边隐藏机构树内容区域
			var _centerlefthidden_client_width = top.frames["perspective_main"].frames["perspective_content"].frames["base_navigator_container"].frames["base_navigator_content"].document.body.clientWidth;
			var _centerlefthidden_client_height = top.frames["perspective_main"].frames["perspective_content"].frames["base_navigator_container"].frames["base_navigator_content"].document.documentElement.scrollHeight;
			var centerLeftHiddenWin = $(top.frames["perspective_main"].frames["perspective_content"].frames["base_navigator_container"].frames["base_navigator_content"].document.body);
			centerLeftHiddenWin.prepend("<div id='jd_shadow' >&nbsp;</div>");
			var _jd_shadowCenterLeft = centerLeftHiddenWin.find("#jd_shadow");
			_jd_shadowCenterLeft.css("width", _centerlefthidden_client_width + "px");
			_jd_shadowCenterLeft.css("height", _centerlefthidden_client_height + "px");
...
**/
		...
		//_jd_dialog_m_h.css("background-color", JqueryDialog.cHeaderBackgroundColor); //原来的
		_jd_dialog_m_h.addClass("popTableTitle"); //yiping.huang modtiy
		...
		$("#jd_dialog_m_b_2").append("<iframe id='jd_iframe' src='"+iframeSrc+"' scrolling='"+isScrolling+"' frameborder='0' width='"+iframeWidth+"' height='"+iframeHeight+"' />");
		//bottom
		...
		if(isSubmitButton){
			_jd_dialog_m_t.append("<span><input id='jd_submit' class='ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all' value='"+JqueryDialog.cSubmitText+"' type='button' onclick='JqueryDialog.Ok();' /></span>");
		}
		if(isCancelButton){
			...
			_jd_dialog_m_t.append("<span><input id='jd_cancel' class='ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all' value='"+JqueryDialog.cCancelText+"' type='button' onclick='JqueryDialog.Close();' /></span>");
		}**/
	
		//register drag
		isDrag = false;//屏蔽掉拖动功能
		if(isDrag){
			DragAndDrop.Register(_jd_dialog[0], _jd_dialog_m_h[0]);
		}
	},
	/// <summary>关闭模态窗口之前的回调函数</summary>
	...
	Close:function(){
		JqueryDialog.BeforeCloseFunction();
		$("#jd_shadow").remove();
		$("#jd_dialog").remove();
		...
		/**全局隐藏
		var leftWin = $(top.frames["perspective_main"].frames["perspective_toolbar"].document.body);
		leftWin.find("#jd_shadow").remove();
		var centerHiddenWin = $(top.frames["perspective_main"].frames["hidden_frame"].document.body);
		centerHiddenWin.find("#jd_shadow").remove();
		if(typeof(top.frames["perspective_main"].frames["perspective_content"].frames["base_navigator_container"].frames["base_navigator_content"]) != "undefined"){
			var centerLeftHiddenWin = $(top.frames["perspective_main"].frames["perspective_content"].frames["base_navigator_container"].frames["base_navigator_content"].document.body);
			centerLeftHiddenWin.find("#jd_shadow").remove();
		}
		**/
	},
