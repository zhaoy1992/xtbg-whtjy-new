...
FancyUpload3.Attach.File = new Class({
	...
	render: function() {
		...
		this.ui.cancel = new Element('a', {'class': 'file-cancel', text: '取消', href: '#'});
		...
		var progress = new Element('img', {'class': 'file-progress', src: getCurrentFilePath()+'../../../../ccapp/oa/resources/plug/fancyupload/assets/progress-bar/progress-green.gif'}).inject(this.ui.size, 'after');
...
(function() {
	...
	var phrases = {
		...
		'validationErrors': {
			'duplicate': '文件 <em>{name}</em> 已经上传,不允许重复上传',
			'sizeLimitMin': '文件<em>{name}</em> (<em>{size}</em>) 太小,最小文件上传大小为 {fileSizeMin}.',
			'sizeLimitMax': '文件 <em>{name}</em> (<em>{size}</em>) 太大,最大文件上传大小为  <em>{fileSizeMax}</em>.',
