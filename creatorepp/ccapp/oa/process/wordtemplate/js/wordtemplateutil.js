...
function createTextInput(wordDocument, ruleName, options) {
	try {
		...
		//wordSelection.FormFields.Shaded = false;//去掉输入框的背景阴影
		//FormText域名称
...
function documentProtect(wordDocument, password) {
	try {
		...
		//文档保护，仅填写窗体可编辑
		wordDocument.Protect(wdAllowOnlyFormFields, 1, "" + password);//
		//wordDocument.Protect(wdAllowOnlyFormFields, false, "",false,false);//第二个参数0：强制使用默认值，1：参数
		//wordDocument.BookMarks("OA_TEST_FORM__FJ_ID").Range.Editors.Add(-1); //常量：wdEditorEveryone=-1 
		//wordDocument.BookMarks("OA_TEST_FORM__FJ_ID").Range.Editors.Add(-1); //常量：wdEditorEveryone=-1 
		//wordDocument.BookMarks("OA_TEST_FORM__TITEL").Range.Editors.Add(-1); //常量：wdEditorEveryone=-1 
		//wordDocument.BookMarks("OA_TEST_FORM__TITEL").AutoFormat();
		//var mSelection = wordDocument.Application.Selection;
		//跳转到书签位置
		//mSelection.GoTo(-1,0,0,"OA_TEST_FORM__TITEL");
...
//获取表格单元格的值table表格对象，row 行号， col 列号
function getTableCellValue(table,row,col){
	try{
	   	var value  = table.Cell(row,col).Range.Text;
	   	if(IsSpace(value)){
	   		return "";
	   	}else{
	   		return value;
