$(function() {
	...
	$("#gridTable").jqGrid({
		url: path + '/pubListServlet?classNameId=com.chinacreator.xtbg.core.common.iweboffice.list.WordTemplateList',  
		datatype: "json",
		mtype: "POST",
		height: "100%",
		autowidth: true,
...
function updateWordTemplate(id){
	...
	openAlertWindows('windowId',getCurrentFilePath() + 'wordtemplatedetailframe.jsp?mb_id='+model.mb_id+'&org_id='+org_id+'&org_name='+org_name+'&windowsId=windowId','更新公文模版','100%','100%','0%','0%',userSearch);
}	
...
function addWordTemplate(){
	...
	openAlertWindows('windowId',getCurrentFilePath() + 'wordtemplatedetailframe.jsp?org_id='+org_id+'&org_name='+org_name+'&windowsId=windowId','新建公文模版','100%','100%','0%','0%',userSearch);
...
function copyMB() {
	...
	if(jSelectedIds.size()==1){
		doReturnOK = function(actionType,mess){
			if(mess){
				...
				alert("操作成功,是否编辑?",{
					...
					okName:'确认',
			        okFunction:function(){
			        	var url = getCurrentFilePath() + "wordtemplatedetailframe.jsp?mb_id="+mess+"&org_id="+org_id+"&org_name="+org_name+'&windowsId=windowId';
						openAlertWindows('windowId',url,'更新公文模版','100%','100%','0%','0%',userSearch);
			        },
			        cancelName:'取消'
				});
			}else{
		...
		jSelectedIds.each(function(){
			...
			if(!IsSpace(ids)){
				var okF = function(){
					var url = "wordtemplatedo.jsp?operationType=docopywordtemplate&ids="+ids;
					document.all.form1.action = url;
					document.all.form1.target = "hiddenFrame";
					document.all.form1.submit();
				};
