...
<%
	...
	String sql = "select i.busitype_code, i.busitype_name from OA_FLOW_BUSITYPE i where i.is_valid = 'Y' order by i.busitype_order asc  ";
...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/pubListServlet?classNameId=com.chinacreator.xtbg.core.process.flowprocessing.list.FlowManageList',  //pubListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
...
var chooseuser = function(id,tousername) {
	var model = jQuery("#gridTable").jqGrid('getRowData', id);
	var url = getCurrentFilePath()+"/flowmanagedo.jsp?user_name="+model.user_name+"&tousername="+tousername+"&action_defid="+model.actInsId;
...
var handle = function(id) {
	...
	//"?ins_id="+model.ins_id+"&busi_id="+model.busi_id+"&list_type=worklis&operType=yiban&act_name="+model.act_name+"&actiontype_code="+model.actiontype_code+"&busiTypeCode="+model.busitype_code
  	//		  +"&template_id="+model.template_id+"&Template=word"+model.template_id;
    //window.parent.location = url,"perspective_content";
    //特检院OA 【后台管理】【传阅文件】【待办任务】的公文查看窗口，默认大小不应当满屏大小。2014-04-24 start 
