...
<%
	...
	String longin_org_name = accesscontroler.getChargeOrgName();
...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/pubListServlet?classNameId=com.chinacreator.xtbg.core.process.fgw_flowprocessing.list.FGW_DoneWorkFindList',  //pubListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
...
var toUserParamJosn = function() {
		...
		// add by lixing   new 
	var str = "{'org_name':'<%=longin_org_name%>','username':'<%=loginUserName%>',"+"'is_open':'1',"+"'status_code':'02',"+
			  "'busitype_code':'"+$('#busitype_code').val()+"',"+
			  "'ins_name':'"+$('#ins_name').val()+"','startSlTime':'"+$('#startSlTime').val()+"','endSlTime':'"+$('#endSlTime').val()+"'}";
