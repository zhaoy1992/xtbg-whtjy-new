...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/pubListServlet?classNameId=com.chinacreator.xtbg.core.process.fgw_flowprocessing.list.FGW_FlowManageList',  //pubListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['flow_id','实例id','业务id','环节实例id','ccformid','流程实例ID','活动定义ID','活动类型编号 ','流程ID','办件编号 ',  '标题  ', '业务类型  ','业务类型 ','发起人 ', '发起时间  ', '当前环节 ','当前环节处理人','处理人登录名','操作','代理人','表单','业务类型CODE','模板ID','action_form'],
		...
		colModel: [ //最重要的数组之一，用于设定各列的参数
       {
		...
		gridComplete: function() { // var idss=jQuery("#gridTable").jqGrid('getColNames');
			...
			for (var i = 0; i < ids.length; i++) {
				...
				//be = "<input id='modify' type='button' class='but_y_01' onMouseOver=javascript:$(this).attr('class','but_y_02'); onMouseOut=javascript:$(this).attr('class','but_y_01'); onclick='chooseActUsers("+cl+")' value = '设置' />";
				//jQuery("#gridTable").jqGrid('setRowData', ids[i], {
				//	act: be
				//});
				...
				if(model.flow_id != '' ){
					be1 = "<input id='modify' type='button' class='but_y_01' onMouseOver=javascript:$(this).attr('class','but_y_02'); onMouseOut=javascript:$(this).attr('class','but_y_01'); onclick='delFlow("+cl+")' value = '删除' />";
					jQuery("#gridTable").jqGrid('setRowData', ids[i], {
						act1: be1
					});
				}
...
var chooseuser = function(id,tousername) {
	var model = jQuery("#gridTable").jqGrid('getRowData', id);
	var url = getCurrentFilePath()+"/flowmanagedo.jsp?user_name="+model.user_name+"&tousername="+tousername+"&action_defid="+model.action_defid;
...
var delFlow = function(id){
	...
	var okF = function(){
		var url = getCurrentFilePath()+"/flowmanagedo.jsp?aciton_type=delflow&ins_id="+ins_id;
		document.all.form1.action = url;
		document.all.form1.target = "hiddenFrame";
		document.all.form1.submit(); 
	}
	...
	var model = jQuery("#gridTable").jqGrid('getRowData', id);
	  if(model.flow_id!=""){
		  var url = '<%=path %>' + model.action_form +  "?ins_id="+model.ins_id+
			...
			//"?ins_id="+model.ins_id+"&busi_id="+model.busi_id+"&list_type=worklis&operType=yiban&act_name="+model.act_name+"&actiontype_code="+model.actiontype_code+"&busiTypeCode="+model.busitype_code
		  	//		  +"&template_id="+model.template_id+"&Template=word"+model.template_id;
		    //window.parent.location = url,"perspective_content";
    	...
    	//发改委老数据 正文
		var url="<%=path%>/ccapp/oa/process/fgw_flowprocessing/showWordContent.jsp";
		var params =  {id:model.ins_id};
		jQuery.ajax({
			url: url,
			type: 'POST',
			dataType:'json',
			...
			success: function(data){
				if(data.result==true){
					var url = "<%=path%>/ccapp/oa/process/fgw_flowprocessing/showFileContent.jsp?textId="+data.textId;
					 var height = window.screen.availHeight;
					 var width = window.screen.availWidth;
					 url= url+"&height="+height+"&width="+width;
					 openAlertWindows('newwindows',url,model.ins_name,width,height,'0%','0%');
...
<!--								</td>-->
		...
		<!-- 查询列表区 cGridArea(查询区样式)-->
				<div class="cGridArea" id="maindiv" style="width: 80%">
