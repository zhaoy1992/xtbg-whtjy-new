...
$(function() {
	...
	$("#gridTable").jqGrid({
		...
		toolbar: [true,"top","<input class='but_y_01' id=add type='button' onclick='addFlow()' value='添加'/>"
		...
		+"<input class='but_y_01' id=del type='button' onclick='deleteFlowInfo()' value='删除'/>"],
		multiselect: true,
...
var addFlow = function() {
	...
	openWindows('sub_flowinfoconfigdetail','流程配置','flowinfoconfigdetail.jsp?operwindowid=sub_flowinfoconfigdetail','processConfig',false,window);
}
...
var editFlowInfo = function(id) {
	...
	//location.href = url;
	url = "flowinfoconfigdetail.jsp?flow_id="+model.flow_id+"&type=flag1"+"&operwindowid=sub_flowinfoconfigdetail";
...
var deleteFlowInfo = function() {
	...
	for(var i = 0; i < selectedIds.length; i++) {
		var model = jQuery("#gridTable").jqGrid('getRowData', selectedIds[i]);
		 ...
		 busitype_code += model.busitype_code;
	...
	alert("删除流程配置会删除所有绑定的流程配置，并且影响该配置的业务类型在途数据，请谨慎操作！确定要删除流程信息吗？",p)
	}
}
...
function copyMB() {
	...
	if(jSelectedIds.size()==1){
		doReturnOK = function(mess){
			if(mess){
				userSearch();
				alert("操作成功,是否编辑?(新数据的'业务类型','所属表格'需要手动选择!)",{
					headerText:'提示',
					okName:'确认',
			        okFunction:function(){
			        	url = "<%=path%>/ccapp/oa/process/processconfig/jsp/flowinfoconfigdetail.jsp?flow_id="+mess+"&type=flag1"+"&operwindowid=sub_flowinfoconfigdetail";
						openWindows('sub_flowinfoconfigdetail','流程配置',url,'processConfig',false,window);
			        },
			        cancelName:'取消'
				});
			}else{
		...
		jSelectedIds.each(function(){
			...
			var ids = model.flow_id;
			if(!IsSpace(ids)){
				var okF = function(){
					var url = "copyflowinfoform.jsp?ids="+ids;
					document.all.form1.action = url;
					document.all.form1.target = "hiddenFrame";
					document.all.form1.submit();
				};
