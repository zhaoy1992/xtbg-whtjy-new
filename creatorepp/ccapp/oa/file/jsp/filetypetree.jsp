...
var setting = {
	...
	async: {
    	enable: true,//启用异步加载
    	//url: window.top.getContextPath() + '/commonTreeServlet?classNameId=com.chinacreator.xtbg.core.common.tree.dao.impl.FileTypeTree', 
    	url: window.top.getContextPath() + '/commonTreeServlet?classNameId=com.chinacreator.xtbg.core.file.dao.imploracle.FileTypeTree', 
		autoParam:["f_type_id", "name=f_type_id", "level=lv"],
...
function orgOnclick(event,treeId,treeNode) {
	...
	//for filedoc 文件管理
	if(!IsSpace(rightIframename)){
		if("autoConfig"==rightIframename){
			if(treeNode.f_file_list){
				parent.setRightIframeSrc(""+treeNode.f_file_list+"?f_type_id="+treeNode.f_type_id+"&f_type_name="+encodeURI(encodeURI(treeNode.name))+"&f_file_detail="+treeNode.f_file_detail );
			}else{
				parent.setRightIframeSrc("");
			}
			return;
		}
	}
...
function onAsyncSuccess(event, treeId, treeNode, msg) {
	if(parent.leftPageOnAsyncSuccess){
		if(parent.leftPageOnAsyncSuccess()){
			var zTree = $.fn.zTree.getZTreeObj("treeDemo");
			var selectedNodes = zTree.getSelectedNodes();
			if(selectedNodes&&selectedNodes.length==0){
				var nodesd =zTree.getNodesByParamFuzzy("f_type_level","3",null);
...
function zTreeOnDrop(event, treeId, treeNodes, targetNode, moveType) {//拖拽完成时间
	...
	var node = zTree.getNodeByParam("f_type_id", treeNodes[0].parent_id, null);
	var childNodes = zTree.transformToArray(node); 
	var nodes = new Array(); 
   for(i = 0; i < childNodes.length; i++) { 
		nodes[i] = childNodes[i].f_type_id;
	}
    params="nodes="+ nodes+"&action=order";
	$.ajax({
			url: 'filetypeinfodo.jsp',
			type: 'POST',
			dataType: 'text',
			data: params,
			success: function(data){
				 if(data){
					var zTree = $.fn.zTree.getZTreeObj("treeDemo");
					var node = zTree.getNodeByParam("f_type_id", treeNodes[0].f_type_id, null);
