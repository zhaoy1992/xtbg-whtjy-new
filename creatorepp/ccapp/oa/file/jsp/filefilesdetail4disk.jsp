...
<%
    ...
    String typeName = f_type_name;
    if(f_type_name.lastIndexOf("-->") != -1){
        typeName = f_type_name.substring(f_type_name.lastIndexOf("-->")+3,f_type_name.length());
    }
...
var userId = "<%=StringUtil.deNull(userId) %>";//用户id
    jQuery(function(){
      ...
      //实物类型默认选中 纸质
        jQuery("#OA_FILE_FILES__F_ENTITY_TYPE option[value=3]").attr("selected","selected");
...
function printFileContent(){
     /*判断浏览器的类型并决定采用哪个对象实例，并检测控件是否安装、是否最新版本、并引导安装或升级。  */
    var LODOP=getLodop(document.getElementById('LODOP'),document.getElementById('LODOP_EM'));
    createPrintHtml(LODOP,fileContentHtml());
    LODOP.PREVIEW();    
}
/*创建部分打印内容  */
function createPrintHtml(LODOP,phtml){
   /*初始化运行环境，清理异常打印遗留的系统资源，设定打印任务名。  */
    ...
    LODOP.ADD_PRINT_HTM('1.25CM','2.3CM','80%','90%',phtml); 
...
function printFileBack(){
     /*判断浏览器的类型并决定采用哪个对象实例，并检测控件是否安装、是否最新版本、并引导安装或升级。  */
    var LODOP=getLodop(document.getElementById('LODOP'),document.getElementById('LODOP_EM'));
    createPrintHtml(LODOP,backHtml());
    LODOP.PREVIEW();
};
/**
 * 打印卷内信息
...
function joinHtml(){
      var html = [
        '<div style="width:100%;">',
        '<table style="width:100%;font: normal 16px/29px \'宋体\';background-color: #333;" border="0" cellspacing="1" cellpadding="0" >',
        ...
        jQuery("select[id$=__F_DOC_DEADLINE] option:selected").text(),
        '</td>',
        '</tr>',
        '<tr>',
        '<td colspan="3" style="text-align: center;letter-spacing:5px;background-color: #fff;height:30px;line-height: 30px;">',
              '<table style="width:100%;font: normal 16px/29px \'宋体\';" border="0" cellspacing="0" cellpadding="0" >',
              '<tr style="height:65px;">',
              '<td style="width:5%;text-align: center;">自</td>',
              '<td style="width:40%;text-align: center;">',
              jQuery("input[id$=__F_START_TIME]").val(),
              '</td>',
              '<td style="width:10%;text-align: center;">起&nbsp;至</td>',
              '<td style="width:40%;text-align: center;">',
              jQuery("input[id$=__F_END_TIME]").val(),
              '</td>',
        ...
        '</tr>',
        '</table>',
        '<table style="width:50%;font: normal 16px/29px \'宋体\';float: right;background-color: #333;',
        'margin-top: 5px;" border="0" cellspacing="1" cellpadding="0" >',
             ...
             ' </tr>',
              ...
              jQuery("select[id$=__F_FONDSNO] option:selected").text(),
              '</td>',
              '<td style="width:33.33%;text-align: center;background-color: #fff;">',
              jQuery("input[id$=__F_LISTNO]").val(),
              '</td>',
          '    </tr>',
         '</table>',
        '</div> '
...
function backHtml(){
   ...
   'text-align:center;width:60px;font: normal 16px/29px \'宋体\';background:#E02A14;',
   ' color:#E02A14;font-weight: bold;" cellspacing="5" cellpadding="0" >',
   '<tr>',
   '<td style="color:#fff;height:400px;" valign="middle" align="center">',
   '   <span style="text-align:center;width:20px;display:inline;line-height:40px;',
   'font-size:34px;margin-left:5px;font-weight: bolder;letter-spacing:8px;">',
   '<%=typeName %>', 
   '</span>',   
   '</td>',
   '</tr>',   
   '<tr><td  align="center"  style="background:#fff;height:30px;text-align:center;">全宗号</td></tr>',
   '<tr><td  align="center"  style="background:#fff;height:50px;text-align:center;color:#000;">',
   jQuery("select[id$=__F_FONDSNO]").val(),
   '</td></tr>',
   '<tr><td align="center" style="background:#fff;height:30px;text-align:center;">目录号</td></tr>',
   '<tr><td  align="center"  style="background:#fff;height:50px;text-align:center;color:#000;">',
   jQuery("input[id$=__F_LISTNO]").val(),
   '</td></tr>',

   '<tr><td  align="center"  style="background:#fff;height:30px;text-align:center;">案卷号</td></tr>',
   '<tr><td  align="center"  style="background:#fff;height:50px;text-align:center;color:#000;">',
   jQuery("input[id$=__F_FILESNO]").val(),
   '</td></tr>',
   
   '<tr><td  align="center"  style="background:#fff;height:50px;text-align:center;line-height: 20px;">',
...
function fileContentHtml(){
   var html =[
              ...
              '<tr><td align="center" style="background:#FFF;height:100px;line-height:100px;font-size:26px;font-weight:bolder;font-family:\'黑体\'">光盘档案总说明</td></tr>',
              '<tr><td style="background:#FFF;height:800px;" valign="top">',
              '  <textarea id="" name="f_note_content"',
              'class="validate[required,maxSize[500]] textarea_575" ',
              'style="margin-left:10px;width:565px;border: 0px;overflow:auto;height: 780px;font: normal 18px \'微软雅黑,宋体\';padding-top:10px;" ',
              'rows="3">',
              jQuery("#OA_FILE_FILES__F_FILES_CONTENT").val(),
              '</textarea></td></tr>',    
              '<table/></div>'
              ];
   return html.join("");    
...
<div id="vcenter" style="float:left; width:100%;" >
    <div style="float: left; width: 800px;">
        <div class="content_02_box" style="width:780px;">
            ...
            <div id="detailDiv" class="content_02_box_div" style="width:780px;font">
                   ...
                   <th class="content_02_box_div_table_th">
                    ...
                    <tr> <td class="content_02_box_div_table_td" colspan="3" >
                            <oa:textarea column="F_FILES_CONTENT" table="OA_FILE_FILES"
                               other='class="validate[maxSize[1000]] textarea_575" style="width:545px;"' rows="50"/>
                        </td></tr>
                </table>
            </div>
        </div>      
