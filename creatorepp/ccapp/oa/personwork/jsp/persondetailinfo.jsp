...
<%@page	import="com.chinacreator.xtbg.core.personwork.entity.PersonInfoBean"%>
<%@page	import="com.chinacreator.xtbg.core.personwork.entity.PersonExtraShowBean"%>
...
<%
	...
	//根据用户id查询用户信息
	PersonInfoBean personInfoBean = service
			.findPersonBaseInfo(person_id);

	//返回分组的html
	String getGroupSelectHtml = service.getGroupSelectHtml(user_id, is_public);

	//查询人员扩展信息
	List<PersonExtraShowBean> phoneBeanList = service.findPersonExtraInfoList(person_id, "1");
	List<PersonExtraShowBean> foxBeanList = service.findPersonExtraInfoList(person_id, "2");
	List<PersonExtraShowBean> emailBeanList = service.findPersonExtraInfoList(person_id, "3");
	List<PersonExtraShowBean> qqBeanList = service.findPersonExtraInfoList(person_id, "4");
	List<PersonExtraShowBean> birthBeanList = service.findPersonExtraInfoList(person_id, "5");
	List<PersonExtraShowBean> companyBeanList = service.findPersonExtraInfoList(person_id, "6");
	List<PersonExtraShowBean> positionBeanList = service.findPersonExtraInfoList(person_id, "7");
	List<PersonExtraShowBean> addressBeanList = service.findPersonExtraInfoList(person_id, "8");
	List<PersonExtraShowBean> websiteBeanList = service.findPersonExtraInfoList(person_id, "9");

	//是否配置了分组信息
	boolean isConfigGroupInfo = service.isGroupExist(user_id,is_public);
%>
...
<script src="../../resources/util/jsonutil.js" type="text/javascript"></script>
    ...
    $.widget("ui.combobox",
                    {
                        _create : function() {
                            ...
                            var button = this.button = $(
                                    "<input  type='button' class='but_y_01' onMouseOver=javascript:$(this).attr('class','but_y_02'); onMouseOut=javascript:$(this).attr('class','but_y_01');  value='添加到组'/>")
                                    .attr("tabIndex", -1)
                                    .attr("title", "添加到组")
                                    .insertAfter(select)
                                    .autocomplete(
                            ...
                            this.button.data("autocomplete")._renderItem = function(
                                    ul, item) {
                                ...
                                var inputhtml = $("<li></li>").data(
                                        "item.autocomplete", item).append(
                                        "<input type='checkbox'  name='group' onclick='onclickcheckbox()' id='"
                                                + item.option.value + "' "
                                                + ischeck + " value='"
                                                + item.option.text + "'/>"
...
$(function() {
	...
	setDataTimeClickShowOrHide('birth',window);
...
var onclickcheckbox = function() {
    ...
    $("input[name='group']").each(function(i) {
        if ($(this).prop("checked")) {// 如果被选中了
                groupstr += $(this).val() + ",";
                $('#groupshowstr').append(
                        "<span class='personwork_fz'>" + $(this).val()
                                + "&nbsp;&nbsp;</span>")
...
function do_Delete(tag) {
    // 如果当前表格里的行数小于等于2，则表示进行删除操作之后要隐藏掉删除按钮
    if ($(tag).parents("tr").eq(1).find("#del").size() <= 2) {
...
function get_TableValue(name) {
    ...
    if (selectList.size() > 0) {
        selectList.each(function(i, item) {
            if (!IsSpace(inputList.eq(i).val())) {// 如果input框的值不为空
                    ...
                    personinfoListStr = personinfoListStr
                            + getJsonString(personinfoInstance) + "_____";
                }
    ...
    personinfoListStr = IsSpace(personinfoListStr) ? "" : personinfoListStr
            .substring(0, personinfoListStr.length - 5);
...
var saveinfo = function() {
    ...
    var baseinfoJson = "{'person_id':'" + person_id + "','name':'" + nametext
            ...
            + "'," + "'remark':'" + remark
            + "'," + "'group_id':'" + $('#groupshowids').val()
            + "','info_type':'personal','is_public':'<%=is_public%>'}";
 ...
 */
function back() {
    var url = path + "/ccapp/oa/personwork/jsp/personaladdressbook.jsp?is_public=<%=is_public%>";
    /* location.href = url; */
    var okF = function() {
...
<table border="0" cellspacing="0" cellpadding="0" class="content_02_box_div_table">
	<tr>
		<th width="191" class="content_02_box_div_table_th"	style="border-top: none; border-left: none;">
		  <span class="color_red">*</span>姓名：
		</th>
		<td class="content_02_box_div_table_td">
		  <input class="validate[required,maxSize[15]] input_188" id="personname"name="personname"
		         value="<%=StringUtil.deNull(personInfoBean.getName())%>"/>
		</td>
	</tr>
	<tr>
		<th width="191" class="content_02_box_div_table_th"	style="border-top: none; border-left: none;">
		...
		<td class="content_02_box_div_table_td">
		    <input class="validate[maxSize[15]] input_188" id="nickname" name="nickname"
			     value="<%=StringUtil.deNull(personInfoBean.getNick_name())%>"/>
		</td>
	</tr>
	<tr>
		<th width="191" class="content_02_box_div_table_th"	style="border-top: none; border-left: none;">
		...
		<td class="content_02_box_div_table_td">
		    <input class="validate[maxSize[500]] input_188" id="remark" name="remark" style="width: 500px;"
			     value="<%=StringUtil.deNull(personInfoBean.getRemark())%>"/>
		</td>
	</tr>
	<tr>
		<td colspan="3"	style="border: 1px solid #EAEAEA; border-top: none; padding: 10px;">
			<label id="groupshowstr"> 
			<%
			...
			<input id="groupshowids" type="hidden" value="<%=StringUtil.deNull(personInfoBean.getGroup_id())%>"/>
				...
				<td class="content_02_box_div_table_td">
					<table id="phonetable" width="100%">
						...
						<tr class="personwork_Area">
							...
							<td align="left" class="personwork_box_leftblank">
								<input class="validate[custom[phone]] input_188" id="phone" name="phone"
									value="<%=phoneBeanList.get(i).getValue()%>"style="border: solid 1px #C6C6C6;" />
							</td>
							<td>
								<input class='but_y_01' id=add type='button' onclick='do_Add(this)' value="添加"
									onMouseOver="javascript:$(this).attr('class','but_y_02');" onMouseOut="javascript:$(this).attr('class','but_y_01');" />
								<input	class='but_y_01' id=del type='button' value="删除"
									 <%if (phoneBeanList.size() <= 1) {%> style="display: none" <%}%>
									onclick='do_Delete(this)' onMouseOver="javascript:$(this).attr('class','but_y_02');"
									onMouseOut="javascript:$(this).attr('class','but_y_01');" />
						    </td>
						</tr>
						<%
							}
				...
				<td class="content_02_box_div_table_td">
					<table id="foxtable" width="100%" class="personwork_Area">
						...
						<tr class="personwork_Area">
							...
							<td class="personwork_box_leftblank">
								<input	class="validate[custom[fax]] input_188" id="fox" name="fox"
									value="<%=foxBeanList.get(i).getValue()%>" style="border: solid 1px #C6C6C6;"/>
							</td>
							<td>
								<input class='but_y_01' id=add type='button' onclick='do_Add(this)' value="添加"
									onMouseOver="javascript:$(this).attr('class','but_y_02');"
									onMouseOut="javascript:$(this).attr('class','but_y_01');" /> 
								<input
									class='but_y_01' id=del type='button' value="删除"
									<%if (foxBeanList.size() <= 1) {%> style="display: none" <%}%>
									onclick='do_Delete(this)' onMouseOver="javascript:$(this).attr('class','but_y_02');"
									onMouseOut="javascript:$(this).attr('class','but_y_01');" />
							</td>
						</tr>
				...
				<td class="content_02_box_div_table_td">
					<table id="emailtable" width="100%" class="personwork_Area">
						...
						<tr class="personwork_Area">
							...
							<td class="personwork_box_leftblank">
							    <input class="validate[custom[email]] input_188" id="email" name="email"
								   value="<%=emailBeanList.get(i).getValue()%>" style="border: solid 1px #C6C6C6;"/>
							</td>
							<td>
							    <input class='but_y_01' value="添加" id=add type='button'
									onclick='do_Add(this)' onMouseOver="javascript:$(this).attr('class','but_y_02');"
									onMouseOut="javascript:$(this).attr('class','but_y_01');" /> 
								
								<input
									class='but_y_01' value="删除" id=del type='button'
									<%if (emailBeanList.size() <= 1) {%> style="display: none" <%}%>
									onclick='do_Delete(this)'
									onMouseOver="javascript:$(this).attr('class','but_y_02');"
									onMouseOut="javascript:$(this).attr('class','but_y_01');" />
							</td>
						</tr>
					...
					<table id="qqtable" width="100%" class="personwork_Area">
						...
						<tr class="personwork_Area">
							<td class="personwork_box">
							  <input
								class="validate[custom[qq]] input_188" id="qq" name="qq"
								value="<%=qqBeanList.get(i).getValue()%>"/>
							</td>
							<td align="left">
								<input class='but_y_01' value="添加" id="add" type='button' onclick='do_Add(this)'
									onMouseOver="javascript:$(this).attr('class','but_y_02');"
									onMouseOut="javascript:$(this).attr('class','but_y_01');" /> 
								<input class='but_y_01' value="删除" id=del type='button'
									<%if (qqBeanList.size() <= 1) {%> style="display: none" <%}%>
									onclick='do_Delete(this)' onMouseOver="javascript:$(this).attr('class','but_y_02');"
									onMouseOut="javascript:$(this).attr('class','but_y_01');" />
							</td>
						</tr>
...
<div>
	<div class="content_02_box_div">
	    <div class="content_02_box_title_bg"><span>更多详情</span></div>
			...
			<tr class="personwork_Area">
				...
				<td class="content_02_box_div_table_td">
					<table width="100%" cellpadding="0" id="birthtable">
						<tr>
							<td class="personwork_box">
								<input class="date_187" id="birth"
									onclick="WdatePicker({maxDate:'%y-%M-%d'})" style="width: 188px;" name="birth"
									readonly="readonly" value="<%=birthBeanList.get(0).getValue()%>"/>
							</td>
							<td>
								<input class='but_y_01' value="删除" id=del type='button'
									<%if (birthBeanList.size() <= 1) {%> style="display: none" <%}%>
									onclick='do_Delete(this)' />
							</td>
						</tr>
					</table>
				...
				<td class="content_02_box_div_table_td">
					<table id="companytable" width="100%">
						<tr>
							<td class="personwork_box">
								<input
									class="validate[maxSize[50]] input_188"
									style="border: solid 1px #C6C6C6; width: 183px;" name="company"
									id="company" value="<%=companyBeanList.get(0).getValue()%>"/>
							</td>
							<td>
								<input class='del' id=del type='button'
									<%if (companyBeanList.size() <= 1) {%> style="display: none" <%}%>
									onclick='do_Delete(this)' />
							</td>
						</tr>
					</table>
					...
					<table id="positiontable" width="100%">
						<tr>
							<td class="personwork_box">
								<input
									class="validate[maxSize[50]] input_188"
									style="border: solid 1px #C6C6C6; width: 183px;" name="position"
									id="position" value="<%=positionBeanList.get(0).getValue()%>"/>
							</td>
							<td>
								<input class='del' id=del type='button'
									<%if (positionBeanList.size() <= 1) {%> style="display: none" <%}%>
									onclick='do_Delete(this)' />
							</td>
						</tr>
					</table>
				...
				<td class="content_02_box_div_table_td">
					<table id="addresstable" width="100%">
						...
						<tr class="personwork_Area">
							...
							<td class="personwork_box_leftblank">
								<input
									class="validate[maxSize[100]] input_188" id="address"
									name="address" value="<%=addressBeanList.get(i).getValue()%>"
							...
							<td>
								...
								<input
									class='but_y_01' value="删除" id=del type='button'
									<%if (addressBeanList.size() <= 1) {%> style="display: none" <%}%>
									onclick='do_Delete(this)'
									onMouseOver="javascript:$(this).attr('class','but_y_02');"
									onMouseOut="javascript:$(this).attr('class','but_y_01');" />
							</td>
				...
				<td class="content_02_box_div_table_td">
					<table id="websitetable" width="100%">
						...
						<tr class="personwork_Area">
							...
							<td class="personwork_box_leftblank">
								<input class="validate[maxSize[100]] input_188" id="website"
									name="website" value="<%=websiteBeanList.get(i).getValue()%>"
									style="border: solid 1px #C6C6C6;"/>
							</td>
							<td>
								<input class='but_y_01' value="添加" id=add type='button'
								...
								<input
									class='but_y_01' value="删除" id=del type='button'
									<%if (websiteBeanList.size() <= 1) {%> style="display: none" <%}%>
									onclick='do_Delete(this)'
									onMouseOver="javascript:$(this).attr('class','but_y_02');"
									onMouseOut="javascript:$(this).attr('class','but_y_01');" />
							</td>
