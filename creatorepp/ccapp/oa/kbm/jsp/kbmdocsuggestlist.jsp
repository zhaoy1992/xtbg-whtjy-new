...
<%
	...
	String k_user = accesscontroler.getUserID();
	UserCacheBean usercachebean=UserCache.getUserCacheBean(k_user);
%>
<title>知识库推荐文档界面</title>
<!-- 引入jQuery -->
...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%=path%>' + '/pubListServlet?classNameId=com.chinacreator.xtbg.core.kbm.list.KbmDocSuggestList',  //pubListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
	 ...
	 */
	 var toJson = function() {
			var str ="{'k_title':'"+$("#k_title").val()+
				"','k_code':'"+$("#k_code").val()+
				"','k_user':'<%=k_user%>','k_dept_name':'"+$("#k_dept_name").val()+
					...
					<table id="searchTable" width="100%" border="0" cellspacing="3"
						...
						<tr>
							...
							<td  class="cx_title_td"><input type="text"
								...
								name="k_title" /></td>
							<th style="width: 70px;" class="input_cx_title_th">编号：</th>
							...
							<td width="60"><input value="搜索" id="sel" type="button"
						...
						<tr>
							...
							<th class="input_cx_title_th" style="width: 120px;">关键字和内容：</th>
							...
							<td width="60"><input value="重置" id="reset" type="button" onclick="clear111();" class="but_y_01" /></td>
						</tr>
					</table> <!-- 固定查询 end-->
				</td>
			</tr>
