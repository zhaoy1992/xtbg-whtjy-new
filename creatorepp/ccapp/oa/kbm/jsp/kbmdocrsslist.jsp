...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%=path%>' + '/pubListServlet?classNameId=com.chinacreator.xtbg.core.kbm.list.KbmDocRssList',  //pubListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		toolbar : [ true, "top","<input class='but_y_01' onMouseOver=javascript:$(this).attr('class','but_y_02'); onMouseOut=javascript:$(this).attr('class','but_y_01'); id=add type='button' onclick='addDocRss()' value='新增'/><input class='but_y_01' onMouseOver=javascript:$(this).attr('class','but_y_02'); onMouseOut=javascript:$(this).attr('class','but_y_01'); id=del type='button' value='删除' onclick='delDocRss()'/>"],
		multiselect: true,
...
/*
	 ...
	 */
var toJson = function() {
	var str ="{'k_type_name':'"+$("#k_type_name").val()+
			 "','k_key':'"+$("#k_key").val()+"','k_rss_user':'<%=userid%>'}";
...
返回值: 无
*/
var addDocRss = function() {
	openWindows('add','新增','<%=path%>/ccapp/oa/kbm/jsp/kbmdocrssinfo.jsp?&subid=add','qzh',false,window);
}
...
var update = function(id) {
		...
		openWindows('update','修改','<%=path%>/ccapp/oa/kbm/jsp/kbmdocrssinfo.jsp?subid=update&k_rss_id='+ model.k_rss_id, 'cxjm', false, window);
	}
			...
			<tr>
				<td>
					...
					<table id="searchTable" width="100%" border="0" cellspacing="3"
						...
						<tr>
							<th class="input_cx_title_th" style="width: 90px;">所属分类：</th>
							...
							<th style="width: 70px;" class="input_cx_title_th">关键字：</th>
							<td style="width: 150px;" class="cx_title_td"><input type="text"
								id="k_key" name="k_key"
...
<form id="delForm1" name="delForm1" method="post" action="kbmdocrssdo.jsp">
	<input type="hidden" name="k_rss_id" id="k_rss_id" /> 
