...
if ( isok == "ok"){
	alert("新增注册用户成功！用户帐号："+name+"  密码："+password);
}
//修改会员
function getMember(memberId,userType){
	userList.action="../member/memberManager.do?method=getMember&memberId="+memberId+"&userType="+userType+"&flag=1";
	userList.submit();
}
...
function dealRecord(dealType) {
    ...
    if (isSelect){
    	if (dealType==1){
        	...
        	if (confirm(outMsg)){
				userList.action="../member/memberManager.do?method=deleteMember&flag=1";
				userList.submit();
	 			return true;
			}
		} 
...
function newMember()
{
	userList.action="../member/memberManager.do?method=newMember&flag=1"
	userList.submit();		
}
//会员查询
function queryUser()
{	
	...
	userList.action="../sitemember/enroll_memberList.jsp"
	userList.submit();	
}
         				...
         				<TR>
         					<td  colspan="2" height='30'valign='middle' align="right" width="40%">
         						<input name="search" type="button" class="input" value="查询" onClick="queryUser()">
           						...
           						<input name="onuser" type="button" class="input" value="转为收费用户" onclick="javascript:onMember(1); return false;">
					...
					<table cellspacing="1" cellpadding="0" border="0" bordercolor="#EEEEEE" width=100% class="thin">
						...
						<pg:pager maxPageItems="13" scope="request" data="MemberList" isList="false">
							<tr>
								...
								<td class="headercolor"  width="45%">联系地址</td>
