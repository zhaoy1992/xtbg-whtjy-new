...
<%!/*' ============================================
	...
	public void InitUpload(String realpath) {
		...
		uw.filename = realpath + "cms/templateManage/eWebeditor/conf/style.xml";
		uw.getInstance();
...
<%
	...
	if (sAction.equalsIgnoreCase("remote")) { //远程自动获取
		...
		if (sAllowExt != "") {
			Pattern pRemoteFileurl = Pattern.compile("((http|https|ftp|rtsp|mms):(//|\\\\){1}(([A-Za-z0-9_-])+[.]){1,}(net|com|cn|org|cc|tv|[0-9]{1,3})(\\S*/)((\\S)+[.]{1}("
			...
			while (result) {
				...
				while (mFileType.find()) {
					...
					+ (String) date.Time_Stamp() + i + SaveFileType;//文件的路径，以时间戳命名
				}
				...
				SaveFileName = config.getServletContext().getRealPath("/")
							+ LoadFile
							+ "\\"
							+ LocalFileurl.substring(LocalFileurl.lastIndexOf("/") + 1);
		...
		out.println("<input type=hidden name=siteId value=\"\">");
		out.println("</form>");
		out.println("<script language=javascript>");
		...
		up.upload();
		//add by lj
		//如果没有接受到站点对象,或者没有没有找到站点信息,将继续用原先的默认的
		String savapath = CMSUtil.getCMSContextPath()+CMSUtil.getWebSiteRootPath();
		String siteId = request.getParameter("siteId");
		if(siteId != null && siteId.trim().length()!=0){
			SiteManager sm = new SiteManagerImpl();
			Site site = sm.getSiteInfo(siteId);
			if(site!=null){
				String dir = site.getSiteDir();
				if(dir!=null && dir.trim().length()!=0){
					savapath = savapath+"/"+dir+"/Template_Files";
				}
					
			}
		}
		sUploadDir = savapath + "/";
		java.io.File f = new java.io.File(rootpath,sUploadDir);
		if(!f.exists()){
			f.mkdirs();
		}
		System.out.println("文件上传的最后目录:"+f.getAbsolutePath());
		//end lj		

		// Select each file
		System.out.println("sUploadDir="+sUploadDir);
