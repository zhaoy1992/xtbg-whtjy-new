...
<%
	...
	//初始化处理bean
	web.filename = config.getServletContext().getRealPath("/")+"cms/templateManage/eWebeditor/conf/style.xml";
	web.filename2 = config.getServletContext().getRealPath("/")+"cms/templateManage/eWebeditor/conf/button.xml";
	web.getInstance();
	//初始化输出bean
...
<html>
	<head>
		...
		<Script Language=Javascript src="../js/extend.js"></Script>
					...
					<table border=0 cellpadding=0 cellspacing=0 width='100%' height='100%'>
						<tr>
							<td height='100%'>
								...
								<iframe class="Composition" ID="eWebEditor"  
									MARGINHEIGHT="1" MARGINWIDTH="1" width="100%" height="100%" scrolling="yes">
								</iframe>
							</td>
						</tr>
					</table>
								...
								<table border=0 cellpadding=0 cellspacing=0 height=20>
									<tr>
										...
										<td class=StatusBarBtnOff id=eWebEditor_VIEW onclick="setMode('VIEW')">
											<img border=0 src="buttonimage/<%=sStyleDir%>/modepreview.gif" 
												width=50 height=15 align=absmiddle>
										</td>
									</tr>
								</table>
...
<% } %>
		...
		<div id="eWebEditor_Temp_HTML" 
			style="VISIBILITY: hidden; OVERFLOW: hidden; POSITION: absolute; WIDTH: 1px; HEIGHT: 1px">
		</div>

		<form id="eWebEditor_UploadForm" action="upload.jsp?action=remote&type=remote&style=<%=sStyleName%>" 
