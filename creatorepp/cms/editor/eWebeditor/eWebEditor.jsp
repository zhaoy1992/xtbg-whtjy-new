...
if(edittype != null && edittype.equals("self"))//自定义显示的button
{
	web.filename = CMSUtil.getPath(config.getServletContext().getRealPath("/"),"/cms/editor/eWebeditor/config/StyleSelf.xml");
}
else
{
	web.filename = CMSUtil.getPath(config.getServletContext().getRealPath("/"),"/cms/editor/eWebeditor/config/style.xml");
}
web.filename2 = CMSUtil.getPath(config.getServletContext().getRealPath("/"),"/cms/editor/eWebeditor/config/button.xml");
web.getInstance();
//初始化输出bean
...
nStateFlag = bean.getNStateFlag();
		...
		//获取频道id
		String docpath = request.getParameter("docpath");//System.out.println("fdsjfjd==="+docpath);
 %>
<html>
<head>
...
config.StyleEditorHeader = "<head><link href=\""+sPath+"css/<%=sStyleCSS%>/EditorArea.css\" type=\"text/css\" rel=\"stylesheet\"><script language=\"JavaScript\" src=\""+sPath+"include/killerror.js\"><" + "/script></head><body MONOSPACE>" ;
