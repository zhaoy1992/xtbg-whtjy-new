...
function setHTML(html) {
	...
	if(bEditMode){
		...
		eWebEditor.document.open();
		eWebEditor.document.write(config.StyleEditorHeader+html);
		//eWebEditor.document.body.innerHTML=setPageTagDisplay(sBody);
		eWebEditor.document.body.contentEditable="true";
		eWebEditor.document.oncontextmenu=new Function("return showContextMenu(eWebEditor.event);");
		eWebEditor.document.execCommand("2D-Position",true,true);
		eWebEditor.document.execCommand("MultipleSelection", true, true);
...
function setMode(NewMode){
	if (NewMode!=sCurrMode){
		...
		try{
			...
			//document.all["eWebEditor_VIEW"].className = "StatusBarBtnOff";//屏蔽预览功能
		...
		case "EDIT":
			eWebEditor.document.designMode="On";
			eWebEditor.document.open();
			eWebEditor.document.write(config.StyleEditorHeader + setPageTagDisplay(sBody));
			//eWebEditor.document.body.innerHTML=setPageTagDisplay(sBody);
			eWebEditor.document.body.contentEditable="true";
			eWebEditor.document.oncontextmenu=new Function("return showContextMenu(eWebEditor.event);");
			eWebEditor.document.execCommand("2D-Position",true,true);
...
//	var arPageTags = eWebEditor.document.body.getElementsByTagName("TRS_PAGE_SEPARATOR");
//	for(var i =arPageTags.length-1; i>=0; i--){
//		var objPageTag = arPageTags[i];
//		if(objPageTag == null)continue;
		
//		var spanTag = eWebEditor.document.createElement("img");
//		spanTag.src = "buttonimage/standard/page.gif";		
//		//spanTag = objPageTag.insertAdjacentElement("beforeBegin", spanTag);
//		//设置样式
//		/*spanTag.style.border = "1 dashed red";
//		spanTag.style.fontSize = "9pt";
//		spanTag.style.color = "blue";*/
//		//设置只读
//		//spanTag.onselectstart = noValid;
//		spanTag.id = "_TRSPage";		

//		objPageTag.replaceNode(spanTag);
//	}
	text = text.replace(/<CMS_PAGE_SEPARATOR><\/CMS_PAGE_SEPARATOR>/g, "<img src=\"buttonimage/standard/page.gif\" id=\"_CMSPage\">");
	return text;
...
function insertTRSPage(){
	...
	//focus
	eWebEditor.focus();
	insertHTML("<img src=\"buttonimage/standard/page.gif\" id=\"_CMSPage\">");
}
