...
function fG(hA) {
    ...
    var html = "<iframe name='myuploadformtarget' style='display:none;position:absolute;width:0px;height:0px' src='blank.htm'></iframe>" 
    			+ "<form action='../" + config.ServerExt + "/upload." + config.ServerExt + "?action=save&type=" + hA + "&style=" + config.StyleName + "&cusdir=" + config.CusDir + "' method=post name=myuploadform enctype='multipart/form-data' style='margin:0px;padding:0px;width:100%;border:0px' target='myuploadformtarget'>" 
	    			+ "<input type=hidden name='MAX_FILE_SIZE' value='" + oH + "'>" 
	    			+ "<input type=file name='uploadfile' size=1 style='width:100%' onchange=\"this.form.originalfile.value=this.value;try{ct();} catch(e){}\">"
	    			+ "<input type=hidden name='originalfile' value=''>"
	    			
	    			+ "<input type=hidden name='_addwater' value=''>" 
	    			+ "<input type=hidden name='_waterimage' value=''>"
	    			+ "<input type=hidden name='_position' value=''>" 
	    			+ "<input type=hidden name='_waterStr' value=''>" 
	    			+ "<input type=hidden name='_backup' value=''>" 
    			+ "</form>";
    return html;
}
