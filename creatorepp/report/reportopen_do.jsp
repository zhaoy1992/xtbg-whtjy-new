...
<%
	...
	String reqFileName = request.getParameter("repId");
	
	ReportManage rm = ReportBeanFactory.getReportManager();
	Tb_report reportObj = rm.getReport(reqFileName);
	...
	String fileName = filePath+reqFileName+pix;
	ReportUtil u = new ReportUtil();
	u.creatFile(fileName,rm.getReportFileBytes(reqFileName),false);
	...
	+"<argument>"+control.getUserAccount()+"</argument>"
	+"<argument>"+serverUrl+"/report/reportFiles/"+reqFileName+pix+"</argument>"
	+"</application-desc>"
	+"<security>"
	+"<all-permissions/>"
	+"</security>"
