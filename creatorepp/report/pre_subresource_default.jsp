...
<head>
  ...
  <script type="text/javascript">
  	...
  	function showPicture(resource_id) {
  		showModalDialog("show_picture.jsp?resource_id="+resource_id,null,"dialogWidth:250px;dialogHeight:250px;dialogLeft:160px;dialogTop:200px;help:no");
  	}
	...
	function alertWarnWhenDelete() {
		...
		var retX=SendHttp("http://<%=request.getServerName()%>:<%=request.getServerPort()%><%=request.getContextPath()%>/reportTools?op=showwarnwhendelete&ids="+b.value);
		if(retX != "") {
			if(window.confirm(retX)){
				var ret=SendHttp("http://<%=request.getServerName()%>:<%=request.getServerPort()%><%=request.getContextPath()%>/reportTools?op=deletesubresource&ids="+b.value);
				alert(ret);
				window.location.reload();
				}
				return;
		}
		var re=SendHttp("http://<%=request.getServerName()%>:<%=request.getServerPort()%><%=request.getContextPath()%>/reportTools?op=deletesubresource&ids="+b.value);
		alert(re);
		window.location.reload();
	}
