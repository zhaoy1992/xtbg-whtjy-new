...
function turnPageSumbmitSet(){
	...
	<%@ include file="/epp/css/cssControl.jsp"%><body >
		<form name="dictList" method="post" action="pageSaveandDel.jsp" target="hiddenFrame">
		    <!--为翻页提交做准备-->
			...
			<tr align="right" style="text-align:left">
				...
				<pg:listdata dataInfo="com.chinacreator.dictionary.tag.UserUsualDictListSelected" keyName="UserUsualDictList" />



				<!--分页显示开始,分页标签初始化-->
				<!--为翻页提交做准备 form="dictList" promotion="false" commitevent="turnPageSumbmitSet()" -->
				<pg:pager maxPageItems="10" scope="request" form="dictList" promotion="true"  commitevent="turnPageSumbmitSet()"  data="UserUsualDictList" isList="false">
					...
					<tr>							
						<td class="tablecells" style="width:20%" nowrap="nowrap" style="text-align:center">
							<pg:equal colName="dataValidate" value="1">
								<input type="checkbox" name="dictdataValue" value="<pg:cell colName='value' defaultValue='' />:<pg:cell colName='name' defaultValue='' /><%if(!data_org.equals("") && data_org != null){%>:<pg:cell colName="dataOrg" defaultValue="" /><%}%>" />
                            </pg:equal>
						    <pg:equal colName="dataValidate" value="0">
						        <input type="checkbox" disabled="true" name="dictdataValue" value="<pg:cell colName='value' defaultValue='' />:<pg:cell colName='name' defaultValue='' /><%if(!data_org.equals("") && data_org != null){%>:<pg:cell colName="dataOrg" defaultValue="" /><%}%>" />
						    </pg:equal>
						</td>									
	...
	function saveReadUnsetup(){
		...
		if(n == 0){
			...
			dictList.action = "saveOrgTaxcodeUsual.jsp?dicttypeId="+dicttypeId+"&orgId="+orgId+"&state=usual";
			dictList.submit();
		}
	}
	...
	function saveAllReadUnsetup(){
		...
		dictList.action = "saveOrgTaxcodeUsual.jsp?identifier=sealed&dicttypeId="+dicttypeId+"&orgId="+orgId+"&state=allUsual";
		dictList.submit();
	}	
	</script>
</html>
