...
//			roleManager.deletePermissionOfRole(resids,resTypeId,tmp,role_type);	
			...
			}if("report_column".equalsIgnoreCase(resTypeId)){
				resids.add(new String[]{resid ,resName});
				CSMenuManager csmenu = new CSMenuManagerImpl();
				List reportList = csmenu.getReportMenuItems(resid);
				for(int i = 0; i < reportList.size(); i++){
					ReportMenu reportMenu = (ReportMenu)reportList.get(i);
					//System.out.println("reportMenu.getId() = " + reportMenu.getId());
					resids.add(new String[]{reportMenu.getId(),reportMenu.getName()});
				}
			}else{
//				List resids = new ArrayList();
				resids.add(new String[]{resid , resName});
