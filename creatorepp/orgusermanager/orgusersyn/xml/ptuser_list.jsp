...
<%@ include file="/epp/css/cssControl.jsp"%>
  ...
  function pttbxml(type){
  	//var savePath = document.getElementById("savePath").value;
  	var queryString2 = document.getElementById("queryString2").value;
  	//if(savePath==""){
  		//alert("请输入你要导出的文件目录");
  		//return false;
  	//}
  	...
  	if(confirm("确定导出吗？")){
  		//document.getElementById("dir").value=savePath;
  ...
  //校验目录格式
  function checkDir(){
　　　var tt = document.getElementById("savePath").value;
...
　　　}
  ...
  function getDir(){
  	try{
  		var objSrc = new ActiveXObject("Shell.Application").BrowseForFolder(0,'请选择导出路径',0x0004 ,0x0011);      
		if(objSrc   !=   null){       
			savePath   =   objSrc.Items().Item().Path;   
			document.getElementById("savePath").value   =   savePath;     
      	}
    }catch(e){
    	alert("没有权限!1.设置可信任站点..2.可信任站点安全级别自定义中设置:'对没有标记为安全的ActiveX控件进行初始化和脚本运行--启用'");
    }
  }
  function jumpPage(){
  ...
  <body>
			<form name="dosave" method="post" action="exp_xml.jsp" target="xmlhiddenFrame">
			...
			<table cellspacing="0" cellpadding="0" border="0" bordercolor="#EEEEEE" width=100% class="thin">
				<tr><td align="center">平台导出到XML</td></tr>
			</table>
		  		<table cellspacing="0" cellpadding="0" border="0" bordercolor="#EEEEEE" width=100% class="thin">
			  			<tr>
	           				    <td colspan=3>
							...
							<div style="width:100%;height:380;overflow:auto;">
		  					<table cellspacing="0" cellpadding="0" border="0"> 
								...
								--%><input name="queryString" value="<%=queryString %>" type="hidden"/>
									<input name="orgName" value="<%=orgName %>" type="hidden">
								</pg:pager>
						  	</table>
						  	</div>
					  	</td>
					...
					<tr height="18px" class="labeltable_middle_tr_01">
						<td align='center' colspan=3 nowrap>
							<!--<input name="savePath" type="text" value="" readonly/>
							<input type=button name=bt0 value="目录选择" class="input" onclick="getDir()"/>	
							<font color=red>导出平台用户，机构，用户机构关联</font> -->
						</td>
					</tr>
				</table>
				<div align="right">
				<input type=button name=bt1 value="上一步" class="input" onclick="jumpPage();"/>
				<input type=button name=bt1 value="导出" class="input" onclick="pttbxml('exp')"/>					
