...
function link(orgId,orgName){
	...
	document.rtxuser_list.submit();
	//parent.ptuser_list.location.href = "<%=request.getContextPath()%>/orgusermanager/orguser_tb/ptrtx/ptuser_list.jsp?queryString="+queryString+"&orgId="+orgId+"&resName="+orgName+"&queryString1="+queryString1;
}
function pttb(type){
var confirmStr = "";
	if(type=='ptzl2rtx')confirmStr = "确定同步吗？该同步将完成平台机构用户到RTX的一次性增量同步，对RTX用户不做删除，直接新增或修改";
...
--%><form name="orgtreeform" action="" method="post"  >
    ...
    <table >
    	<%--<tr><td><input type=button name=bt2 value="返回" class="input" onclick="parent.window.history.go(-1)"/>	</td>
    	</tr>
        --%><tr><td align="left">
         <tree:tree tree="rtx_org_tree"
    	           node="rtx_org_tree.node"
    			   ...
    			   mode="static-dynamic"
    			   >     
    			   <tree:treedata treetype="com.chinacreator.epp.util.orgusertb.RxtOrgTree"
    	                   scope="request"
    	                   rootid="0"  
    	                   rootName="RTX机构树"
    	                   expandLevel="1"
...
<form name="rtxuser_list" action="rtxuser_list.jsp" target="rtxuser_list" method="post">
