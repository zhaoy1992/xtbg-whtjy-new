...
<%AccessControl accesscontroler = AccessControl.getInstance();
	...
	java.util.List list = rtxManager.selectAllRxtUser(tmpString,uid,cn,orgId,orgName);
	if(list!=null){
...
<%@ include file="/epp/css/cssControl.jsp"%>
  ...
  function rtxtbpt(type){
  	...
  	//if(type=='rtxwq2pt'&&allDept_User.split(":")[0]==''){//完全同步时必须选择机构
  	//	alert("请选择你要同步的机构后再选择要同步到该机构下的用户");
  	//	return false;
  	//}
  	var message = "";
  	if(type=='rtxzl2pt'){
  		message = "该操作只同步与选择用户关联的机构及其父机构,\n若rtx某机构下没有用户或者用户没有被选中，该机构将不会被同步。\n注意：平台与RTX有相同用户时需要确保用户与机构的关系一致";
  	}
  	if(type=='rtxwq2pt'){
  		message = "完全同步将把rtx所有机构增量同步到平台，先删除平台下同步用户及其机构用户关系，再根据RTX机构用户更新平台";
  	}
  	if(confirm("确定同步吗？\n"+message)){
  ...
  function checkOrgUser(allDept_User){
  	var orguser = allDept_User.split(";");
  }
  ...
  <body>
			<form name="dosave" method="post" action="do_rtx.jsp" target="rtxhiddenFrame">
				<input name="allDept_User" value="" type="hidden">
				<input name="type" value="" type="hidden">
			</form>
			<form name="userList" method="post" action="rtxuser_list.jsp" onsubmit="document.getElementById('orgId').value='';">
		  		<table cellspacing="0" cellpadding="0" border="0" bordercolor="#EEEEEE" width=100% class="thin">
				<tr><td align="center">RTX同步到平台</td></tr>
				</table>
		  		<table cellspacing="0" cellpadding="0" border="0" bordercolor="#EEEEEE" width=100% class="thin">
			  			<tr>
	           				    <td colspan=3>
						...
						<tr>
	           				    <td colspan=3 align="center">
	           				    		<input type="submit" name="bt_cx" class="input" value="查询RTX用户">
	           				    </td>
	           			</tr>
						<tr>
						<td valign=top>
							<div style="width:100%;height:430;overflow:auto;">
		  					<table cellspacing="0" cellpadding="0" border="0"> 
		         				<pg:listdata dataInfo="com.chinacreator.epp.util.orgusertb.Rtx_UserList" keyName="UserListSn" />
		         				<!--分页显示开始,分页标签初始化-->
								<pg:pager maxPageItems="15" scope="request" data="UserListSn" isList="false">
									<tr><td align=center nowrap colspan=6><B>RTX用户列表</B></td></tr>
									<tr>
										<!--设置分页表头-->
						...
						<td width="5%" align="center" valign="middle">
					  	...
					  	<td align="left" width="25%" valign="top">
					  			<b>平台机构【<%=resName %>】用户</b>
								<pg:listdata dataInfo="com.chinacreator.epp.util.orgusertb.Sel_Rtx_UserList" keyName="sel_user_id_list" />
								<pg:pager maxPageItems="10" scope="request" data="sel_user_id_list" isList="false">
								<select name="select" multiple style="width:100%" size=27>
									<pg:list>
				...
				<div align="right">
					...
					<input type=button name=bt1 value="同步&gt;平台" class="input" onclick="rtxtbpt('rtxzl2pt')"/>
					<input style="display:none" type=button name=bt2 value="完全同步&gt;平台" class="input" onclick="rtxtbpt('rtxwq2pt')"/>						
