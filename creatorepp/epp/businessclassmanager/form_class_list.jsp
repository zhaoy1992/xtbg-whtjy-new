...
<html>
	...
	function createFormClass()
	{
		...
		var ret=window.showModalDialog('addFormClass.jsp?action=add&ec_id=<%=formClass.getEc_id()%>',window);
		if(ret) window.location.reload();
	}
	
	function editFormClass(){	    
	    var ret=window.showModalDialog('addFormClass.jsp?action=edit&ec_id=<%=formClass.getEc_id()%>',window);
		if(ret) window.location.reload();
	}
