...
select pri_id,
       ...
       '' m_close_time,
       m_is_need_signup
  from (select f.circularize_id as pri_id,
               f.circularize_title as work_name,
               '传阅文件' info_type,
               to_char(f.entering_date, 'yyyy-mm-dd hh24:mi') as start_time,
               f.receive_userid,
               f.is_circularize,
               f.action_form as m_check_flag_name,
               f.ins_id as m_is_need_signup,
               f.busi_id as fk_id,
               f.send_userid,
               u.user_realname as input_user,
               f.busitype_code as m_check_flag,
               decode(f.is_view, '1', '已查看', '未查看') as isViews,
               f.template_id
          from oa_circularize_flow f, td_sm_user u
         where 1 = 1
           and f.receive_userid = userid
           and u.user_id = f.send_userid
           and f.is_view is null
         order by start_time desc) tt
         where tt.pri_id is not null
    );
 ...
 where a.receive_userid = userid;
  return(waitsp || ',' || waitread || ',' || waitarchive|| ',' || waitprocess || ',' || meetingnotice|| ',' || waittask);
end getCountInfo;
