...
<%
	...
	//有问必答 流程ID
	String sql = "select i.busitype_code, i.busitype_name from oa_flow_busitype i where i.is_valid = 'Y' and  instr('uuid_ywbd',i.busitype_code)>0";
...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/pubListServlet?classNameId=com.chinacreator.xtbg.tjy.answers.list.DoneAnswersList',  //pubListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['实例id','业务id','表单','办件编号 ',  '标题  ','问题类型','提出人','提出时间','当前环节 ','状态', '办件历史 ', 'busitype_code', 'act_index','环节实例ID','流程ID','模板ID','流程定义ID'],
...
function getbackFlow(id){
	...
	var url = "<%=path%>/ccapp/oa/process/flowprocessing/jsp/getbackflowajax.jsp";
