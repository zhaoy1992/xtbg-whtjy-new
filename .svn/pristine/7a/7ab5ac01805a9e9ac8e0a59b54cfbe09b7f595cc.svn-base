...
日期：2014-04-08
--%>
<%@page import="com.chinacreator.xtbg.tjy.device.entity.DevCompareRecode"%>
...
<%
	...
	if("addInfo".equals(action)){
		responseCode = "1";
		DevCompareRecode bean = new DevCompareRecode();
		bean = (DevCompareRecode)RequestBeanUtils.getValuesByRequest(request,bean);
		String beanId = service.insertOrUpdateDevCompareRecode(bean);
	...
	if("deleteInf".equals(action)){
		...
		if(deleteArr.length>0){
			flagStr = service.deleteDevCompareRecodeByIds(deleteArr);
		}
