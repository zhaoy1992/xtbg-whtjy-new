...
<%
	...
	String GET_LIST_CLASS1="com.chinacreator.xtbg.tjy.detectionsupplies.list.OfficeDetailList";
...
var addLevel = function() {
	...
	openAlertWindows('windowId',"../../../tjy/detectionsupplies/jsp/officedetailinfo.jsp?windowsId=windowId&actionName=insert&org_id=<%=org_id%>",pageName,550,550,'25%','25%');
}
...
var updateLevel = function(id) {
	...
	openAlertWindows('windowId','../../../tjy/detectionsupplies/jsp/officedetailinfo.jsp?windowsId=windowId&actionName=update&org_id=<%=org_id%>&id='+ model.id,pageName,550,550,'25%','25%');
}
...
var importExcel = function(){
	...
	var url = '${pageContext.request.contextPath}/ccapp/oa/resources/util/excel/excelimport.jsp?excelImplClass=officedetailDaoExcelImpl1&excelTemplateFilePath=detectiondetail_excelTemplateFilePath&title='+title+'&remark='+remark;
...
function exportFile(){
    ...
    html.push("<input type='hidden' name='excelImplClass' value='officedetailDaoExcelImpl1' /> ");//导入导出的实现类，要提前在spring中或orcleConfig中配置好
    html.push("<input type='hidden' name='excelTemplateFilePath' value='detectiondetail_excelTemplateFilePath' /> ");//导出的excel模版路径，要提前在系统properties中配置好
    html.push('</form>');
    html.push("</div>");
    //加载到body，提交后再删除
