...
//tab2
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/commonListServlet?classNameId=officeEqStockList',  //commonListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['库存ID', '设备名称', '类别',' 设备编号','型号','购买日期','供货厂商','售后电话','单价(元)',' 状态 '],
		...
		filterNames:["state"],
		filterOptions:[[[" ","状态"],["1","在库"],["2","出库"],["3","报废"]]],//alue text 后的值与显不
		...
		toolbar:[true, "top" ,"<input class='but_y_01' id=add value='入库' type='button' onclick='addLevel()' value=''/><input class='but_y_01' value='删除' id=del type='button' onclick='deleteStockLevel()'/><input class='but_y_01' value='库存明细' id=del type='button' onclick='stockLevel()'/>"],
		multiselect: true,
...
*/
 var updateLevel = function(id){
	...
	openWindows("windowId","当前物品的库存信息","../../officeassets/jsp/SeeStcokDetailed.jsp?widowsid=windowId&stock_Id=" + model.stock_id,'officeEqStock',false,window);
}
...
var toUserParamJosn = function() {
		var str = "{'stock_startTime':'"+$('#input_starttime').val()+"','stock_endTime':'"+$('#input_endtime').val()+"',"+
			"'equipment_name':'"+$('#equipment_name').val()+"','equipment_no':'"+$('#equipment_no').val()+"','equipment_model':'"+$('#equipment_model').val()+ "',"+
...
var addLevel = function(id) {
	openWindows("windowId","办公室入库","../../officeassets/jsp/addOfficeStock.jsp?widowsid=windowId",'officeEqStockList',false,window);
}
/*
	 函数:  updateLevel
	 说明:  修改缓急程度
	 ...
	 返回值: 无
	 */
var stockLevel = function(id) {
	var model = jQuery("#gridTable").jqGrid('getRowData', id);
	openWindows("windowId","库存明细","../../officeassets/jsp/SeeStcokDetailed.jsp?widowsid=windowId&apply_id=" + model.apply_id,'PurchaseApply',false,window);
}
					...
					<!-- 隐藏查询 -->
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabs_search_ctable" id="hiddenArea" style="display:none">
							<tr>
								...
								<th class="input_cx_title_th">型号：</th>
								<td  class="cx_title_td">
									<input class="input_cx_title_283" id="equipment_model" name="equipment_model" type="text" />
								</td>
							</tr>
							<tr>
								...
								<td  class="cx_title_td">
									...
									<th class="input_cx_title_th">购买日期：</th>
								<td  class="cx_title_td">
...
<form id="delForm2" name="delForm2" method="post" action="addOfficeStockDo.jsp">
	<input type="hidden" name="userIds2" id="userIds2"/> 
