...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/pubListServlet?classNameId=com.chinacreator.xtbg.tjy.processing.list.ToBeDeWorkList',  //pubListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['实例id','业务id','环节实例id','ccformid','流程定义ID','办件编号 ', '公文标题 ','业务类别','业务类型 ','业务类型CODE',  '发起人 ', '发起时间 ', '当前环节 ','操作', '表单','模板ID'],
...
var delFlow = function(id){
	...
	alert('删除事务实例会将相关联的事务数据全部删除，确定执行吗？ ',p)
