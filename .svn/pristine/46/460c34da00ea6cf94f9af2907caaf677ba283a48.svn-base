...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/pubListServlet?classNameId=com.chinacreator.xtbg.tjy.officesupplies.list.OsAppRegYhpList',  //pubListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['t_appregyhp_id','申领标题','申领部室id','申领部室','申领人id','申领人','填表时间','是否出库','操作'],
		...
		colModel: [ //最重要的数组之一，用于设定各列的参数
        {
   			...
   			hidden:true
       	   	...
       	   	name: 't_appregyhp_title',
   			index: 't_appregyhp_title',
   			width: 8
       	},{
       	   	name: 't_appregyhp_org_id',
   			index: 't_appregyhp_org_id',
   			hidden:true
       	},{
       	   	name: 't_appregyhp_org_name',
   			index: 't_appregyhp_org_name',
   			width: 8
       	},{
   			...
   			width: 8
       	   	...
       	   	name: 't_appregyhp_ischuku',
   			index: 't_appregyhp_ischuku',
   			hidden:true
       	},{
       	   	name: 'cz',
   			index: 'cz',
...
var toUserParamJosn = function() {
	var str = "{'t_appregyhp_title':'"+$('#ins_name').val()+"',"+
		       "'start_time':'"+$('#startSlTime').val()+"',"+
		       "'end_time':'"+$('#endSlTime').val()+"',"+
		       "'t_appregyhp_org_name':'"+$('#org_name').val()+"',"+
		       "'t_appregyhp_user_name':'"+$('#user_name').val()+"','type' : '<%=type%>'}";
	return descape(escape(str));
}
function handle(id){ 
    var model = jQuery("#gridTable").jqGrid('getRowData', id);
	var path ="<%=path%>/ccapp/xtbg/tjy/officesupplies2/jsp/oschukuinfo.jsp?type=<%=type%>&subid=read&action=update&t_appregyhp_id="+model.t_appregyhp_id;
	location.href = path;
}
				...
				<tr>
					<td>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabs_search_ctable">
							<tr>
								<th class="input_cx_title_th">申领标题：</th>
					...
					<td>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabs_search_ctable" id="hiddenArea" style="display:none">
							...
							<tr>
								<th class="input_cx_title_th">申领部室：</th>
								<td  class="cx_title_td">
									<input id="org_name" name="org_name" type="text" class="input_cx_title_283" 　/> 
								</td>
							</tr>
							<tr>
								<th class="input_cx_title_th">申领人：</th>
								<td  class="cx_title_td">
