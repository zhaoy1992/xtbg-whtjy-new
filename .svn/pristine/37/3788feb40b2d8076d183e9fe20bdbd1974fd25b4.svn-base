...
<script language="javascript" type="text/javascript">
 	...
 	*/
 	function changeSelect(action_defid,selectAct){
 		//-------分院院长请假流程  -------
 		//1.省院办公室审核和 2.省院院长审核
 		if(!IsSpace(action_defid)&&action_defid=="fyyzqj_wp1_act2"){
 			selectAct.find("option[value=fyyzqj_wp1_act6]").attr("selected","selected").change();
 			selectAct.find("option[value=fyyzqj_wp1_act1]").remove();
 		} else if(!IsSpace(action_defid)&&action_defid=="fyyzqj_wp1_act6"){//省院分管院长审核
 			selectAct.find("option[value=fyyzqj_wp1_act7]").attr("selected","selected").change();
 			selectAct.find("option[value=fyyzqj_wp1_act2]").remove();
 			
 		} else if(!IsSpace(action_defid)&&action_defid=="fyyzqj_wp1_act7"){//省院院长审核
 			selectAct.find("option[value=fyyzqj_wp1_act4]").attr("selected","selected").change();
 			selectAct.find("option[value=fyyzqj_wp1_act6]").remove();
 			
 		} else if(!IsSpace(action_defid)&&action_defid=="fyyzqj_wp1_act3"){//省院分管领导审核
 			selectAct.find("option[value=fyyzqj_wp1_act8]").attr("selected","selected").change();
 			selectAct.find("option[value=fyyzqj_wp1_act1]").remove();
 		} else if(!IsSpace(action_defid)&&action_defid=="fyyzqj_wp1_act8"){//省院院长审核
 			selectAct.find("option[value=fyyzqj_wp1_act4]").attr("selected","selected").change();
 			selectAct.find("option[value=fyyzqj_wp1_act3]").remove();
 		}
 	}
