...
<%
	//excelTemplateFilePath excel模版路径在配置文件中的key值
	//title 导入的标题
	//remark 导入说明
    ...
    remark =DataControlUtil.replaceStr(remark);
    remark = Escape.unescape(remark);
...
<script>
	...
	function Ok(){
		...
		form1.action="excelimportdo.jsp?&wdo=batch&excelImplClass=<%=excelImplClass%>";
		$("#form1").submit();
	}
	 ...
	 */
	function setChange(){
		$("#impIfr").attr("src","downloadExcelTemplate.jsp?excelTemplateFilePath=<%=excelTemplateFilePath%>");
	}	
</script>
<body style="overflow-y:hidden;" onload="adaptationWH('_top','vcenter',1)">
	<form id="form1" action="" method="post" class="formular" enctype="multipart/form-data">
		...
		<div class="content_02_box_div_table_th" style=" width: 99%;text-align: left; ">
			<font style="color: red;font-size: 14px;"><%=remark %></font>
		</div>
		<div id="vcenter" style="float:left; width:100%; " >
			<div style="float: left; width: 775px; ">
				<div class="content_02_box_div">	
					<table >
						<tr>
							<th class="content_02_box_div_table_th" rowspan="2"><FONT style="COLOR: red">*</FONT>选择${title}</th>
						...
						<tr>
							<td class="content_02_box_div_table_td" width="250px;">
								&nbsp;请按照模板格式填写<%=title %>&nbsp;&nbsp;&nbsp;<a onclick="setChange()" href="#"  style="color: blue">模版下载</a>
							</td>
						</tr>
					</table>
					<table width="456px">
