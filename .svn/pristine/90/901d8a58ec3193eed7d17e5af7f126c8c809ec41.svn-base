...
$(function() {
	...
	//接收表
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/commonListServlet?classNameId=carManageCarYmjList',  //commonListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['car_id','车牌号','品牌','型号','颜色','购买日期','driver_id','驾驶员'],
		...
		filterOptions:[
		               [
		                ["","状态"],["3","审批未通过"],["4","已派车"],["5","派车结束"]
		               ],//value text 后的值与显不
		              ],
		filterOptionsDefValue:[0,1],  */
		...
		toolbar: [true,"top","<input type='button' class='but_y_01' value='新车登记' onMouseOver=javascript:$(this).attr('class','but_y_02'); onMouseOut=javascript:$(this).attr('class','but_y_01'); onclick='addCarinfo()'/><input type='button' class='but_y_01' value='删除' onMouseOver=javascript:$(this).attr('class','but_y_02'); onMouseOut=javascript:$(this).attr('class','but_y_01'); onclick='delCarInfo()'/>"],
		multiselect: true,
...
var toUserParamJosn = function() {
	 var str = "{'use_starttime':'" + $("#use_starttime").val()
 ...
 * 新增用车申请
 */
var addCarinfo = function() {
	var url = "<%=path%>/ccapp/xtbg/yimingju/carmanage/jsp/addCarinfo.jsp";
	location.href = url;
}
...
var updateCarinfo = function(){
		...
		if (IsSpace(ids)) {
			alert('<center>'+"请选择要修改的记录！"+'</center>');
		} else {
			var model = jQuery("#gridTable").jqGrid('getRowData',selectedIds[i]);
			var url = "<%=path%>/ccapp/xtbg/yimingju/carmanage/jsp/addCarinfo.jsp?car_id="+ids;
			location.href = url;
		}
}
...
var viewCarinfo = function(id) {
	...
	var url = "<%=path%>/ccapp/xtbg/yimingju/carmanage/jsp/addCarinfo.jsp?car_id="+model.car_id;
	
	openWindows("sub_ymjclda"+id,window,url+"&subid=sub_ymjclda"+id,'ymjclda',false,window);
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabs_search_ctable_box" id="queryTable">
		<tr>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabs_search_ctable">
				<tr>
					<th class="input_cx_title_th">购车日期：</th>
