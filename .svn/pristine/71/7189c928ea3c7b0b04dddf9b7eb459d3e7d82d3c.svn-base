...
//tab2
	$("#gridTable").jqGrid({
		...
		toolbar:[true, "top" ,"<input class='but_y_01' id=add value='添加' type='button' onclick='addLevel()' value=''/>"+
...
var addLevel = function() {
	...
	openAlertWindows('windowId',"../../../tjy/officesupplies/type/jsp/osupplietypeinfo.jsp?windowsId=windowId&actionName=insert&oftype_flag=<%=oftype_flag%>",pageName,500,205,'25%','25%');
}
...
var updateLevel = function(id) {
	...
	openAlertWindows('windowId','../../../tjy/officesupplies/type/jsp/osupplietypeinfo.jsp?windowsId=windowId&actionName=update&oftype_flag=<%=oftype_flag%>&oftype_id='+ model.oftype_id,pageName,500,205,'25%','25%');
}
...
var importExcel = function(){
	var title = descape(escape('办公用品易耗品类别维护信息')); //转码,避免中文传输乱码
	var remark = descape(escape('注意:导入的文件设备编号是唯一的。')); //转码,避免中文传输乱码
	var url = '${pageContext.request.contextPath}/ccapp/oa/resources/util/excel/excelimport.jsp?excelImplClass=officeTypeInfoDaoExcelImpl&excelTemplateFilePath=bgypyhplb_excelTemplateFilePath&title='+title+'&remark='+remark;
