...
//tab2
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/commonListServlet?classNameId=detailAlreadList',  //commonListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['采购申请ID', '申请标题 ', '申请人','申请处室','申请时间','采购时间'],
...
var toUserParamJosn = function() {
			...
			"'apply_room':'"+$('#apply_room').val()+"','state':'6'}";
...
var updateLevel = function(id) {
	...
	openWindows("windowId","采购已审批","../../officeassets/jsp/SeeApprovalList.jsp?widowsid=windowId&whatDo=ready&apply_id=" + model.apply_id,'PurchaseApply',false,window);
}
