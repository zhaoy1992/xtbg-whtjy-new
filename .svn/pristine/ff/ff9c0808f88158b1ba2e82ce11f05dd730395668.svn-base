...
var addFileDocDetail = function () {
    ...
    openAlertWindows('addFileDocDetailWindow', getCurrentFilePath() 
    	    + 'filedocdetail4video.jsp?isgd=yes&busiTypeCode=wj-001&windowId=addFileDocDetailWindow'+typeurl, '添加文件' ,1020,530,'10%','10%');
}
...
var updateFileDocDetail = function(f_doc_id) {
	  ...
	  openAlertWindows('updateFileDocDetailWindow',
   	       getCurrentFilePath() + 'filedocdetail4video.jsp?isgd=yes&busi_id='
...
function exportExcelList(){
    var exportKey = [ "f_doc_num","f_doc_name","f_file_formtime4export","f_shooting_user"
                      ,"f_secrecy_name4export"];
   
    var exportHeader = ["件号","    视 频 名 称       "," 摄影日期 "," 摄影者 "," 备   注 "];
    var data = {
        exportKey : exportKey.join(","),
        exportHeader : descape(escape(exportHeader.join(","))),
        exportTitle : descape(escape("视频档案卷内目录"))
    };
...
function exportExcelDetail(){
    var exportKey = [ "f_fondsno","f_listno","f_typeno",
          "f_filesno","f_doc_num","f_doc_name","f_shooting_user","f_file_formtime4export"];   
    var exportHeader = ["全宗号","目录号","分类号","卷  号","盘  号","视     频     名    称","摄影人","摄  影 日  期"];
    var data = {
        exportKey : exportKey.join(","),
        exportHeader : descape(escape(exportHeader.join(",")))
    };
...
<form action="" id="form1" name="form1" method="post" >
     <div class="content_02_top" style="margin-bottom: 10px;float: none;border: #ebebeb 1px solid;padding-right: 10px;">
           ...
           <input onclick="exportExcelDetail();" name="exportButton" id="exportButton"  value="导出视频标签" type="button" class="but_y_03" style="float: right;width:105px;"/>
