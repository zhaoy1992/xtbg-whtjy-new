...
/**
 * 
 *<p>Title:MchSmsServiceImpl.java</p>
 *<p>Description:</p>
 ...
 *2013-5-9
 */
public class MchSmsServiceImpl implements SmsServiceIfc{
	private static final Logger LOG = Logger.getLogger(MchSmsServiceImpl.class);
/**
 * 
  * <b>Summary: </b>
 ...
 */
	...
	public void smsDataAssembling(Map<String, String> map,Connection con) {
			...
			try {
				...
				if (list.size() > 0 && null != list) {
					for (int i = 0; i < list.size(); i++) {
						...
						smsBean.setMSG(buf.append(map.get("content")).toString());
		...
		}else if("zhbg".equalsIgnoreCase(map.get("isSend"))){ //1333333333
			try {
				String phonenuo =map.get("phonenuo");
				String[] phonenuostr =phonenuo.split(";");
				if (phonenuostr.length > 0 && null != phonenuostr) {
					for (int i = 0; i < phonenuostr.length; i++) {
						StringBuffer buf = new StringBuffer();
						SmsBean smsBean = new SmsBean();
						smsBean.setRECEIVERMOBILENO(phonenuostr[i]);
						smsBean.setMSG(buf.append(map.get("content")).toString());
						smsUtil.smsSendDao(smsBean);
					}
				}
