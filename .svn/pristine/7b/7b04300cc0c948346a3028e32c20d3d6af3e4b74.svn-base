...
<%
    ...
    String repunit=usercachebean.getUnit_id();
...
var userid = '<%=userid%>';
var opentype ="accepted";
var p_flag_id = "<%=PaperConstants.STATUS_ACCEPT%>";
var accunit = "<%=accunit%>";
var repunit = "<%=repunit%>";
...
$(function() {
	...
	if(unittype=="isrep"){
		$("#p_rep_unit_id").val(repunit);
		$("#p_rep_unit_name").val(repunitname);
	}
       ...
       //表头设置       
        ...
        onCellSelect:function (id,icol)
		{
			if(icol != 0)
			{
        		...
        		urlTmp = "&loadurl=paperlist4manageaccepted&actionName=updatefromacced&p_manage_id="+model.p_manage_id;
...
<body>
	<div>
				...
				<td>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0"
						...
						<tr>
							...
							<td width="200" class="cx_title_td" ><%=p_rep_title_selectHtml%>
							</td>
		...
		<table width="100%" style="margin-left: 0" class="tabs_search_ctable_box">
			...
			<%
			if(!"isrep".equalsIgnoreCase(unittype)){
			%>
			<tr>
				<td width="100%" align="left" valign="top">
					<input name="AddOne4Acc" value="信息新增" type="button"  onclick="btnClickFn(this)" class="but_y_01" /><span style="width:5px;display:block;float:left;">&nbsp;</span>
				</td>
			</tr>
			<% 
			}
