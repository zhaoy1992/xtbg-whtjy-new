...
<%
	...
	try {
		...
		PagingBean pb = null;
		pb= dao.findOsHavaLYObjectList(sJson,"t_ly_date","desc",0,1000000);
		...
		if (list.size() > 0) {
			...
			String filename = new String("办公用品易耗品发放".getBytes("GBK"),"iso8859-1");
			...
			WritableSheet ws = wwk.createSheet("办公用品易耗品发放信息", 0);
			//设置列宽
			ws.setColumnView(0, 10);//序号
			ws.setColumnView(1, 15);//物品名称
			ws.setColumnView(2, 20);//物品类型名称
			ws.setColumnView(3, 20);//领用数量
			ws.setColumnView(4, 20);//规格名称
			ws.setColumnView(5, 20);//领用人
			ws.setColumnView(6, 20);//领用处室
			ws.setColumnView(7, 20);//领用时间
			...
			Label lab2 = new Label(4, 1, "办公用品易耗品发放", wcf);
			ws.addCell(lab1);
			ws.addCell(lab2);
			//设置标题区域的表格
			...
			Label lab6 = new Label(3, 2, "领用数量",wc_title);
			Label lab7 = new Label(4, 2, "规格名称",wc_title);
			Label lab8 = new Label(5, 2, "领用人",wc_title);
			Label lab9 = new Label(6, 2, "领用处室",wc_title);
			Label lab10 = new Label(7, 2, "领用时间",wc_title);
			...
			for (int i = 3; i < list.size() + 3; i++) {
				...
				Label lab24  = new Label(3 , i, list.get(i - 3).get("t_ly_no"),wc_cols);
				Label lab25  = new Label(4 , i, list.get(i - 3).get("it_name"),wc_cols);
				Label lab26  = new Label(5 , i, list.get(i - 3).get("t_ly_user_name"),wc_cols);
				Label lab27  = new Label(6 , i, list.get(i - 3).get("t_ly_org_name"),wc_cols);
				Label lab28  = new Label(7 , i, list.get(i - 3).get("t_ly_date"),wc_cols);
