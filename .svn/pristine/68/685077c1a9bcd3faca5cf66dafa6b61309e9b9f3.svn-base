...
<script type="text/javascript">
	...
	function updateRoleType(){
		...
		if(isFlag){
			...
			form.action = "updateroletypedo.jsp";
	...
	function checkRoleTypeName(){
		...
		if(type_name != ""){//角色名称验证
			$.ajax({
				...
				data: "type_name="+type_name+"&type_id="+type_id+"&action=update",
...
<body style="overflow-y:hidden;">
	<form id="form1" name="form1" action="" method="post">
		...
		<div id="users-contain">
			<table  width="100%" border="0" cellspacing="0" cellpadding="0" >
				<tr>
					...
					<td class="content_02_box_div_table_td">
						<input type="text" name="type_name" id="type_name" onblur="checkRoleTypeName()"
							class="validate[required,maxSize[50]] input_152" style="width:70%;height=23px"  value="<%=type_name %>" maxlength="50">
				...
				<tr>
					...
					<td class="content_02_box_div_table_td">
						<textarea name="type_desc" id="type_desc" class="validate[required,maxSize[250]] textarea_556" style="width:70%;" ><%=type_desc %></textarea>
					</td>
				</tr>
			</table>
		</div>
		...
		<div class="cue_box_foot">
			...
			<input class="but_y_01" type="button" id="sel"  value="保存" onclick="updateRoleType()"/>
		</div>
	</form>
