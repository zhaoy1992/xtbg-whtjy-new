...
$(function() {
	...
	$("#gridTable").jqGrid({
		...
		toolbar: [true,"top",
		          ...
		          "<input class='but_y_06' id=add value='办公用品申购表' type='button' onclick='newosxaFlow()'/>"+
...
function addLevel(id){   
	var path ="<%=path%>/ccapp/xtbg/tjy/officesupplies/jsp/osmoreobjectinfo.jsp?windowsId=windowIds1";
	openAlertWindows('windowIds1',path,'易耗品入库',1024,768,'25%','25%');
}


function newosxaFlow(){
	var url="<%=path%>/ccapp/xtbg/tjy/officesupplies/jsp/osexamination.jsp?busiTypeCode=uuid_521";
	var flowname="办公用品申购审批表";
...
var exprotExcel = function(){
	var url = getCurrentFilePath()+"/ajaxexceOfficeInfo.jsp?" + "windowId=windowId_paperPS&sJson="+toUserParamJosn();  
