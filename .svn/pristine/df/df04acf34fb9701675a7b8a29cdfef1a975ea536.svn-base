...
<%
	...
	String GET_LIST_CLASS1="com.chinacreator.xtbg.tjy.device.list.DevCalibrateRecodeList";
	String path = request.getContextPath();
%>
<title>校准记录列表界面</title>
<!-- 公共部分开始 -->
<!-- 引入jQuery -->
...
//tab2
	$("#gridTable").jqGrid({
		...
		colNames: ['主键ID', '设备编号 ','设备名称', '校准证书编号','校准单位','校准日期','有效期','参数范围','校准类型'],
		...
		{
			name: 'calib_valitime',
			index: 'calib_valitime',
			width: 60
		},
		{
			name: 'calib_parrange',
			index: 'calib_parrange',
			width: 70
		},
		{
			name: 'calib_type',
...
var toUserParamJosn = function() {
	var str = "{'dev_code':'" + StringCheck($("#dev_code").val()) +
	"','dev_name':'" + $("#dev_name").val() + 
	"','calib_type':'" + $("#calib_type").val() + 
	"'}";
...
var addLevel = function() {
 	var pageName ="新增校准记录";
	openAlertWindows('windowId',"../../../tjy/device/type/jsp/devcalibraterecodeinfo.jsp?windowsId=windowId&actionName=insert",pageName,530,400,'25%','25%');
}
...
var updateLevel = function(id) {
	...
	var pageName ="修改校准记录";
	openAlertWindows('windowId','../../../tjy/device/type/jsp/devcalibraterecodeinfo.jsp?windowsId=windowId&actionName=update&calib_id='+ model.calib_id,pageName,530,400,'25%','25%');
}
...
var delLevel = function(id) {
	...
	$(selectedIds).each(function () {
		...
		if(!IsSpace(model.calib_id)){
			idsArrTemp.push(model.calib_id);
		}
	});
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabs_search_ctable">
							   <tr>
									...
									<td width="296"  class="cx_title_td">
											<input type="text" id="dev_code" class="input_cx_title_283" >
									</td>
					...
					<td>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabs_search_ctable" id="hiddenArea" style="display:none">
							<tr>
								...
								<td>
									<input type="text" id="dev_name" class="input_cx_title_283" >
								</td>
							</tr>
							<tr>
								<th width="90" class="input_cx_title_th">校准类型：</th>
								<td>
									<select id="calib_type" name="calib_type" style="width: 160px;">
...
<form id="delForm2" name="delForm2" method="post" action="devcalibraterecodeinfodo.jsp">
