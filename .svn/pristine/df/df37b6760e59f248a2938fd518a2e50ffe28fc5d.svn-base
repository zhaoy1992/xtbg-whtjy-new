     ...
     /*
   	...
   	*/
   	function changeSelect(action_defid,selectAct){
	 	//-------分院科室主要人员  流程-------
	 	//1.分院综合办公室审核
	 	if(!IsSpace(action_defid)&&action_defid=="kszyfzrqj_wp1_act2"){
	 		selectAct.find("option[value=kszyfzrqj_wp1_act4]").attr("selected","selected").change();
	 		selectAct.find("option[value=kszyfzrqj_wp1_act1]").remove();
	 	} else if(!IsSpace(action_defid)&&action_defid=="kszyfzrqj_wp1_act4"){//分院分管院长审核
	 		selectAct.find("option[value=kszyfzrqj_wp1_act8]").attr("selected","selected").change();
	 		selectAct.find("option[value=kszyfzrqj_wp1_act2]").remove();
	 	} else if(!IsSpace(action_defid)&&action_defid=="kszyfzrqj_wp1_act8"){//分院院长审核
	 		selectAct.find("option[value=kszyfzrqj_wp1_act6]").attr("selected","selected").change();
	 		selectAct.find("option[value=kszyfzrqj_wp1_act4]").remove();
	 	} else if(!IsSpace(action_defid)&&action_defid=="kszyfzrqj_wp1_act3"){//分院分管院长审核
	 		selectAct.find("option[value=kszyfzrqj_wp1_act5]").attr("selected","selected").change();
	 		selectAct.find("option[value=kszyfzrqj_wp1_act1]").remove();
	 	} else if(!IsSpace(action_defid)&&action_defid=="kszyfzrqj_wp1_act5"){//分院院长审核
	 		selectAct.find("option[value=kszyfzrqj_wp1_act6]").attr("selected","selected").change();
	 		selectAct.find("option[value=kszyfzrqj_wp1_act3]").remove();
	 	} 
    }
