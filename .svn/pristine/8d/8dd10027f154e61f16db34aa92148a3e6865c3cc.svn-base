...
$(function() {
	...
	var wdNum=0;
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/pubListServlet?classNameId=com.chinacreator.xtbg.core.email.list.YjSendEmailList',  //commonListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		toolbar: [true,"top","<input class='but_y_01' onMouseOver=javascript:$(this).attr('class','but_y_02'); onMouseOut=javascript:$(this).attr('class','but_y_01'); value='追回' id=del type='button' onclick='replevyMail()' value=''/><input class='but_y_01' onMouseOver=javascript:$(this).attr('class','but_y_02'); onMouseOut=javascript:$(this).attr('class','but_y_01'); value='删除' id=del type='button' onclick='delUser()' value=''/>"],
		<%}else{%>
		toolbar: [true,"top"],
		<%}%>
		multiselect: true,
...
var toUserParamJosn = function() {
	var str = "{'sendpid':'" + '<%=userid%>'+ "','state':'"+ '1'
...
var viewMailinfo = function(id) {
	...
	var url = "<%=path%>/ccapp/oa/email/jsp/mailinfo.jsp?mid="+model.mid+"&back=yj";
...
<!-- 查询条件选区    tabs_search_ctable(查询table样式) -->
	...
	<input type="hidden" name="deltype" id="deltype" value="yjdelete"/>
