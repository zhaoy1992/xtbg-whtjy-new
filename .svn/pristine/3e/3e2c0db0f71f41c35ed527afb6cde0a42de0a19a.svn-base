...
//tab2
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/commonListServlet?classNameId=officeEquipmentTypeList',  //commonListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
...
var toUserParamJosn = function() {
	var str = "{'oetype_name':'" + StringCheck($("#oetype_name").val()) + "'}";
...
var addLevel = function() {
	openAlertWindows('windowId','../../../yimingju/officeassets/jsp/addorupdateequipmenttype.jsp?windowsId=windowId','添加设备类型',500,175,'25%','25%');
}
...
var updateLevel = function(id) {
	...
	openAlertWindows('windowId','../../../yimingju/officeassets/jsp/addorupdateequipmenttype.jsp?windowsId=windowId&oetype_id='+ model.oetype_id,'修改设备类型',500,175,'25%','25%');
}
...
var delLevel = function(id) {
	...
	$(selectedIds).each(function () {
		...
		if(!IsSpace(model.oetype_id)){
			idsArrTemp.push(model.oetype_id);
		}
	});
					...
					<td>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabs_search_ctable">
							<tr>
								...
								<input type="text" id="oetype_name" class="input_cx_title_283" >
								</td>
...
<form id="delForm2" name="delForm2" method="post" action="addorupdateequipmenttypedo.jsp">
