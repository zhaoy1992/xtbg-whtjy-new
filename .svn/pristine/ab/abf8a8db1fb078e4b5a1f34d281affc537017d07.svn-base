...
<%
	...
	String GET_LIST_CLASS1="com.chinacreator.xtbg.tjy.device.list.DevAppraisalList";
	String path = request.getContextPath();
%>
<title>核定记录列表界面</title>
<!-- 公共部分开始 -->
<!-- 引入jQuery -->
...
//tab2
	$("#gridTable").jqGrid({
		...
		colNames: ['主键ID', '设备编号 ','设备名称', '检定证书编号','检定单位','检定日期','下次检定日期','参数范围'],
...
var addLevel = function() {
 	var pageName ="新增检定记录";
	openAlertWindows('windowId',"../../../tjy/account/jsp/devappraisalinfo.jsp?windowsId=windowId&actionName=insert",pageName,530,450,'25%','25%');
}
...
var updateLevel = function(id) {
	...
	var pageName ="修改检定记录";
	openAlertWindows('windowId','../../../tjy/account/jsp/devappraisalinfo.jsp?windowsId=windowId&actionName=update&appr_id='+ model.appr_id,pageName,530,450,'25%','25%');
}
...
var delLevel = function(id) {
	...
	$(selectedIds).each(function () {
		...
		if(!IsSpace(model.appr_id)){
			idsArrTemp.push(model.appr_id);
		}
	});
...
<form id="delForm2" name="delForm2" method="post" action="devappraisalinfodo.jsp">
