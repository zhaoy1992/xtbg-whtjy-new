...
<SCRIPT language="javascript">
	...
	function advance_EditDict(){
		...
		for(var i = 0; i < obj.length; i++){
			var objText = document.frames[0].document.getElementById("a"+i);
			if(obj[i].checked){
		...
		<form name="dictList" method="post" target="hiddenFrame">
			<table cellspacing="1" cellpadding="0" border="0" width=100%  class="thin">
				...
				<input type="hidden" name="docid">
 				<tr valign='top'>
				<td height='30' valign='middle' colspan="2"><img src='../images/edit.gif' width="16" height="16" border=0 align='absmiddle'>&nbsp;<strong>【<%=dicttype_name%>-<%=description%>】数据列表</strong></td>
 				</tr>
 				<tr>
			    ...
			    <input name="queryBut" type="button" value="查询" class="input" onClick="queryDictItem('-1');" />
			    <input name="resetBut" type="button" value="重置" class="input" onclick="resetQuery();" />
 				...
 				<tr>
					...
					<%
						if(showDataValidate){
					%>
 				        <input name="changeState1" <%if(!state){%>disabled="true"<%}%> type="button" value="启用" class="input" onclick="changeState_(1)">
 				        <input name="changeState0" <%if(!state){%>disabled="true"<%}%> type="button" value="停用" class="input" onclick="changeState_(0)">						
 				    <%
 				         }
 				    %>
					</td>
		  ...
		  <tr>
			 ...
			 %>
				 <input name="deletebt" type="button" value="删除" <%if(!stateDel){%>disabled="true"<%}%> class="input" onclick="javascript:dealRecord(1); return false;">
			 <%
			     }
			 }else{
			 %>	 
