...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%=path%>' + '/pubListServlet?classNameId=com.chinacreator.xtbg.core.paper.list.PaperMakeNoticeList',  
		...
		gridComplete: function() {
	        ...
	        for (var i = 0; i < ids.length; i++) {
	            ...
	            be2 = "<input type='button' class='mybtn' id='b2' onclick=openRead('"+model.p_notice_id+"')     value='"+model.isread+"' >";
		...
		toolbar: [true,"top","<input class='but_y_01' onMouseOver=javascript:$(this).attr('class','but_y_02'); onMouseOut=javascript:$(this).attr('class','but_y_01'); id=add type='button' onclick='addMakeNotice()' value='新增'/>"],
		multiselect: false,
...
var toJson = function() {
	var str ="{'p_title':'"+$("#p_title").val()
		...
		+"','p_send_unit_id':'<%=unit_id%>','end_time':'"+$("#end_time").val()+"'}";
...
var updateMakeNotice = function(id) {
	...
	var url = getCurrentFilePath()+"papermakenoticeview.jsp?&subid=windowId&p_notice_id="+model.p_notice_id;
			...
			<tr>
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0"
					...
					<tr>
						...
						<td width="150" class="cx_title_td"><input class="input_cx_title_150" type="text"  id="p_title" name="p_title" value="" />
						</td>
