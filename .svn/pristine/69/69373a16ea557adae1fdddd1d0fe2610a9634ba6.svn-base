...
jQuery(function() {
    ...
    $("#gridTable").jqGrid({
        url: '<%= path %>' + '/pubListServlet?classNameId=com.chinacreator.xtbg.core.vote.list.VotePaperOperateList',  //pubListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
        ...
        colNames: ['问卷id', '考核标题','所属单位','创建人','截止时间','考核类型','问卷类型CODE','是否完成填写','是否填写'],
...
var modifyVote = function(id) {
   ...
   if(papaer_type_code == '<%=Constant.RYCP%>'){
	   openWindows('VoteEvalPaperList','人员测评','voteevaloperate.jsp?windowId=VoteEvalPaperList&status='+model.complete+'&paperId='+model.id,'cz_dwll',false,window);
   }else{
	   openWindows('VotePaperList','在线评分','voteoperate.jsp?windowId=VotePaperList&status='+model.complete+'&paperId='+model.id,'windowId',false,window);
   }
}
</script>
</head>
    ...
    <!-- 查询条件选区    tabs_search_ctable(查询table样式) -->
            <table id="queryTable" width="100%" border="0" cellspacing="0" cellpadding="0" class="tabs_search_ctable_box">
                <tr>
                    <td>
                        <!-- 固定查询 -->
                        <table width="100%" style="padding:5px;" cellspacing="0" cellpadding="2"
            class="tabs_search_ctable">
            <tr>
                <th width="15%" class="input_cx_title_th">标题：</th>
                <td width="20%" class="cx_title_td"><input type="text"
                    id="title" name="title" class="input_cx_title_150" />
                </td>
                <th width="15%" class="input_cx_title_th">考核类型：</th>
                <td width="30%" class="cx_title_td" style="width:400px">
                  <select id="papaer_type" name="papaer_type"
                ...
                <td width="10%" rowspan="2"><input value="搜索" id="sel"
                    type="button" onclick="Search()" class="but_y_01" />
            ...
            <tr>
                ...
                <input type="text"
                    id="user_name" name="user_name" class="input_cx_title_150" />
                
                </td>
                <th  class="input_cx_title_th">创建时间：</th>
                <td  class="cx_title_td">
                 <input class="date_120" type="text" id="startdate" name="startdate" 
                                onfocus="WdatePicker({maxDate:'#F{$dp.$D(\'enddate\'||\'1980-01-01\')}',dateFmt:'yyyy-MM-dd'})"  />
                <span style="float:left"> &nbsp;至&nbsp;</span>
                 <input class="date_120" type="text" id="enddate" name="enddate" 
                                onfocus="WdatePicker({minDate:'#F{$dp.$D(\'startdate\'||\'2099-01-01\')}',dateFmt:'yyyy-MM-dd'})"  />
