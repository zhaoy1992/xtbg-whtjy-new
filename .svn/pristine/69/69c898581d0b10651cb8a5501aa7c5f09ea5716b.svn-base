...
<%
	...
	String roleid = (String)session.getAttribute("currRoleId");
	String role_type = (String)session.getAttribute("role_type");//organization
	String username = request.getParameter("username");//System.out.println(role_type + "|" + roleid);
	if(resname == null)
		resname = (String)request.getAttribute("resname");
	session.setAttribute("resname",resname);
	//UserManager userManager = SecurityDatabase.getUserManager();
	boolean flag = false;//AccessControl.isAdmin(userManager.getUserById(roleid).getUserName());
...
<SCRIPT LANGUAGE="JavaScript"> 
	...
	function okRecord(dealType,id) {
			...
			Form1.action="<%=rootpath%>/accessmanager/securityManager.do?method=siteAppAuthorization&isRecursion="+tt+"&resTypeId="+id;
			Form1.submit();
...
<div id="contentborder">
			...
			<form target="saveform1" name="Form1" action="" method="post" >
			...
			<IFRAME name="saveform1" width="0" height="0">
			</IFRAME>
			</div> 
