...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/commonListServlet?classNameId=infoReportPublishedList',  //commonListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['报送id', '标题' , '存档时间', '存档人', '上报单位','信息状态','隐藏信息编号'],
...
var editPerson = function(id) {
	//'&operwindowid=sub_poeaddoreditpage'+model.submittedinfo_id
	var model = jQuery("#gridTable").jqGrid('getRowData', id);
	var url = "../../inforeported/jsp/infoviewforpublished.jsp?submittedinfoId="+model.submittedinfo_id+"&state="+model.submittedinfo_typecode+"&comelist=saveinfo"+'&operwindowid=sub_infoviewforpublished'+model.submittedinfo_id;
	//location.href = url;
	openWindows('sub_infoviewforpublished'+model.submittedinfo_id,'信息采办存档',url,'infocaibian',false,window);
}

var CancelFb = function(){
	var ids = "";
	var tempArrs = [];
	...
	if (IsSpace(ids)) {
		...
		$("#state").val('<%=InfoType.HAS_BE_AUDIT%>');
		document.all.form1.action = url;
		document.all.form1.target = "hiddenFrame";
...
<table width="100%" class="cQueryTableArea" id="queryTable">
	...
	<tr>
		...
		<td class="tableValue"><input class="input_date Wdate" id="input_starttime" name="input_starttime" type="text" onclick="WdatePicker()"/></td>
		<td class="tableLable">录入截止时间</td>
		<td class="tableValue" ><input class="input_date Wdate" id="input_endtime" name="input_endtime" type="text" onclick="WdatePicker()"/></td>
		  <td></td>
	</tr>
		-->
