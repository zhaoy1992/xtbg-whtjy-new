...
function findCanAvermentCase(){
    ...
    var sql = DocumentList.findDocumentList(pagePrams);
...
function initAverment(grid, rowIndex, e){
    ...
    if(avermentState == "已申请" && IsSpace(tp_status)){//跳到陈述申请界面
        var flag = checkTime(wft_id);//检测申请是否超时
        if(!flag){  //超时
              saveEndInfo(wft_id);//写入强制终止信息
              var tempPara = "dialogWidth:780px,dialogHeight:250px;";
            var tempURL = "20100909163528703907.jsp?djbh=222&wft_id=" + wft_id + "&flag=1&temp=0&avermentState=" + avermentState;
            window.showModalDialog(tempURL, "window", tempPara);
            reloadExtGrid();
        } else {   //未超时
              var tempPara = "dialogWidth:780px,dialogHeight:250px;";
            var tempURL = "20100909163528703907.jsp?djbh=222&wft_id=" + wft_id + "&flag=0&temp=0&avermentState=" + avermentState;
            window.showModalDialog(tempURL, "window", tempPara);
            reloadExtGrid();
        }    
    } else if(avermentState == "未申请" || avermentState == "已申辩" || (avermentState == "已申请" && tp_status == 0)){ //跳转到陈述申辩界面
        var tempPara = "dialogWidth:780px,dialogHeight:250px;";
        var tempURL = "20100909161026046925.jsp?djbh=222&wft_id=" + wft_id + "&avermentState=" + avermentState + "&tp_status=" + tp_status + "&eflag=0";
      ...
      <TR>
         <TD>
            <TABLE class=Ctable cellSpacing=0 cellPadding=0 width="100%" border=0>
               <TBODY>
                  <TR>
                     <TD class=taobox colSpan=4>陈述申辩查询</TD>
                  </TR>
                  <TR>
                     <TD class=c2 width="15%">执法主体</TD>
                     <TD width="35%"><INPUT class=input_out id=orgNames style="LEFT: 0px; WIDTH: 100%; POSITION: static; TOP: 0px; HEIGHT: 20px" onmovestart=moveStart() readOnly controltype="text" onclick='bill_onclick("openUserTree()")'><INPUT id=orgIds type=hidden></TD>
