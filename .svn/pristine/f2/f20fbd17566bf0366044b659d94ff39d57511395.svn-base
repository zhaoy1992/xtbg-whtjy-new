...
$(function() {    
   ...
   function exportExcelList(){
       var exportKey = [ "f_filesno", "f_type_name_export","f_listno","f_files_num",
                        "f_doc_deadline_name","备注"];//备注要为空，用一个不存在的key代替
       var exportHeader = [" 案卷号 ", "    照片类别    ", "  目录号  ", " 张数  "," 期限 ","备注"];
       var data = {
           exportKey : exportKey.join(","),
           exportHeader : descape(escape(exportHeader.join(","))),
           exportTitle : descape(escape("照片档案案卷目录"))
       };
    ...
    */
    var fitFiles = function() {
        ...
        urlArray.push("&f_secrecy_name="+jQuery("#f_secrecy_name").val());
        urlArray.push("&f_type_id="+window.parent.f_type_id);
        urlArray.push("&columnName="+descape(escape("组号")));
    ...
    function printFileBack(){
        ...
        $(selectedIds).each(function () {
            ...
            if(i%5 == 0 && i!=(selectedIds.length)){
                htmlArray.push("<div style='float:left;height:75px;width:100%;'></div>");
            }
            i++;         
        });
        htmlArray.push('</div>    ');
    ...
    function backHtml(model){
       ...
       ' color:#3F9CC9;font-weight: bold;float:left;margin-left:30px;" cellspacing="0" cellpadding="0" >',
       ...
       '<tr><td style="height:45px;line-height:20px;border-bottom:2px solid #0099CC;text-align:center;color:#000;">',
       model.f_listno,
       '</td></tr>',
...
<div>
           ...
           <table style="width:830px;display:none;margin: 0px;" border="0" cellspacing="3" cellpadding="0" id="hiddenArea" >
                      <tr>
                          ...
                          <td class="cx_title_td" style="width:100px;">
                           ...
                           <th class="input_cx_title_th" style="width:90px;">卷内张数：</th>
    ...
    <div id="tool_buttons" class="ui-jqgrid .ui-userdata ui_t_bottombar_content" 
         style="width:97%;height:28px;margin-left:10px;margin-bottom:0px;float: none;border: #ebebeb 1px solid;">            
           ...
           <input name="exportButton" id="exportButton" onclick="exportExcelList();" value="导出案卷目录" type="button" class="but_y_03" style="float: left;margin-left: 5px;" />
           <input name="printFileBack" id="printFileBack" onclick="printFileBack();" value="打印背脊标签" type="button" class="but_y_03" style="float: left;margin-left: 5px;" />
