...
<%
	...
	String username = user.getUserRealname();//request.getParameter("username");//System.out.println(role_type + "|" + roleid);
	if(resname == null)
		resname = (String)request.getAttribute("resname");
	session.setAttribute("resname",resname);
	
	boolean flag = AccessControl.isAdmin(user.getUserName());
