...
{   //对页面表单进行验证的函数
		...
		//5.非空的实数型
		PatternsDict.isfloat = /(^(0|\-?[1-9]+[0-9]*|\-?[1-9]+[0-9]*\.\d+|\-?[0-9]*\.\d+)$)|(^$)/;
		//6.可以为空的实数型
	  ...
	  PatternsDict.postalCode=/^[1-9]{1}(\d+){5}$/;
	 
	  //18、可以为空的6位邮政编码
	  PatternsDict.postalCodeNull=/(^[1-9]{1}(\d+){5}$)|(^$)/;		
	...
	　　　　　　　　　　　　　　if (v=="email") vname="非空的Eamil";
						...
						if (v=="string") vname="非空";
						 ...
						 if(v == "datetimeNull") vname="日期(例如：2005-01-02 10:20:40),可以留空不填";
						  if(v=="postalCode") vname="非空6位的邮政编码";
			    if(v=="postalCodeNull") vname="6位的邮政编码";
	　　　　　　　　　　}
			    	else 
			    	{
			    	    if(maxlength){
...
　　　　　　{   //对页面表单进行验证的函数
		...
		//定义正则表达式
		var PatternsDict_1=new Object();
		//1.非空的日期型数据
		PatternsDict_1.date = /^[1-2]\d{3}-(0?[1-9]|1[0-2])-(0?[1-9]|[12][0-9]|3[0-1])$/;
		//2.可以空的日期型数据
		PatternsDict_1.dateNull = /(^[1-2]\d{3}-(0?[1-9]|1[0-2]|3[0-1])-(0?[1-9]|[12][0-9]|3[0-1])$)|(^[1-2]\d{3}-([1-9]|1[0-2]|3[0-1])-([1-9]|[12][0-9]|3[0-1])$)|(^$)/;
		//3.非空的数字类型
		PatternsDict_1.num= /^\d+$/;
		//4.可以为空的数字类型
		PatternsDict_1.numNull = /^\d*$/;
		//5.非空的实数型
		PatternsDict_1.isfloat= /^(0|\-?[1-9]+[0-9]*|\-?[1-9]+[0-9]*\.\d+|\-?[0-9]*\.\d+)$/;
		//6.可以为空的实数型
		PatternsDict_1.floatNull = /(^(0|\-?[1-9]+[0-9]*|\-?[1-9]+[0-9]*\.\d+|\-?[0-9]*\.\d+)$)|(^$)/;
		//7.非空的最大10位整数，2位小数的实数型
		PatternsDict_1.float10_2= /^(0|\-?[1-9]{1}\d{0,9}|\-?[1-9]{1}\d{0,9}\.\d{1,2}|\-?\d{0,9}\.\d{1,2})$/;
		//8.可以为空的最大10位整数，2位小数的实数型
		PatternsDict_1.float10_2Null = /(^(0|\-?[1-9]{1}\d{0,9}|\-?[1-9]{1}\d{0,9}\.\d{1,2}|\-?\d{0,9}\.\d{1,2})$)|(^$)/;
		//9.非空的整型
		PatternsDict_1.isint= /^(0|\-?[1-9]{1}\d*)$/;
		//10.可以为空的整型
		PatternsDict_1.intNull = /(^(0|\-?[1-9]{1}\d*)$)|(^$)/;
		//11、非空字符串
		PatternsDict_1.string = /^[\S\s]+$/;
		//12、可以为空字符串
		PatternsDict_1.stringNull = /^[\S\s]*$/;
		//13、非空合法的字符串：除去（$*&'=<#>%^@~ ）
		PatternsDict_1.stringLegal = /^[^\$\*&'=<#>%\^@~\s]+$/;
		//14、可以为空的合法字符串：除去（$*&'=<#>%^@~ ）
		PatternsDict_1.stringLegalNull = /(^[^\$\*&'=<#>%\^@~\s]+$)|(^$)/;
		//15、不可以为空的Email
		PatternsDict_1.email = /^[\-!#\$%&'\*\+\\\.\/0-9=\?A-Z\^_`a-z{|}~]+@[\-!#\$%&'\*\+\\\.\/0-9=\?A-Z\^_`a-z{|}~]+(\.[\-!#\$%&'\*\+\\\.\/0-9=\?A-Z\^_`a-z{|}~]+)+$/;
		//16、可以为空的Email
		PatternsDict_1.emailNull = /(^[\-!#\$%&'\*\+\\\.\/0-9=\?A-Z\^_`a-z{|}~]+@[\-!#\$%&'\*\+\\\.\/0-9=\?A-Z\^_`a-z{|}~]+(\.[\-!#\$%&'\*\+\\\.\/0-9=\?A-Z\^_`a-z{|}~]+)+$)|(^$)/;
		
		//17、非空带时分秒的日期类型
		PatternsDict_1.datetime = /^[1-2]\d{3}-(0?[1-9]|1[0-2])-(0?[1-9]|[12][0-9]|3[0-1]) (0?[0-9]|1[0-9]|2[0-3]|):(0?[0-9]|[1-5][0-9]):(0?[0-9]|[1-5][0-9])$/;
		
		//18、可空带时分秒的日期类型
		PatternsDict_1.datetimeNull = /(^[1-2]\d{3}-(0?[1-9]|1[0-2])-(0?[1-9]|[12][0-9]|3[0-1]) (0?[0-9]|1[0-9]|2[0-3]|):(0?[0-9]|[1-5][0-9]):(0?[0-9]|[1-5][0-9])$)|(^$)/;
...
　　　　　　　　　　var thePat = PatternsDict_1[v]; //选择对应的正则表达式
