...
<%
    ...
    String orgName = accesscontroler.getChargeOrgName(); //问卷service
    VotePaperService service = new VotePaperServiceImpl();
    //获得所有问卷类型
    List<Map<String, Object>> paperTypeList = (List<Map<String, Object>>) service
            .findListPaperType();

    boolean isTotal = false; //此处查看配置是否显示统计连接[暂时未加配置，默认为true]
%>
<title>问卷</title>
...
jQuery(function() {
    ...
    $("#gridTable").jqGrid({
        ...
        autowidth: true,        
        colNames: ['问卷id',"考核类型",//注意：更改列后需要将函数onCellSelect也调整下
                   '是否置顶','是否置顶',
                   '考核标题','考核类型','创建人','所属单位','状态',
                    '状态','投票情况','已提交问卷','创建时间',"操作",'创建人id'],
        ...
        gridComplete : function() {
            ...
            for ( var i = 0; i < ids.length; i++) {
                ...
                " onclick='paperYL(\""+cl
                +"\")' style='font-weight:bold;color:red;'>预览</a>";
        ...
        toolbar:[true, "top" ,"<input class='but_y_01' id=add value='添加' type='button' onclick='addLevel()'/>"+
                 "<input class='but_y_01' value='删除' id=del type='button' onclick='deleteFn()'/>"+
                 "<input class='but_y_01' value='问题管理' id=manageQuestion type='button' onclick='addQuestion()'/>"+
                 "<input class='but_y_01' value='复制' id=copyPaper type='button' onclick='copyPaper()'/>"],
        multiselect: true,
        onCellSelect:function (id,iCol)
        {
            
            if(iCol != 0 && iCol != 11&& iCol != 12&& iCol != 14)
            {
                var model = jQuery("#gridTable").jqGrid('getRowData', id);
                updateLevel(model.id,model.stauts);
            }
        }
    }).navGrid('#gridPager', {
        edit: false,
...
var toUserParamJosn = function() {
    ...
    + "','user_id':'"+userId //当前登录的用户（搜索条件用于控制职能看到已发布的问卷，和自己的草稿）
    + "'}";
...
/*
     ...
     返回值: 无
     */
var updateLevel = function(id,updateStatus) {
   
   openWindows('sub_main_updateVotePaper','新建问卷'
           ,'addvotepaper.jsp?subid=sub_main_updateVotePaper&stauts='
           +updateStatus+'&type=0&id='+id,'cz_dwll',false,window);
}
...
var answerRate = function(id){
    ...
    openAlertWindows('windowId_answerRate',url,'考核情况查看','700px','450px','10%','25%');
...
var paperYL = function(id){
    ...
    openWindows('windowId_paperYL','考核预览'
...
var paperTJ = function(id){
    ...
    if(rycpid == model.papaer_type){
        ...
        var form = jQuery("#downFrom").attr("src",url);
        form[0].submit();
        //window.open();
    }else{
      ...
      openWindows('windowId_paperTJ','考核统计'
            ,url,'cz_dwll',false,window);
    }
}
   ...
   //ajax方式提交数据
    jQuery.ajax({ 
        ...
        data : {
            action : "copy",
            id : model.id, //要复制的id
            userId : '<%=userId%>',//执行复制操作的用户id
            userName : '<%=userName%>',//执行复制操作的用户名
            orgId : "<%=orgId%>",//执行复制操作的用户所属机构
            orgName : "<%=orgName%>"//执行复制操作的用户所属机构名            
        },
        dataType: "json", 
        success: function (data) {
            //如果删除成功，刷新数据
            if(data&&data.flag&&"true" == data.flag){
                userSearch();                               
                alert(data.msg+",是否立即编辑复制的考核？",{
                    headerText:'提示',
                    okName:'确认',
                    okFunction:function(){
                       updateLevel(data.id,"0");
                    },
                    cancelName:'取消'
                 }); 
            }else{
    ...
    <tr>
        ...
        <table width="100%" style="padding: 5px;" cellspacing="0"
            ...
            <tr>
                ...
                <th width="100" class="input_cx_title_th">状态：</th>
                <td class="cx_title_td"><select id="stauts" name="stauts"
...
<form id="downFrom" name="downFrom" method="post" action="" target="hiddenFrame">
</form>
</body>
</html>