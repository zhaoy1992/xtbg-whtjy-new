...
<html>
	...
	function view(checkName){		
		...
		document.forms[0].action="<%=rootpath%>/sysmanager/schedularmanage/action/schManager.jsp?method=getSchedularAndRemind&path=history";
	...
	function resume(checkName){		
		...
		document.forms[0].action="<%=rootpath%>/sysmanager/schedularmanage/action/schManager.jsp?method=getResumeSchedular";
		document.forms[0].submit();
	}
	...
	function dealRecord(checkName,dealType) {
	    if(!validatecheck(checkName))
		{
	    	...
	    	if (dealType==1){
	        	...
	        	if (confirm(outMsg)){
		        	Org.action="<%=rootpath%>/sysmanager/schedularmanage/action/schManager.jsp?method=deleteSchedular&path=history";
					Org.submit();
			 		return true;
				}
			} 
