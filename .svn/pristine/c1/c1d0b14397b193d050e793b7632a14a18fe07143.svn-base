...
var toUserParamJosn = function() {
	 var str = "{'apply_personid':'" + loginUserID 
	 		...
	 		//+ "','state':'" + $("#state").val()
	 		+ "','list_type':'"+"carHistoryList','user_id':'"+$("#use_id").val()+"'}";
...
var viewApplyinfo = function(id) {
	...
	var url = "<%=path%>/ccapp/xtbg/yimingju/carmanage/jsp/viewCarManageApplyinfo.jsp?apply_id="+model.apply_id;
	
	openWindows("sub_ymjclsq"+id,window,url+"&hight=true&subid=sub_ymjclsq"+id,'ymjclsq',false,window);
}
				...
				<td>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabs_search_ctable" id="hiddenArea" style="display:none">
						<tr>
							...
							<td  class="cx_title_td" width="120px;">
								<%=infotypeHtml %>
							</td>
							<th class="input_cx_title_th">用车人：</th>
							<td  class="cx_title_td">
								<input type="text" class="validate[required,maxSize[500]] input_150" style="width:90%;" id="use_name" name="use_name" value="" onclick='chooseReportUnits()'></input>
							   <input name="" type="button" value="" class="but_x" onclick="chooseReportUnits()"/>
								<input id="use_id" name="use_id" type="hidden" value=""/>
							</td>
						</tr>
					</table>
						<!-- 隐藏查询 end-->
