...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/commonListServlet?classNameId=infoReportOnlyReadList',  //commonListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['报送id', '标题' , '录入时间', '录入者', '上报单位', '发布时间', '信息状态', '信息状态code'],
		...
		gridComplete: function() { // var idss=jQuery("#gridTable").jqGrid('getColNames');
			...
			for (var i = 0; i < ids.length; i++) {
				...
				if(model.submittedinfo_typename == '未查看') {
					col = "<strong><span class=rowhighlight>"+model.submittedinfo_typename+"</span></a></strong>";
					bt = "<strong><span class=rowhighlight>"+model.submittedinfo_title+"</span></a></strong>";
					nrsj = "<strong><span class=rowhighlight>"+model.input_date+"</span></a></strong>";
					nrz = "<strong><span class=rowhighlight>"+model.inputuser_name+"</span></a></strong>";
					dw = "<strong><span class=rowhighlight>"+model.org_name+"</span></a></strong>";
					fbsj = "<strong><span class=rowhighlight>"+model.release_time+"</span></a></strong>";
					
				} else {
					
					col = model.submittedinfo_typename;
					bt = model.submittedinfo_title;
					nrsj = model.input_date;
					nrz = model.inputuser_name;
					dw = model.org_name;
					fbsj = model.release_time;
				}
				jQuery("#gridTable").jqGrid('setRowData', ids[i], {
					submittedinfo_typename: col,
					submittedinfo_title: bt,
					input_date: nrsj,
					inputuser_name: nrz,
					org_name: dw,
					release_time: fbsj
		...
		filterOptions:[[["信息状态","信息状态"],["已批示","已批示"],["已查看","已查看"],["未查看","未查看"]]],//alue text 后的值与显不
...
//var toUserParamJosn = function() {
	//var str = "{'submittedinfo_title':'"+descape($('#submittedinfo_title').val())+"','org_id':'"+$('#org_id').val()+"',"+
		//		"'input_starttime':'"+$('#input_starttime').val()+"','input_endtime':'"+$('#input_endtime').val()+"',"+
			//	"'infotype_id':'"+$('#infotype_id').val()+"','org_name':'"+descape($('#org_name').val())+"',"+
			//	"'infostate':'"+descape($('#infostate').val())+"',"+
			//	"'inputuser_id':'<%=accesscontroler.getUserID()%>'}";
	//return escape(str);
//}*/
var toUserParamJosn = function() {
	var str = "{'submittedinfo_title':'"+$('#submittedinfo_title').val()+"','org_id':'"+$('#org_id').val()+"',"+
				"'input_starttime':'"+$('#input_starttime').val()+"','input_endtime':'"+$('#input_endtime').val()+"',"+
				"'infotype_id':'"+$('#infotype_id').val()+"','org_name':'"+$('#org_name').val()+"',"+
				"'inputuser_id':'<%=accesscontroler.getUserID()%>'}";
...
var editPerson = function(id) {
	...
	var url = "../../inforeported/jsp/inforeportdetailview.jsp?submittedinfoId="+model.submittedinfo_id+"&state="+model.submittedinfo_typecode+'&operwindowid=sub_inforeportdetailview'+model.submittedinfo_id;
	//location.href = url;
	openWindows('sub_inforeportdetailview'+model.submittedinfo_id,'查看信息',url,'infoview',false,window);
}
