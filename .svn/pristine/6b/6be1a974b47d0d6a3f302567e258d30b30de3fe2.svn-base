...
//tab2
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/commonListServlet?classNameId=purchaseApplyList',  //commonListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['采购申请ID', '申请标题 ', '申请处室','申请日期','状态'],
		...
		filterNames:["state"],
		filterOptions:[[[" ","状态"],["1","等待处室审批"],["2","处室审批通过"],["3","处室审批未通过"],["4","办公室审批通过"],["5","办公室审批未通过"],["6","已采购"]]],//alue text 后的值与显不
		...
		toolbar:[true, "top" ,"<input class='but_y_01' id=add value='新增采购' type='button' onclick='addLevel()' value=''/>"],
		multiselect: false,
...
var toUserParamJosn = function() {
			...
			"'apply_room':'"+$('#apply_room').val()+"','apply_users':'"+ <%=userId%> + "'}";
...
var addLevel = function() {
	openWindows("_sub_windowId","采购申请单","../../officeassets/jsp/purchaseApplyOrder.jsp?widowsid=_sub_windowId&whatDo=1",'_sub_windowId',false,window);
}
...
var updateLevel = function(id) {
	...
	var whatDo ="0";
	var url ="";
	if("处室审批通过"==model.state||"办公室审批通过"==model.state||"已采购"==model.state){
		url ="../../officeassets/jsp/SeeApprovalList.jsp?widowsid=windowId&apply_id=" + model.apply_id;
	}
	if("处室审批未通过"==model.state||"办公室审批未通过"==model.state){
		whatDo="1";
		url ="../../officeassets/jsp/purchaseApplyOrderEdit.jsp?widowsid=windowId&whatDo="+ whatDo +"&apply_id=" + model.apply_id ;
	}
	if("等待处室审批"==model.state){
		Mark="1";
		whatDo="1";
		url ="../../officeassets/jsp/purchaseApplyOrder.jsp?widowsid=windowId&whatDo="+ whatDo +"&apply_id=" + model.apply_id + "&mark="+Mark;
	}
	openWindows("windowId","采购申请单",url,'PurchaseApply',false,window);
}
