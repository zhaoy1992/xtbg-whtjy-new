...
<%
	...
	String sql = " select t.org_name from td_sm_organization t where t.org_id= '"+org_id+"' ";
	String user_orgname = DaoUtil.sqlToField(sql.toString()); 
	if(userBean==null){
	...
	if(!StringUtil.deNull(unit_name).equals("湖南省特检院")){
...
//tab2
	$("#gridTable").jqGrid({
		...
		colNames: ['主键ID','设备编号', '名称','使用部门', '保管人','规格型号','制造厂家','生产日期','出厂编号','领用日期','领用人','库存状态','完好状态','登记人','登记时间','金额','查看预警数量'],
				...
				var cl = ids[i];
				   var model = jQuery("#gridTable").jqGrid('getRowData', cl);
					...
					$("#seebtn").val(html);
	             	...
	             	//设备使用状态  1:在库 2:借出
					if("1"==model.dev_stockflag){
						dev_stockflag1 = "在库";
					} else if("2"==model.dev_stockflag){
						dev_stockflag1="借出";
					} else {
						dev_stockflag1="";
					}
	             	//设备完好状态  1:完好2:在修 3:报废 4:损耗
					if("1"==model.dev_state){
						dev_state1 = "完好";
					} else if("2"==model.dev_state){
						dev_state1="在修";
					} else if("3"==model.dev_state){
						dev_state1="报废";
					} else if("4"==model.dev_state){
						dev_state1="损耗";
					} else if("5"==model.dev_state){
						dev_state1="停用";
					} else{
						dev_state1="";
					}
					jQuery("#gridTable").jqGrid('setRowData', ids[i], {
						dev_stockflag:dev_stockflag1,
						dev_state:dev_state1
					});
			}
		},
						...
						<!-- 隐藏查询 -->
						 <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabs_search_ctable" id="hiddenArea" style="display:none">
							...
							<tr>
									...
									<td >
										<input type="text" id="user_orgname" name="user_orgname" class="input_cx_title_283" value="<%=user_orgname %>">
									</td>
							</tr>
							<tr>
