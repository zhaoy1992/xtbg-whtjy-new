...
<%
	...
	String item_plainid = request.getParameter("item_plainid");
%>
...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/pubListServlet?classNameId=com.chinacreator.xtbg.tjy.device.list.DevAppItemList',  //pubListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['物品ID','申购ID','物品名称','型号规格','技术参数','推荐制造厂家','数量 ','购置理由','预算费用 ',  '排序号  '],
...
var toUserParamJosn = function() {
		var str="";
    	var str = "{'item_plainid':'<%=item_plainid%>'}";
...
function handle(id){
	 ...
	 var item_name = model.item_name;                    //物品名称
	 var item_type = model.item_type;                    //规格参数
	 var item_jscs = model.item_jscs;                    //技术参数
	 var item_tjzccj = model.item_tjzccj;                //推荐厂家
	 var item_sl = model.item_sl;                        //数量
	 var item_no = model.item_no;                        //排序号
	 var item_ysfy =model.item_ysfy;					 //预算费用
	 var item_id = model.item_id;						//物品明细ID
	 
	 var parentParentObject = getParentWindow('windowIds2').getParentWindow('windowIds1');
	 var parentObject = getParentWindow('windowIds2');
	 
	 parentParentObject.setSBinfo(item_name,item_type,item_tjzccj,item_id,item_ysfy);
	 parentObject.removeWindws();
	 removeAlertWindows('windowIds2',true,true,'',false,'','');
}
