...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/pubListServlet?classNameId=com.chinacreator.xtbg.tjy.canteenitemssupplies.list.OsExaForHavaObjectList',  //pubListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['计划审批表id','机构ID','申请标题','机构名称','填报时间','申请人名称 '],
    	...
    	{
   			name: 't_osexa_org_name',
   			index: 't_osexa_org_name',
   			width: '40',
			search: true
    	},
    	{
		...
		{
			name: 't_osexa_user_name',
			index: 't_osexa_user_name',
			width: '100',
			search: true
		}
		],
...
function addLevel(id){   
    ...
    $(selectedIds).each(function () {
		...
		if(!IsSpace(model.t_osexa_id)){
			ids += model.t_osexa_id;
		}
	});
