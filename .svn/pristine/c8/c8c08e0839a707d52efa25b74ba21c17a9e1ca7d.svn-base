...
<%	
	...
	if( inputTypeId.equals("3") || inputTypeId.equals("25")) //当前时间或者选择时间时确定日期格式
	{
	    dateFormat = selectDicttye;
	}
	else if(selectDicttye.trim().length()>0){
    ...
    dictatt.setDateFormat(dateFormat);
	boolean saveFlag = false;
	
	saveFlag = dictManager.storeDictdataAttachField(dictatt);
