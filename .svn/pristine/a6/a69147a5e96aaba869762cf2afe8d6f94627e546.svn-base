 ...
 *2013-9-13
 */
public class FlowInfoTree extends TreeDataInfo {
	...
	public Object getTreeDateInfo() throws Exception {
		...
		sql.append("select * from OA_FLOW_BUSITYPE_TYPE WHERE IS_VALID = '1' AND IS_FLOW = '1' ");
		...
		if(!StringUtil.isBlank(orgid)){
			...
			sql.append(" where instr('"+orgid+"',b.org_id) >0");

		}
		sql.append(" ) ) ");
		if(!StringUtil.isBlank(busitype_type_code)){
			sql.append(" and busitype_type_code='"+busitype_type_code+"'");
		}
