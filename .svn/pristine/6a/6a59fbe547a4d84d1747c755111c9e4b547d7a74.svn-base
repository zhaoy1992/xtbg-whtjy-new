...
//tab2
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/commonListServlet?classNameId=myAssetsList',  //commonListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['设备ID', '设备名称', '类别','设备编号','型号','领用日期','状态'],
...
var toUserParamJosn = function() {
			...
			"'eq_name':'"+$('#equipment_name').val()+"','collar_user':'"+<%=userid%>+"'}";
...
var updateLevel = function(id) {
	...
	var collarState= model.collarState;
	if(collarState=="归还"){
		collarState='1';
	}else if(collarState=="领用"){
		collarState='2';
	}else if(collarState=="报废"){
		collarState='3';
	} else{
		collarState=' ';
	}
	openWindows("windowId","资产信息","../../officeassets/jsp/SeeAssetsList.jsp?widowsid=windowId&stock_Id=" + model.stock_Id+"&collarState="+collarState,'myAssetsList',false,window);
}
