...
/*
*######################################
* eWebEditor v6.0 - Advanced online web based WYSIWYG HTML editor.
...
function jC() {
    ...
    if (config.AutoDetectPasteFromWord == "1") {
        var ab = /<\w[^>]* class=\"?(MsoNormal|MsoHeader)\"?/gi;
        var b = ab.test(as);
        if (!b) {
            ab = /<\w[^>]* mso-/gi;
            b = ab.test(as);
...
function setHTML(html, nL) {
    ...
    switch (H.aK) {
      ...
      case "EDIT":
        ...
        D().write(hh() + iK(setPageTagDisplay(html))  + "</body>");
...
function hh() {
    ...
    switch (H.aK) {
      case "CODE":
        dQ += "<link href='" + H.bV + "/skin/" + config.Skin + "/editorarea.code.css' type='text/css' rel='stylesheet'>";
        break;
      case "TEXT":
        dQ += "<link href='" + H.bV + "/skin/" + config.Skin + "/editorarea.text.css' type='text/css' rel='stylesheet'>";
        break;
      case "EDIT":
      case "VIEW":
        if (config.AreaCssMode != "1") {
            dQ += "<link href='" + H.bV + "/skin/" + config.Skin + "/editorarea.edit.css' type='text/css' rel='stylesheet'>";
        }
        dQ += H.gb;
        break;
    }
...
function pH() {
    ...
    jE = "<head>" + "<link href='" + H.bV + "/skin/" + config.Skin + "/menuarea.css' type='text/css' rel='stylesheet'>" + "</head>" + "<body scroll='no' onConTextMenu='event.returnValue=false;' ondragstart='event.returnValue=false;' onselectstart='event.returnValue=false;' onselect='event.returnValue=false;'>";
...
function bD(menu) {
    ...
    switch (menu) {
      ...
      case "edit":
        ...
        C += F(at, "UnDo");
        if (I.position >= I.data.length - 1 || I.data.length == 0) {
            at = "disabled";
        }
        C += F(at, "ReDo");
        C += aw;
...
function mE() {
    ...
    document.write("<form id='eWebEditor_UploadForm' action='" + config.ServerExt + "/upload." + config.ServerExt + "?action=remote&type=remote&style=" + an.StyleName + "&cusdir=" + an.CusDir + "' method='post' target='eWebEditor_UploadTarget'>");
...
/**将分页置标转换成图片**/
function setPageTagDisplay(text){
	text = text.replace(/<CMS_PAGE_SEPARATOR><\/CMS_PAGE_SEPARATOR>/g, "<img src=\"sysimage/customimage/page.gif\" id=\"_CMSPage\">");
	return text;
}
...
function insertTRSPage(){
	eWebEditor.focus();
	insertHTML("<img src=\"sysimage/customimage/page.gif\" id=\"_CMSPage\">");
}
