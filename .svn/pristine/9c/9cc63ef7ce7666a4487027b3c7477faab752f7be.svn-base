...
$(function() {
   ...
   jQuery("#degree_of_importance").change(function(){
	...
	toolbar: [true,"top","<input class='but_y_01' id='add' type='button' onclick='oK()' value='确定'/>"],
	multiselect: false,
	...
	var oK = function() {
		...
		if (IsSpace(ids)) {
			...
			var tasks = {
					...
					"task_needinfo" :model.task_needinfo,//任务要求
					"task_content" :model.task_content//生产厂
				}
				 var parentObject = getParentWindow('windowId2');
				 parentObject.setInfoTasks(tasks);
