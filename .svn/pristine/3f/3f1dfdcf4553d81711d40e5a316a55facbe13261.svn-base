	...
	$("#tabs").tabs({//每个页面只刷新一次
		select : function(event, ui) {
			...
			if ($(iframeitem).attr("id")=="frame2" && $(iframeitem).attr("src")==""){
				$(iframeitem).attr("src", "meetingreceiverlist4nosignup.jsp?m_notice_id=<%=m_notice_id %>&m_is_reply=no");
			}
		}
	});
	$("#titleids").css('display','block');
    adaptationWH('titleids','vcenter',30);
				...
				<div>
					<div id="tabs">
						<div id="titleids" style="display:none;">
							<ul>
								<li><a href="#tabs-1">已报名</a></li>
								<li><a href="#tabs-2">未报名</a></li>
							</ul>
						</div>
						<div id="maindiv" class="vcenter" style="height:300px;overflow-x:hidden;overflow-y:auto;">
							<div id="tabs-1">
								<iframe src="meetingreceiverlist.jsp?m_notice_id=<%=m_notice_id %>&m_is_reply=<%=MeetingConstants.REPLY_YES %>" id="frame1" frameborder="0"
									 height="750px;" width="98%"></iframe>
							</div>
							<div id="tabs-2">
								<iframe src="" id="frame2" frameborder="0"
								  height="1000px;"	width="98%"></iframe>
							</div>
						</div>
					</div>
				</div>
