...
$(function() {
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/commonListServlet?classNameId=infoReportRecommendedList',  //commonListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['报送id', '标题' , '推荐时间', '推荐人', '上报单位', '发布状态', '信息状态code', '推荐', '是否推荐'],
		...
		{
			name: 'recommend_time',
			index: 'recommend_time',
			width: 100,
			editable: true
		},
		{
			name: 'recommend_username',
			index: 'recommend_username',
			width: 100,
			editable: true
		},
		{
		...
		gridComplete: function() { // var idss=jQuery("#gridTable").jqGrid('getColNames');
			...
			for (var i = 0; i < ids.length; i++) {
				...
				if("Y" == model.is_recommend){
					be = "<button id='modify' class='ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all' onclick=setRecommend('"+model.submittedinfo_id+"','N')>" + 
				     "<span class='ui-button-text'>已推荐</span>" +
					 "</button> ";
				}else{
					be = "<button id='modify' class='ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all' onclick=setRecommend('"+model.submittedinfo_id+"','Y')>" + 
				     "<span class='ui-button-text'>推荐</span>" +
					 "</button> ";
				}
	...
	.append("<input class='cr' id=cancelrecommended type='button' onclick='CancelRecommended()' />")
	.append("</div>");
...
var editPerson = function(id) {
	...
	var url = "infoviewforrecommended.jsp?submittedinfoId="+model.submittedinfo_id+"&state="+model.submittedinfo_typecode;
	location.href = url;
}
...
var CancelRecommended = function(){
	var ids = "";
	var tempArrs = [];
	...
	if (IsSpace(ids)) {
		...
		var url = "infolistrecommendsaveform.jsp";
...
<input id="is_recommended" name="is_recommended" type="hidden" value="N"></input>
</form>
