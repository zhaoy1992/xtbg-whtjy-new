...
<%
    ...
    BusiExcelToData busiExcelToData = (BusiExcelToData) LoadSpringContext
			.getApplicationContext().getBean(excelImplClass);
    ...
    if ("batch".equals(wdo)) {
        ...
        String file_url = busiExcelToData.getTempExportFilePath();//应用更新临时文件路径
        ...
        if ("false".equals(err)) {
            outStr = busiExcelToData.importExcelToData(file_url);
            java.io.File fileurl = new java.io.File(file_url);
        	fileurl.delete();
        }
    }
