...
xyTree.Node.prototype.innerhtml = function() {
  ...
  img.onclick = function() { 
    ...
    if(current.checked==0&&tempMap.get(current.id)=="true"){
   		var a = []; var j=0;
      	for(i= 0;i<map.size() ;i++){
      			if(map.keys[i].indexOf(current.id)!=-1){
      				a[j++] = map.keys[i];
      			}
      	}
    ...
    if (current.checked == 1 || current.checked == 2) {
      if(defTempMap.get(current.id)!=true)map.remove(current.id);
      current.checked = 0;
