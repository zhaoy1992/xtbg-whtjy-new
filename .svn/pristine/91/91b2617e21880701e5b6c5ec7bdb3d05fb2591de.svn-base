...
$(function() {
	...
	$("#gridTable").jqGrid({
		...
		colNames: ['f_destroy_id','ins_id','busi_id','busitype_code','标题','销毁理由','批准人','申请日期','流程状态'],
		...
		toolbar: [true,"top","<input type='button' value='销毁申请' class=\"but_y_01\" onclick=\"addflow();\"/>"+
			"<input  type='button' value='删除' class=\"but_y_01\" onclick=\"del();\"/>"],
		multiselect: true,
...
var addflow = function(){
	var url='<%=path%>/ccapp/oa/file/jsp/filedestroyflow.jsp?busi_id=&busiTypeCode=uuid-337&subid=windowsId';
	var height = window.screen.availHeight;
	var width = window.screen.availWidth;
	openAlertWindows('newwindows',url,"档案销毁流程",width,height,'0%','0%');
}
	
function handle(model){   
	 var url = "filedestroyflow.jsp";
	 url += "?busi_id="+model.busi_id;
	 url += "&ins_id="+model.ins_id;
	 url += "&busiTypeCode="+model.busitype_code;
	 url += "&jsptype=false&operType=yiban&backurl=filedestroytabs.jsp";
...
<table width="100%">
	<tr>
		...
		<!-- 查询列表区 -->
		<form id="delForm1" name="delForm1" method="post" action="filedestroyDo.jsp?subid=<%=subid%>">
			<input type="hidden" name="f_destroy_id" id="f_destroy_id" /> 
