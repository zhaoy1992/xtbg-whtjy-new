...
日期：2014-02-21
--%>
<%@page import="com.chinacreator.xtbg.core.personholder.entity.TrainBean"%>
...
<%
	...
	if("addInfo".equals(action)){
		...
		bean = (TrainBean)RequestBeanUtils.getValuesByRequest(request,bean);
		String beanId = service.insertOrUpdateTrain(bean);
	...
	if("deleteInf".equals(action)){
		...
		if(deleteArr.length>0){
			flagStr = service.deleteTrainByIds(deleteArr);
		}
