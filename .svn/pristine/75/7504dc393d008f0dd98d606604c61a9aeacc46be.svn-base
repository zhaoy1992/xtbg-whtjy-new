...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	function saveform(closeFlag){
		...
		//}
			...
			myform.action="<%=request.getContextPath()%>/cms/zwgkml/add_document.jsp?flag=" + closeFlag;
	...
	function subpublish(){
		...
		showModalDialog("/cms/docManage/doc_PublisherList.jsp?flag=4&channelId=<%=channelId%>",window,"dialogWidth:400px;dialogHeight:500px;help:no;scroll:auto;status:no");
	}
	function publish(){
	    if(!isURL(document.all("content").value)){
		    alert('请填写正确的URL!');
	...
	function deliver(){
		...
		showModalDialog("/cms/docManage/doc_AuditorList.jsp?flag=4&channelId=<%=channelId%>",window,"dialogWidth:400px;dialogHeight:500px;help:no;scroll:auto;status:no");
	}
	//检测URL的合法性
	function isURL(url){
			...
			%>
							...
							<tr>
								...
								<td height="23" colspan="2">
									...
									<input type="button" name="addpic" class="Channel_5wordsBtn" onClick="chooseImage()" value="选择图片" />
								</td>
								<td height="23">
