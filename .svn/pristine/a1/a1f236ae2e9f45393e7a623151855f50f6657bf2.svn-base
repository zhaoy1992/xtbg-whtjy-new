...
$(function() {
	...
	$("#gridTable").jqGrid({
		...
		colNames: ['库存id','物品名称','物品类型ID','物品类型名称','规格名称 ','设备编号','入库时间'],
		...
		onPaging : function(pgButton,w) {      //当点击翻页按钮但还为展现数据时触发此事件, 当然这跳转栏输入页码改变页时也同样触发此事件
		    var selectedIds = $("#gridTable").jqGrid("getGridParam", "selarrrow");
		    $(selectedIds).each(function() {
		          var model = jQuery("#gridTable").jqGrid('getRowData', this);
          		  if (!IsSpace(model.t_ha_id)){
		                	if(t_ha_ids.indexOf(model.t_ha_id)<0){
		                		t_ha_ids += (model.t_ha_id+',');
		                	}
		                }
		     });
...
var toUserParamJosn = function() {
		 var str = "{'t_ha_iswhat':'1','ofustype_name':'"+jQuery("#ofustype_name").val()+"','it_name':'"+jQuery("#it_name").val()
 		+"','startSlTime':'"+jQuery("#startSlTime").val()+"','endSlTime':'"+jQuery("#endSlTime").val()+"','t_ha_sbstate':'1'}";
...
function addLevel(id){   
    ...
    $(selectedIds).each(function () {
		var model = jQuery("#gridTable").jqGrid('getRowData', this);
		if(!IsSpace(model.t_ha_id)||t_ha_ids.indexOf(ids)<0){
			t_ha_ids += ',';
		}
		if(!IsSpace(model.t_ha_id)||t_ha_ids.indexOf(ids)<0){
			t_ha_ids += model.t_ha_id;
    ...
    if (IsSpace(t_ha_ids)) {
		...
		var okF = function(){
			...
			parentObject.loadInfo(t_ha_ids);
