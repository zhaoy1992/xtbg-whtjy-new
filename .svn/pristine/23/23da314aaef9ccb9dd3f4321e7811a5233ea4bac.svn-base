...
<%@ page import="java.util.ArrayList,com.chinacreator.sysmgrcore.manager.RoleManager"%>
...
<%
	...
	List hasOper = operManager.getOperResRoleList(role_type,roleid,resid,"site.channel");
	if(list == null){
		list = new ArrayList();
	}	
...
<SCRIPT LANGUAGE="JavaScript"> 
	...
	function okRecord(dealType,id) {
			...
			Form1.action="<%=rootpath%>/accessmanager/securityManager.do?method=sitecnlAuthorization&isRecursion="+tt+"&resTypeId="+id;
			Form1.submit();
			...
			<table width="100%" height="22" border="0" cellpadding="0" cellspacing="1" class="thin">
			      ...
			      <%boolean flag1;
					...
					if (flag1 == true) {%>
							 <tr>
			        <td>
			        <input type="checkBox" hideFocus=true name="checkBoxAll" onClick="checkAll('checkBoxAll','alloper1')"checked disabled="true">
			        </td>
			        <td>>>>>全选</td>
			        <td>&nbsp;<input name="isRecursion" onclick="" type="checkbox"checked disabled="true">递归授权子频道</td>
			      </tr>
			      ...
			      <tr class="tr">
			        ...
			        %>
			        value="<pg:cell colName="id"/>" onclick="changebox1(this,'<pg:cell colName="priority" defaultValue=""/>','<pg:cell colName="id"/>')"checked disabled="true">
			        </td>
					...
					<tr>
			        <td>
			        <input type="checkBox" hideFocus=true name="checkBoxAll" onClick="checkAll('checkBoxAll','alloper1')">
			        </td>
			        <td>>>>>全选</td>
			      ...
			      <%
					...
					if(!accesscontroler.checkPermission(resid,opId,"site.channel"))
			      ...
			      >
			       ...
			       <tr>
			        ...
			        <td></td>
			        <td>
			        <input type="submit" value="确定" class="input" onclick="javascript:okRecord(1,'site.channel'); return false;">
