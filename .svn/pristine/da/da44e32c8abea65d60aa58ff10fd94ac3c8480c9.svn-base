...
日期：2014-03-07
--%>
<%@page import="com.chinacreator.xtbg.tjy.detectionsupplies.service.impl.OsupplieRepairServiceImpl"%>
<%@page import="com.chinacreator.xtbg.tjy.detectionsupplies.service.OsupplieRepairService"%>
<%@page import="com.chinacreator.xtbg.tjy.detectionsupplies.entity.OsupplieRepairBean"%>
...
<%@page import="com.chinacreator.xtbg.tjy.detectionsupplies.dao.OsupplieRepairDao"%>
...
<%
	...
	if("insert".equals(actionName)){
		...
		bean.put("deal_status", "doing");//设置维修类别为固定资产
	} else {
	...
	bean.put("repair_type", "assets");//设置维修类别为固定资产
...
function choseObject(){
	var path ="<%=path%>/ccapp/xtbg/tjy/detectionsupplies/repair/jsp/choseosnomoveobjectlist.jsp?windowsId=windowIds";
	openAlertWindows('windowIds',path,'固定资产列表',810,400,'25%','25%');
}

function loadInfo(ids,ofustype_names){
	jQuery("#replace_partids").val(ids);
	jQuery("#replace_parts").val(ofustype_names);
}
	...
	<form id="form1" action="repairDevicedo.jsp" method="post"  >
		...
		<div id="users-contain">
			<table  width="100%" border="0" cellspacing="0" cellpadding="0" >
					...
					<tr>
						<td class="content_02_box_div_table_th" class="poptableLable">故障设备：</td>
						<td class="content_02_box_div_table_td">
						<input type="hidden" name="replace_partids" id="replace_partids" class="validate[maxSize[1000]] input_188" value="<%=StringUtil.deNull(bean.get("replace_partids"))%>">
						<input style="width:90%;height=23px" type="text" readonly="readonly" name="replace_parts" class="validate[maxSize[1000]] input_188"
							id="replace_parts"  onclick="choseObject();"
							value="<%=StringUtil.deNull(bean.get("replace_parts"))%>">
						</td>
					</tr>
					<tr>
