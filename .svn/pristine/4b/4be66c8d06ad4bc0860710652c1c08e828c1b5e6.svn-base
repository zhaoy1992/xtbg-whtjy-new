...
var updateactuvuty = function(id) {
	...
	var url = "<%=path%>/ccapp/xtbg/caizhengting/ibrary/jsp/ibraryborrowadd.jsp?subid=queryinfo&whereForm=1&t_regb_id="+model.t_regb_id+"&sqtype="+model.t_regb_sqtype;
...
<table width="100%" style="margin-left: 0" class="tabs_search_ctable_box">
	...
	<tr>
		...
		function toParamJosn(){
			...
			var str = "{'whatTabs':'2','jy_start_time':'"+jy_start_time+"','jy_end_time':'"+jy_end_time+"','t_regb_gh_date':'<%=nowDateString%>','t_regb_dept_id':'<%=dept_id%>'}";
	    ...
	    */
	    function mathLeaveDays(startTime,endTime){
	    	/* if(startTime != "" && endTime != ""){
	    		var start_time = startTime;//开始时间
	    		start_time = start_time.replace(/-/g,"/");
	    		start_time = new Date(start_time);
	    		
	    		var end_time = endTime;//结束时间
	    		end_time = end_time.replace(/-/g,"/");
	    		end_time = new Date(end_time);
	    		if(start_time>end_time){
	    			 alert('开始时间大于结束时间，请重新选择');
