...
<%
	...
	Event event = null;
	if (eventType.equals("")) {
	 event = new EventImpl(map, WebOfficeEventType.DELETE_EVENT,eventTarget, Event.REMOTE);
	} else {
	 event = new EventImpl(map, AttachEventType.DELETE_TYPE,eventTarget, Event.REMOTE);
	}
