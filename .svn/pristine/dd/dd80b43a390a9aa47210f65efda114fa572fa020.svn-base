...
<html>
	...
	function modify(checkName){		
		...
		document.forms[0].action="<%=rootpath%>/sysmanager/schedularmanage/action/schManager.jsp?method=getModifySchedular&path=bookingSchedularList";
	...
	function view(checkName){		
		...
		document.forms[0].action="<%=rootpath%>/sysmanager/schedularmanage/action/schManager.jsp?method=getSchedularAndRemind&path=bookingSchedularList";
		document.forms[0].submit();
	}
	function history(checkName){
		...
		document.forms[0].action="<%=rootpath%>/sysmanager/schedularmanage/action/schManager.jsp?method=history&path=bookingSchedularList";
		document.forms[0].submit();
	}
	...
	function dealRecord(checkName,dealType) {
	    if(!validatecheck(checkName))
		{
	    	...
	    	if (dealType==1){
	        	...
	        	if (confirm(outMsg)){
		        	Org.action="<%=rootpath%>/sysmanager/schedularmanage/action/schManager.jsp?method=deleteSchedular&path=bookingSchedularList";
					Org.submit();
			 		return true;
				}
			} 
		...
		<div id="contentborder" align="center">
			<FORM name="Org" action="" method="post">
				<TABLE width="100%" cellpadding="1" cellspacing="1" bordercolor="#EEEEEE" class = "thin">
					...
					<pg:listdata dataInfo="BookingSchedularList" keyName="BookingSchedularList" />

					<pg:pager maxPageItems="15" scope="request" data="BookingSchedularList" isList="false">
