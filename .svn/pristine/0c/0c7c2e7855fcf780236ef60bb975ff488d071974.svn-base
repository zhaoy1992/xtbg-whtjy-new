	...
	jQuery("#gridTable").jqGrid({
		...
		colModel: [ //最重要的数组之一，用于设定各列的参数
           {
			...
			label:'flow_action_id',
   			name: 'flow_action_id',
   			index: 'flow_action_id',
   			hidden: true
   		},{
			label:'recordid',
   			name: 'recordid',
		...
		gridComplete: function() { // var idss=jQuery("#gridTable").jqGrid('getColNames');
			...
			for (var i = 0; i < ids.length; i++) {
				...
				if(!IsSpace(model.recordid)){
					be = "<input id='modify' type='button' class='but_y_01' onMouseOver=javascript:$(this).attr('class','but_y_02'); onMouseOut=javascript:$(this).attr('class','but_y_01'); onclick='backnow("+cl+")' value = '恢复' />";
...
var backnow=function(id){
	...
	var okF=function(){
		jQuery.ajax({ 
	        ...
	        contentType: "application/x-www-form-urlencoded; charset=utf-8",
	        data : {
	        	flow_act_id : model.flow_action_id,
