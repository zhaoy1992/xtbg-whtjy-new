...
$(function() {
	...
	//接收表
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/commonListServlet?classNameId=carManageApplyinfoYmjList',  //commonListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['apply_id','申请人ID','用车人','用车人id','用车时间','用车结束时间','申请时间','所用车辆','car_id','状态'],
		//字符串数组，用于指定各列的题头文本，与列的顺序是对应的。
		colModel: [ //最重要的数组之一，用于设定各列的参数
		{name: 'apply_id',index: 'apply_id',hidden:true,align:'left'},
		{name: 'apply_personid',index: 'apply_personid',hidden:true,align:'left'},
		{name: 'use_name',index: 'use_name',edit:false,width: 200,sortable: false},
	    {name: 'use_id',index: 'use_id',edit:false,hidden:true,sortable: false},
		{name: 'use_starttime',index: 'use_starttime',edit:false,width: 200,sortable: false},
   	 	{name: 'use_endtime',index: 'use_endtime',edit:false,hidden:true,sortable: false},
   		{name: 'sqrq',index: 'sqrq',width: 150,align:'left',sortable: false},
    	{name: 'car_num',index: 'car_num',width: 100,align:'left',sortable: true},
    	{name: 'car_id',index: 'car_id',hidden:true},
    	{name: 'state',index: 'state',width: 120,align:'left',sortable: false}
		],
		sortname: 'userid',
		...
		filterOptions:[
		               [
						//["","状态"],["1","等待审批"],["2","审批通过"],["3","审批未通过"],["4","等待车辆分派"],["4","不派车"],["5","已派车"],["6","派车结束"]
		                ["","状态"],["1","等待派车"],["6","派车不通过"],["4","等待审批"],["3","审批未通过"],["2","审批通过"],["5","派车结束"]
		               ],//value text 后的值与显不
		              ],
		filterOptionsDefValue:[0,2], 
		...
		toolbar: [true,"top","<input type='button' class='but_y_01' value='用车申请' onMouseOver=javascript:$(this).attr('class','but_y_02'); onMouseOut=javascript:$(this).attr('class','but_y_01'); onclick='addApplyinfo()'/>"],
		multiselect: false,
...
var viewApplyinfo = function(id) {
	...
	var url = "<%=path%>/ccapp/xtbg/yimingju/carmanage/jsp/viewCarManageApplyinfo.jsp?hight=true&apply_id="+model.apply_id;
