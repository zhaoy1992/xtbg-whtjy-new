...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%=path%>/pubListServlet?classNameId=com.chinacreator.xtbg.core.file.list.MyFileLendingForMeNowList',  //pubListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
...
function handle(id){ 
	var model = jQuery("#gridTable").jqGrid('getRowData', id);
	if(model.page_name){
		openAlertWindows('updateFileDocDetailWindow', getCurrentFilePath() +model.page_name+ '?busi_id='+model.f_doc_id+'&busiTypeCode=wj-001&windowId=updateFileDocDetailWindow&isview=1&isgd=yes', '查看文件' ,1020,520,'10%','10%');
	}
}
function borrowAdd(){ 
