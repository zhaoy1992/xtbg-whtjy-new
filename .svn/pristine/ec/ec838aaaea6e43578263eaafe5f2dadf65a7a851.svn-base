...
showFlowHistoryByInsId = function(insid){
	var url = getContextPath()+"/ccapp/xtbg/public/workflow/jsp/showflowhistorytiaozhuan.jsp?insid="+insid;
...
function getLocalDateTime(){
	var date = new Date();
	var fullyear = date.getFullYear();
	var month = changeDateNum(date.getMonth()+1);
	var day = changeDateNum(date.getDate());
...
function defaultValueControl(showValue,inputId,defaultColor,normalColor){
	...
	this.setBlurValue = function(){
	   // 如果用户没有输入任何值，则设置为默认值
	   if(!$(This.inputId).val() || $(This.inputId).val()==This.showValue){		   
		 // 设置默认颜色为灰色
        $(This.inputId).css('color',This.defaultColor);
        $(This.inputId).val(This.showValue);        
        $(This.inputId).attr("val",This.showValue);
	   }
	}

	this.setFocusValue = function(){
        if(This.showValue == $(This.inputId).val()){
        	$(This.inputId).val("");
        	$(This.inputId).css('color',This.normalColor);
    		$(This.inputId).attr("val","");
        }		
	}

	this.init = function(){
        // 绑定事件
		$(this.inputId).bind('blur',this.setBlurValue);
		$(this.inputId).bind("focus",this.setFocusValue);
		// 初始设值
		this.setBlurValue();
	}
	/**
...
/**
* 上传附件后，自动取上传的第一个文件的文件名为title，如果title为空的话,tmp_titleDom,这个为"#title"或".title"，这种形式
* tmp_fileName 文件名，带后缀，"readme.txt"
* add by 陈建华，附件上传的时候会用，自动取文件名填充标题
*/
function autoTitle(tmp_titleDom,tmp_fileName){
	var tmp = jQuery.trim(jQuery(tmp_titleDom).val());
	if(tmp == ""){
		if(!IsSpace(tmp_fileName)){
			try {
				var lastindexofpoint = tmp_fileName.lastIndexOf(".");
				if( lastindexofpoint != -1){
					var fileName = tmp_fileName.substring(0,lastindexofpoint);
					jQuery(tmp_titleDom).val(fileName);
				} else {
