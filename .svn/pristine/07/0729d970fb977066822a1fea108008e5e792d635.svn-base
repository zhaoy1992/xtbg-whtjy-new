...
<%
	...
	//判断是否是部门或者单位管理员
	boolean roleunit=KbmUtil.isRolesUser(accesscontroler.getUserID(), DataDataBean.ROLE_UNIT);
	boolean roledept=KbmUtil.isRolesUser(accesscontroler.getUserID(), DataDataBean.ROLE_DEPT);
%>
	...
	$("#gridTable").jqGrid({
		...
		onCellSelect:function(rowid,iCol){
			if(iCol>0){
				...
				urlTmp = "&d_data_id="+model.d_data_id+"&actionName=update&d_type_id="+model.d_type_id;
				toNewPage(urlTmp,"修改");
			}
		}
	}).navGrid('#gridPager', {
...
function toParamJosn(){
	...
	if(<%=roleunit%>){
		var str = "{'d_type_id':'<%=d_type_id%>','d_data_name':'"+d_data_name+"','d_data_code':'"+d_data_code+"','d_data_unit_id':'<%=d_data_unit_id%>','d_data_year':'"+d_data_year+"'}";
	}else if(<%=roledept%>){
		var str = "{'d_type_id':'<%=d_type_id%>','d_data_name':'"+d_data_name+"','d_data_code':'"+d_data_code+"','d_data_dept_id':'<%=userBean.getOrg_id()%>','d_data_year':'"+d_data_year+"'}";
	}else{
		var str = "{'d_type_id':'<%=d_type_id%>','d_data_name':'"+d_data_name+"','d_data_code':'"+d_data_code+"','d_data_user_id':'<%=userId%>','d_data_year':'"+d_data_year+"'}";	
	}
	return escape(str);
}
...
<table width="100%" style="margin-left: 0" class="tabs_search_ctable_box">
	<tr>
		<td width="100%" align="left" valign="top">
			<input id="AddOne" name="AddOne" value="增加" type="button" onclick="btnClickFn(this)" class="but_y_06" /><span style="width:5px;display:block;float:left;">&nbsp;</span>
