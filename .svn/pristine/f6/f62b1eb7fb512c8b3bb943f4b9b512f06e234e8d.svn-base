...
<%
	...
	//System.out.println(grinfoIds+"11");
%>
...
$(function() {
	...
	//接收表
	$("#gridTable1").jqGrid({
		url: '<%= path %>' + '/commonListServlet?classNameId=loginViewList',  //commonListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
...
var editguestinfo1 = function(id) {
	...
	var url = "guestreceptionView.jsp?grinfo="+model.grinfo_id+"&statusname="+status+"&loginUserID="+loginUserID+"&from=2&feedbackStatus="+model.feedbackStatus;
	/**
	location.href = url;
	**/
	//var model = jQuery("#gridTable").jqGrid('getRowData', id);
	//url = "guestreceptionEdit1.jsp?grinfo="+model.grinfo_id+"&statusname="+status;
	//location.href = url;
	openWindows("sub_lbjdview"+id,"来宾接待信息查看","../../../guestreception/jsp/"+url+"&subid=sub_lbjdview"+id,'lbjdview',false,window);
								...
								<tr>
									<td>
										<table width="100%" border="0" cellspacing="0" cellpadding="0"
											...
											<tr>
												<th class="input_cx_title_th">抵达日期：</th>
												<td class="cx_title_td">
													<input type="text" id="startSlTime" name="startSlTime"
														class="date_120"
														onclick="WdatePicker({dateFmt:'yyyy-MM-dd H:mm'})" />
													<span class="date_120_span">至</span>		
													<input type="text" id="endSlTime" name="endSlTime"
														class="date_120"
														onclick="WdatePicker({minDate:'#F{$dp.$D(\'startSlTime\')}',dateFmt:'yyyy-MM-dd HH:mm'})" />
												</td>
