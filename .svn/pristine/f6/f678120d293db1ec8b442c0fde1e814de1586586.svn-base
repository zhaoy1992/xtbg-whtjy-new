...
<%
	...
	String k_create_user_name=StringUtil.deNull(request.getParameter("k_create_user_name"));
%>
<title>知识库新增文档查询界面</title>
<!-- 引入jQuery -->
...
$(function() {
	if(""!="<%=k_create_user_name %>"){
		$("#k_create_user_name").attr("disabled","disabled");
		$("#k_create_user_name").val("<%=k_create_user_name %>");
	}
	...
	$("#gridTable").jqGrid({
		url: '<%=path%>' + '/pubListServlet?classNameId=com.chinacreator.xtbg.core.kbm.list.NewKbmDocFindList',  //pubListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
...
var toJson = function() {
	var str ="{'k_title':'"+$("#k_title").val()+
		...
		"','k_check_user_name':'<%=k_create_user_name%>'}";
				...
				<td>
					...
					<table id="searchTable" width="100%" border="0" cellspacing="3"
						...
						<tr>
							...
							<th class="input_cx_title_th" style="width: 90px;">标题：</th>
							<td  class="cx_title_td"><input type="text"
								class="input_cx_title_150" id="k_title"
								name="k_title" /></td>
							<th style="width: 90px;" class="input_cx_title_th">编号：</th>
							<td class="cx_title_td"><input
								class="input_cx_title_150"  name="k_code"
								id="k_code" /></td>
							...
							<td ><input value="搜索" id="sel" type="button"
								onclick="sysKbmDocSearch();" class="but_y_01" /></td>
						</tr>
						<tr>
							<th style="width: 90px;" class="input_cx_title_th">上传人：</th>
							<td  class="cx_title_td"><input type="text" 
								id="k_create_user_name" name="k_create_user_name"
								class="input_cx_title_150"  /></td>
							<th class="input_cx_title_th" style="width: 120px;">关键字或内容：</th>
							<td class="cx_title_td"><input type="text" class="input_cx_title_150"  id="k_key" name="k_key" /></td>
							...
							<td ><input value="重置" id="reset" type="button" onclick="clear111();" class="but_y_01" /></td>
						</tr>
					</table> <!-- 固定查询 end-->
				</td>
			</tr>
