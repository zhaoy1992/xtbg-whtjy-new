...
String isallowdelete = request.getParameter("isallowdelete");
	...
	if(rs){
		out.println("<script language='JavaScript'>window.alert('附件删除成功！');window.parent.location.href('filelist.jsp?buessid="+buessid+"')</script>");
	}else{
		out.println("<script language='JavaScript'>window.alert('附件删除失败！');window.close();</script>");
	}
	}catch(Exception e){
	//	e.printStackTrace();
		out.println("<script language='JavaScript'>window.alert('附件删除失败！');window.close();</script>");
