...
public class OsAppObjectServiceImpl implements OsAppObjectService {
	 ...
	 * 
	  ...
	  * @param objectBean
	  * @return 
	  * @see com.chinacreator.xtbg.tjy.officesupplies.service.OsAppObjectService#insertOrUpdateOsApp(com.chinacreator.xtbg.tjy.officesupplies.entity.OsAppObjectBean)
	 */
	 ...
	 * 
	  ...
	  * @param objectBean
	  * @return 
	  * @see com.chinacreator.xtbg.tjy.officesupplies.service.OsAppObjectService#onlyUpdateOsApp(com.chinacreator.xtbg.tjy.officesupplies.entity.OsAppObjectBean)
	 */
	...
	@Override
	public ResultMap<AResultCode, OsAppObjectBean> getCountData(
			OsAppObjectBean bean) {
		ResultMap<AResultCode, OsAppObjectBean> result = new ResultMap<AResultCode, OsAppObjectBean>();
		OsAppObjectDao dao = new OsAppObjectDaoImpl();
        Connection con = null;
        try {
            con = DbManager.getInstance().getConnection();
            OsAppObjectBean b = dao.getCountData(bean, con);
            if(b != null) {
	...
	public ResultMap<AResultCode, OsAppObjectBean> getCount(OsAppObjectBean bean) {
        ...
        try {
            con = DbManager.getInstance().getConnection();
            OsAppObjectBean b = dao.getCount(bean, con);
            if(b != null) {
                result.setCode(AResultCode.OK);
                result.setResult(b);
            } else {
	...
	public boolean changeItemState(OsAppObjectBean bean) {
		...
		try {
			...
			flag = dao.changeItemState(bean, conn);
			conn.commit();
		} catch (Exception e) {
			DbManager.rollBackNoThrow(conn);
			LOG.error(e.getMessage(), e);
