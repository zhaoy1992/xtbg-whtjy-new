...
<%
	...
	String tableName = "V_APP_PROCESS_MONITOR_LIST";
...
<html>
	...
	function handle(procInsId,flag){
      //  window.open("workflow_record_list.jsp?procInsId="+procInsId,"","height=630,width=850,status=yes,toolbar=no,menubar=no,location=no");
      window.open("monitor/ShowProcessInstanceMonitor.jsp?procInsId="+procInsId+"&flag="+flag,"","height=630,width=850,status=yes,toolbar=no,menubar=no,location=no");
	}
	function search(){
	...
	function orderList(ordercon) {
		...
		var url = "monitor_workflow_forapp_list.jsp?ordercondition=" + ordercon + orderType;
		document.location = url;
	}
</script>
	<%@ include file="/epp/css/cssControl.jsp"%><body class="contentbodymargin"  scrolling="no">
		<div id="contentborder" align="center">
			<FORM name="work" id="work" action="monitor_workflow_forapp_list.jsp" method="post">			
			<table width="100%" border="0" cellpadding="1" cellspacing="1"
				...
				<TABLE width="100%" cellpadding="1" cellspacing="1" bordercolor="#EEEEEE" class="thin">
					<pg:listdata dataInfo="com.chinacreator.commonworkflow.list.SHKMonitorWorkForAppList" keyName="SHKMonitorWorkForAppList" />
					<pg:pager maxPageItems="10" scope="request" data="SHKMonitorWorkForAppList" isList="false">
					<tr>
						<%
							...
							<tr class="labeltable_middle_tr_01" onMouseOver="this.className='mousestyle1'"
                             ...
                             <%String datas = dataSet.getString("remark1");
								...
								<%}%>
                                 ...
                                 <INPUT type="button" class="input" name="handle_ass" value="查看已运行"
                                  onclick="handle('<pg:cell colName="procInsId"/>','true')">
                                </td>
							</tr>
						</pg:list>
