...
public class GeneratorInfoJsp implements ClassGenerator {
	...
	public void init(ClassProperty classProperty) {
		...
		jspPageGenerator = new GeneratorInfoJspCode();
	}
	@Override
	public void before() {
		code.append("<%--\r\n");
		code.append("  Description："+classProperty.getTableChiName()+"详情页面\r\n");
	...
	public void after() {
		try {
			File file = new File(classProperty.getJspPath()+File.separator+
								...
								CodeConstant.getClassNameForJspInfo(classProperty)+".jsp");
