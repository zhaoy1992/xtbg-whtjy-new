...
<%
	...
	String infotypeHtml = DictDropDownList.buildSelect(sql,"ocname_name","","",true,"");//信息来源下拉框
%>
<title>易耗品设备领用</title>
<!-- 公共部分开始 -->
<!-- 引入jQuery -->
...
//tab2
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/commonListServlet?classNameId=collarSumStockList',  //commonListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['设备ID', '品名', '类别','规格',' 领用人','领用处室','领用数量','领用日期'],
		...
		{
			name: 'collarCon_userName',
			index: 'collar_username',
			width: 90
		},
		{
			name: 'collarCon_departName',
...
var toUserParamJosn = function() {
			var str = "{'collarCon_startTime':'"+$('#input_starttime').val()+"','collarCon_endTime':'"+$('#input_endtime').val()+"',"+
			"'conEq_type':'"+$('#ocname_name').val()+"',"+	
			"'collarCon_userName':'"+$('#collarCon_userName').val()+"',"+	
			"'conEq_name':'"+$('#equipment_name').val()+"',"+"'collarCon_departName':'"+$('#collar_depart').val()+"'}";
...
var addLevel = function(id) {
	openWindows("windowId","领用登记","../../officeassets/jsp/addCollarSumEqStock.jsp?widowsid=windowId",'officeSumEqStockList',false,window);
}
					...
					<!-- 隐藏查询 -->
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabs_search_ctable" id="hiddenArea" style="display:none">
							...
							<tr>
								...
								<th width="90" class="input_cx_title_th" >领用人：</th>
								<td width="296"  class="cx_title_td">
									<input class="input_cx_title_283" id="collarCon_userName" name="collarCon_userName" type="text" />
								</td>
