...
<%@ page
	import="com.chinacreator.xtbg.zhangjiajie.inforeported.dao.imploracle.InfoStaDaoImpl"%>
<%@page import="com.chinacreator.xtbg.pub.util.DaoImplClassUtil"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<%
	InfoStaDao infoStaDao = (InfoStaDaoImpl) DaoImplClassUtil
...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%=path%>' + '/commonListServlet?classNameId=infoStaDetailList',  //commonListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['上报单位','信息标题','上报时间','信息类型','采用类型'],
...
var toUserParamJosn = function() {
	var str = "{'username':'"+loginUserName+"','infotype_id':'"+$("#cbox2").val()+"',"+
			  "'org_name':'"+$('#org_name').val()+"','org_id':'"+orgID+"',"+"'status':'"+$("#status").val()+"',"+
...
<table width="100%" class="cQueryTableArea" id="queryTable">
  <tr>
    <td width="10%" class="tableLable">上报单位</td>
    <td width="28%" class="tableValue"><INPUT class="cText_out"
			style="width: 80%" id="org_name" name="org_name" value="" readOnly
			size=13 >
   </td>
    <td width="15%"  class="tableLable">业务类型</td>
    <TD width="25%" class="tableValue"><%=getTypeSelectHtml%></TD>
    <td width="27%" ></td>
  </tr>
  <tr>
    <td class="tableLable">上报时间</td>
    <td class="tableValue"><input type="text" id="startSlTime"
			name="startSlTime" class="input_date Wdate" onclick="WdatePicker()"/>
    <span class="tableLable">至
    <input type="text" id="endSlTime"
			name="endSlTime" class="input_date Wdate" onclick="WdatePicker()"/>
    </span></td>
    <td class="tableLable"> 采用状态 </td>
    <td class="tableValue"><select id="status" style='width:130px;margin-left:5px;'>
        <option value="all">所有类型</option>
        <option value="used">已采用</option>
        <option value="unused">未采用</option>
      </select></td>
    <td></td>
  </tr>
  <!-- 查询功能区 cQueryFunArea(查询区样式), cButton(按钮样式)-->
  <tr class="cQueryFunArea">
    <td colspan="5"><input class="cButton" type="button" id="sel"
			value="统计" onClick="userSearch()" />
      <input class="cButton"
			type="button" id="resert" value="重置" onClick="clearSearch()" /></td>
  </tr>
  <!-- 查询功能区 -->
</table>
<div class="cGridArea">
