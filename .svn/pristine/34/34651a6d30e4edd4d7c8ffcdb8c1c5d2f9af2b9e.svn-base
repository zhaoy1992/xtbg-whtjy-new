...
<%
	...
	String location_org_id = StringUtil.deNull(request.getParameter("location_org_id"));
...
//tab2
	$("#gridTable").jqGrid({
		...
		colNames: ['主键ID','所在地','编号','名称','类别','型号规格','领用部门','领用人','领用时间','原值','财政编号','状态'],
...
var toUserParamJosn = function() {
	var str = "{'name':'"+StringCheck($("#name").val())+"',"+
				"'num':'"+StringCheck($("#num").val())+"','type_id':'<%=type_id%>',"+
				"'show_baofei':'"+$("#show_baofei").val()+"',"+
				"'it':'"+StringCheck($("#it").val())+
		"','classify':'<%=classify%>','location_org_id':'<%=location_org_id%>'}";
...
var updateLevel = function(id){
	...
	var url = "<%=path%>/ccapp/xtbg/tjy/account/jsp/officedetailinfo.jsp?windowsId=windowsId1&classify=<%=classify%>&action=read&id="+model.id;
	var pageName ="更新台帐界面";
