...
function queryWs(){
    ...
    var sql = PunishInsConfig.queryInsConfigByName(buId, wsMc);
    wsDataset.Open(sql);
    reloadExtGrid();
}
...
function chooseWs(){
    ...
    if(ids.length==0){    
       alert("请选择要执法主体");
        ...
        wsObj.rfl_id = ids[0];
        wsObj.rfl_name = wsName[0];
        wsObj.rfl_brief_name = wsBrief[0];
        wsObj.mb_id = mb_id[0];
         ...
         <TD>
            <TABLE class=Ctable style="POSITION: static" cellSpacing=0 cellPadding=0 width="100%" align=left border=0>
               <TBODY>
                  <TR>
                     <TD class=c2>文书名称</TD>
                     <TD width="90%"><INPUT class=input_out id=Ws_MC style="LEFT: 0px; WIDTH: 47.85%; POSITION: static; TOP: 0px; HEIGHT: 20px" onmovestart=moveStart() size=9 controltype="text"></TD>
                  </TR>
                  <TR>
...
**/
cm.setHidden(1,true);
cm.setHidden(4,true);ExtGrid2.render();
