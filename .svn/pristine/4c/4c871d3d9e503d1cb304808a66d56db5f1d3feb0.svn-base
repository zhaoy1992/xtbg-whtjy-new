...
{
	var url = 'excel.jsp?title_id=<%=request.getParameter("titleid")%>';
	...
	<table bgcolor="#336699" border="0" cellPadding="0" cellSpacing="0" width="100%">
		...
		<table border="1"width="100%" bordercolor="#F0F1EF" cellSpacing="0" >
			...
			for (int j = 0; j < qusetionid.length; j++) {
				...
				voteTotal = vote.getTotalByQid(id);
					...
					%>
                        <tr>
			               <td class=c1 style="FONT-FAMILY: MS Sans Serif; FONT-WEIGHT: bold; top:0 width=31 color=#000000">
							...
							<%
								float sub_count = ((Item) items.get(i)).getCount();
								float total_count = voteTotal;
							%>
							<td nowrap><%=Math.round((sub_count*100)/(total_count))%>%</td>
						</tr>
						<%}%>
          </table>
        </td>
