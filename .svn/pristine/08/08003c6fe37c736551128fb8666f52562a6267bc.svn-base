...
<%
	...
	String sql = "select i.SUBMITTEDINFO_TYPECODE, i.SUBMITTEDINFO_TYPENAME from ta_oa_infre_submittedinfo_type"+
	" i where i.SUBMITTEDINFO_SOURCE = '"+InfoSource.INFORMATION_COLLECTION+"' "+ 
	" and i.SUBMITTEDINFO_TYPECODE in ('"+InfoType.HAS_BE_SAVEINFO+"','"+InfoType.HAS_BE_SUBMITTED+"','"+InfoType.HAS_BE_REJECTION+"','"+InfoType.HAS_BE_AUDIT+"','"+InfoType.HAS_BE_PUBLISHED+"')";
...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/commonListServlet?classNameId=infoReportAllShowList',  //commonListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['报送id', '标题' , '上报时间', '录入者', '送审接收人','发布接收人','上报单位', '信息状态', '信息状态code'],
...
var editPerson = function(id) {
	//+'&operwindowid=sub_infoviewforpublished'+model.submittedinfo_id
	var model = jQuery("#gridTable").jqGrid('getRowData', id);
	var url = "../../inforeported/jsp/infoviewforpublished.jsp?submittedinfoId="+model.submittedinfo_id+"&state="+model.submittedinfo_typecode+"&comelist=allshow"+'&operwindowid=sub_infoviewforpublished'+model.submittedinfo_id;
	openWindows('sub_infoviewforpublished'+model.submittedinfo_id,'信息采编已办',url,'infocaibian',false,window);
	//location.href = url;
}

var addInfo = function(){
	var url = "infoinputdetail4allshow.jsp";
	location.href = url;
}
...
<table width="100%" class="cQueryTableArea" id="queryTable">
	<tr>
		...
		<td width="15%" class="tableLable">
			上报单位		</td>
		<TD width="25%" class="tableValue" ><INPUT class="cText_out" style="width:80%"
	...
	<tr>
		...
		<td class="tableValue"><input class="input_date Wdate" id="input_starttime" name="input_starttime" type="text" onclick="WdatePicker()"/> 至
		<input class="input_date Wdate" id="input_endtime" name="input_endtime" type="text" onclick="WdatePicker()"/></td>
		<td class="tableLable">信息状态</td>
