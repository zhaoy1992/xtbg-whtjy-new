...
public class GeneratorDoJsp implements ClassGenerator {
	...
	public void init(ClassProperty classProperty) {
		...
		jspPageGenerator = new GeneratorDoJspCode();
	}
	@Override
	public void before() {
		code.append("<%--\r\n");
		code.append("  Description："+classProperty.getTableChiName()+"action页面\r\n");
	...
	public void after() {
		try {
			File file = new File(classProperty.getJspPath()+File.separator+
								...
								CodeConstant.getClassNameForJspDo(classProperty)+".jsp");
