...
<%@page import="com.chinacreator.config.ConfigManager"%><%@ include file="../../../sysmanager/include/global1.jsp"%>
...
<SCRIPT language="javascript">
	...
	function dealRecord(dealType) {
	    ...
	    if (checks != ""){
	    	if (dealType==1){
	    		outMsg = "你确定要将所选用户的权限复制给用户( <%=userName%> )吗？";
	        	if (confirm(outMsg)){
	        		orgForm.action="selfPurviewCopyUser.jsp?userId=<%=uid%>&checks="+checks+"&userSelf="+userSelf+"&userRole="
	        				+ userRole+"&flag=2&userJob="+userJob+"&curOrgId=<%=curOrgId%>&orgId=<%=orgId%>&userOrgId=<%=userOrgId%>";
					orgForm.submit();
		 			return true;
			    }
			} 
...
<%@ include file="/epp/css/cssControl.jsp"%><body  class="contentbodymargin" scroll="no">
	<fieldset>
	   <LEGEND align=left><strong><FONT size=2>提示</FONT></strong></LEGEND>
	    ...
	    <table><tr><td>将所选用户的自身权限和所属角色权限复制给当前用户
	      </td>
	     </tr>
	    </table>
...
<div id="contentborder" align="center">
	...
	<tr><td height='40' class="detailtitle" align=center colspan=3><b>复制所选用户的权限给：<%=userName%> </b></td></tr>
