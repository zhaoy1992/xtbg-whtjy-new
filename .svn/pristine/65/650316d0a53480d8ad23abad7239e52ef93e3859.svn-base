...
$(function() {
	$("#gridTable").jqGrid({
		url: '<%=path%>' + '/commonListServlet?classNameId=commonToDoWorkList', //commonListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
					...
					//filterOptions:[[["会议通知","会议通知"]]],//alue text 后的值与显不
	...
	*/
	...
	var toUserParamJosn = function() { 
		var str = "{'listtype':'waitingapproval','inputuser_id':'<%=userid%>','info_type':'会议通知'}";
	...
	var showDetailInfo = function(id){
		...
		var url = '../../conference/jsp/showconferencedetail2.jsp?height=100&showState=' + state + '&conSn=' + model.pri_id + '&operType=3&widowsid=sub_myreceivenotice_tab1_chakan'+model.sn+'&type='+<%=type%>;
		 $("#titleids3",window.parent.document).html(parseInt(<%=mettingTotal %>));
		// alert(parseInt(<%=mettingTotal %>));
		 $("#titleids3",window.parent.document).html(parseInt($("#titleids3",window.parent.document).html())+")");
		 window.top.$("#workCount").html(parseInt(window.top.$("#workCount").html()));
		 window.top.getIFrameDOM("_document_body_mian_firstPagerAAAS").location.reload();
		openWindows("sub_myreceivenotice_tab1_chakan"+model.sn,"会议接收查看",url,'my_receive_notice',false,window);
		...
		// $("#titleids3",window.parent.document).html(parseInt($("#titleids3",window.parent.document).html()-1)+")");
		 
		//window.top.$("#workCount").html(parseInt(window.top.$("#workCount").html())-1);
		 /* window.top.$("#workCount").html(parseInt(window.top.$("#workCount").html())-1);
		 $("#mettingTotal",window.top.getIFrameDOM("_document_body_mian_firstPagerAAAS")).html(parseInt($("#mettingTotal",window.top.getIFrameDOM("_document_body_mian_firstPagerAAAS")).html())-1);
		  */
		//window.top.getIFrameDOM("_document_body_mian_firstPagerAAAS").location.reload();
	}
