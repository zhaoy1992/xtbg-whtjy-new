		...
		$("#tabs").tabs({//每个页面只刷新一次
			select : function(event, ui) {
				...
				if ($(iframeitem).attr("id")=="frame2" && $(iframeitem).attr("src")==""){
					$(iframeitem).attr("src", "meetingviewlist.jsp?timesupflag=yes&m_check_flag=<%=MeetingConstants.STATUS_PUBLISHED %>");//过期的
				}
			}
		});
				...
				<div>
					<div id="tabs">
						<div id="titleids" style="display:none;" id="_top">
							<ul>
								<li><a href="#tabs-1">未过期会议</a></li>
								<li><a href="#tabs-2">已过期会议</a></li>
							</ul>
						</div>
						<div id="tabs-1">
							<iframe src="meetingviewlist.jsp?timesupflag=&m_check_flag=<%=MeetingConstants.STATUS_PUBLISHED %>" id="frame1" frameborder="0"
								width="100%"></iframe>
						</div>
						<div id="tabs-2">
