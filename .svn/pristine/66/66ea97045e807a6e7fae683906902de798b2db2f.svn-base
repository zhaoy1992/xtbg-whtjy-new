...
function saveres(){
		...
		document.forms[0].action="saveResOp.jsp?opType=new&restypeId=<%=restype%>";
		document.forms[0].submit();

}

function backRes(){
	window.location.href="<%=rootpath%>/sysmanager/resmanager/reslist.jsp?restypeId=<%=restype%>&restypeName=<%=restypeName%>";
}
</script>
</head>
