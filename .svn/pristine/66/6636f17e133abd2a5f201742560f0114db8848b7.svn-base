...
//tab2
	$("#gridTable").jqGrid({
		...
		colNames: ['主键ID','设备编号', '名称','使用部门', '保管人','规格型号','到货日期','设备库存状态','设备使用状态','制造厂家','生产日期','出厂编号','供应商名称','金额','基本参数','设备保管人ID'],
...
var toUserParamJosn = function() {
	...
	"','type':'" + $("#type").val() +
	"','dev_state':'<%=dev_state%>','dev_stockflag':'<%=dev_stockflag%>','make_users':'" + $("#make_users").val() + 
...
var addLevel = function(id) {
	...
	jQuery(selectedIds).each(function () {
		...
		if(!IsSpace(model.id)){
			...
			myinfo[0] = model.id;
			myinfo[1] = model.dev_code;
			myinfo[2] = model.dev_name;
