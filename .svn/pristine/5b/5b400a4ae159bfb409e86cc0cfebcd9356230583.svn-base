...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/pubListServlet?classNameId=com.chinacreator.xtbg.core.process.flowprocessing.list.ToBeWorkList',  //pubListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['实例id','业务id','环节实例id','ccformid','流程定义ID','办件编号 ', '公文标题 ','业务类型 ','业务类型CODE',  '发起人 ', '发起时间 ', '当前环节 ','操作', '表单','模板ID'],
		...
		gridComplete: function() { // var idss=jQuery("#gridTable").jqGrid('getColNames');
			...
			for (var i = 0; i < ids.length; i++) {
				var cl = ids[i];
				//特检院OA 2014-06-03 start 暂存列表追加删除流程功能
				var model = jQuery("#gridTable").jqGrid('getRowData', cl);
				be = "<input id='modify' type='button' class='but_y_01' onMouseOver=javascript:$(this).attr('class','but_y_02'); onMouseOut=javascript:$(this).attr('class','but_y_01'); onclick='delFlow("+cl+")' value = '删除' />";

				jQuery("#gridTable").jqGrid('setRowData', ids[i], {
					act: be
				});
				//特检院OA 2014-06-03 end 暂存列表追加删除流程功能
...
function handle(id){   
    ...
    var url = "<%=path %>"+ model.flow_form + "?" + "ins_id="+model.ins_id+"&busi_id="+model.busi_id+"&busiTypeCode="+model.busitype_code+"&actInsId="+model.actInsId+"&cc_form_instanceid="+model.cc_form_instanceid+"&def_id="+model.def_id+"&list_type=tobe_work&operType=nosend"
    +"&template_id="+model.template_id+"&Template=word"+model.template_id+"&from=zc";
