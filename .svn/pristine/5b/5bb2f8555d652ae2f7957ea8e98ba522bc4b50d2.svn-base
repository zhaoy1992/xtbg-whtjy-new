...
日期：2014-02-18
--%>
<%@page import="com.chinacreator.xtbg.tjy.device.entity.DevSupplier"%>
...
<%
	...
	if("addInfo".equals(action)){
		responseCode = "1";
		DevSupplier bean = new DevSupplier();
		bean = (DevSupplier)RequestBeanUtils.getValuesByRequest(request,bean);
		String beanId = service.insertOrUpdateDevSupplier(bean);
	...
	if("deleteInf".equals(action)){
		...
		if(deleteArr.length>0){
			flagStr = service.deleteDevSupplierByIds(deleteArr);
		}
