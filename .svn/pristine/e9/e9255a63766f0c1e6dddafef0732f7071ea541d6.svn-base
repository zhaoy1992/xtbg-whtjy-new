    ...
    if (isSelect){
    	if (dealType==1){
    		if(checks != ""){
				...
				if(winaddorg=="ok"){
					userList.action = window.location.href;
...
function userInfo(userId){ 
	...
	var winbasic;
	var url="userInfo_lisan.jsp?userId="+userId;
				...
				<form name="userList" method="post" >
					<table cellspacing="1" cellpadding="0" border="0" width=98%  class="thin">
						...
						<pg:pager maxPageItems="15" scope="request" data="DiscreteUserList" isList="false">
							...
							<tr height="18px" class="labeltable_middle_tr_01">
								<td id="test11" colspan=6 align='center'>
									每页15条记录 共<pg:rowcount/>条记录 <pg:index custom="true"/>
								</td>
							</tr>
							<tr>
					            ...
					            {%>
								<input type="submit" value="删除" name="deletebutton" class="input" onclick="javascript:dealRecord(1); return false;">
								<%}%>
								<%
								 if (accessControl.checkPermission(resId,
					                         "lisanusertoorg", AccessControl.ORGUNIT_RESOURCE))
					            {%>
								<input type="submit" value="用户调入机构" name="deletebutton" class="input" onclick="javascript:addorg(1); return false;">
								<%}%>
								
								<input type="button" value="关闭窗口" class="input" onclick="window.close(); return false;">
