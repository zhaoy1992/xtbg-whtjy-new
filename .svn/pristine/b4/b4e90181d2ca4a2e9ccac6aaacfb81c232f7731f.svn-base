...
$(function() {
	...
	$("#gridTable").jqGrid({
		url: '<%= path %>' + '/pubListServlet?classNameId=com.chinacreator.xtbg.tjy.device.list.DevAppLicationList',  //pubListServlet列表公共的servlet,classNameId在servletBean.xml文件中配置要调用的类,isPage:是否翻页(true,false),默认为true
		...
		colNames: ['采购id','附件id','正文id','机构ID','申请标题 ','机构名称','填报时间'],
...
function removeWindws(){
	removeAlertWindows('windowIds1',true,true,'',false,'','');
}

function handle(id){   
    var model = jQuery("#gridTable").jqGrid('getRowData', id);
    var path ="<%=path%>/ccapp/xtbg/tjy/device/jsp/devappitemlist.jsp?windowsId=windowIds2&item_plainid="+model.t_id;
	openAlertWindows('windowIds2',path,'采购物品',810,400,'25%','25%');
}
