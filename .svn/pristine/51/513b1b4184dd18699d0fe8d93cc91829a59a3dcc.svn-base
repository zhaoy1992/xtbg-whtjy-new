...
%>
	...
	var setting = {
		...
		async: {
	    	enable: true,//启用异步加载
	    	url: window.top.getContextPath() + '/commonTreeServlet?classNameId=com.chinacreator.xtbg.core.wordnumdef.dao.impl.WordNumTree', 
			autoParam:["number_type", "name=number_type", "level=lv"],
	    ...
	    */
		...
		function onclick(event,treeId,treeNode){
			if(2===treeNode.level){
				var url="wordnuminfo.jsp?number_type="+_pub.lang.zwzm(treeNode.id)+"&number_year="+treeNode.parent_id;
