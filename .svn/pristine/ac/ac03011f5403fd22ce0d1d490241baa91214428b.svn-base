...
var addFileDocDetail = function () {
    ...
    openAlertWindows('addFileDocDetailWindow', getCurrentFilePath() 
    	    + 'filedocdetail4disk.jsp?isgd=yes&busiTypeCode=wj-001&windowId=addFileDocDetailWindow'+typeurl, '添加文件' ,1020,530,'10%','10%');
}
...
var updateFileDocDetail = function(f_doc_id) {
	  ...
	  openAlertWindows('updateFileDocDetailWindow',
   	       getCurrentFilePath() + 'filedocdetail4disk.jsp?isgd=yes&busi_id='
...
var addDocToFiles = function(){
    ...
    urlArray.push(getCurrentFilePath()+'filedoclist4addtofiledisk.jsp?windowId=addDocToFiles');
    //调用父页面的函数
...
function exportExcelList(){
    var exportKey = [ "f_doc_num","f_doc_name","f_file_formtime4export","f_burn_user_id"
                      ,"f_secrecy_name","f_secrecy_name4export"];   
    var exportHeader = ["件号","    光 盘 名 称       ","刻录日期 ","刻录人","密级"," 备   注 "];
    var data = {
        exportKey : exportKey.join(","),
        exportHeader : descape(escape(exportHeader.join(","))),
        exportTitle : descape(escape("光盘档案卷内目录"))
    };
    exportExcel("exportDocList",data);
}
/**
 * 导出卷内文件标签
 */
function exportExcelDetail(){
    var exportKey = [ "f_fondsno","f_listno","f_typeno",
          "f_filesno","f_doc_num","f_doc_name","f_burn_user_id","f_file_formtime4export"];
   
    var exportHeader = ["全宗号","目录号","分类号","卷  号","盘  号","光    盘     名    称","刻录人","刻  录日  期"];
    var data = {
        exportKey : exportKey.join(","),
        exportHeader : descape(escape(exportHeader.join(",")))
    };
    exportExcel("exportDiskDetail",data);
}
...
<form action="" id="form1" name="form1" method="post" >
     <div class="content_02_top" style="margin-bottom: 10px;float: none;border: #ebebeb 1px solid;padding-right: 10px;">
           ...
           <input onclick="exportExcelDetail();" name="exportExcelDetailButton" id="exportExcelDetailButton"  value="导出光盘标签" type="button" class="but_y_03" style="float: right;width:105px;"/>
