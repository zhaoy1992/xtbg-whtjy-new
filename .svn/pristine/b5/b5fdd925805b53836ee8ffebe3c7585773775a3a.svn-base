...
<%@page import="com.chinacreator.message.util.MsgConfig"%><%@ include file="../include/global1.jsp"%>
...
<html>
	<head>
		...
		function addMessageResource() {
			var retx=window.showModalDialog("<%=request.getContextPath()%>/sysmanager/message/addmessageresource.jsp",window);
			if(retx=='refresh'){
		...
		function editMessageResource(r_id) {
			...
			var retx=window.showModalDialog("<%=request.getContextPath()%>/sysmanager/message/addmessageresource.jsp?action=edit&r_id="+r_id,window);
			if(retx=='refresh'){
				window.location.reload();
			}
		}
		...
		<div id="contentborder" align="center">
			<form name="MessageResourceForm" id="MessageResourceForm" action="messageresourcelist.jsp" method="post">
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="1"
					...
					<pg:listdata
						dataInfo="com.chinacreator.epp.message.menu.MessageResourceList"
