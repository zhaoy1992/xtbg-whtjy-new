			...
			<tr>
				...
				<div id="vcenter" style="float: left; width: 100%; overflow-x: hidden;" scrolling="yes">
					<div style="float: left; width: 90%;">
								...
								<table border="0" cellspacing="0" cellpadding="0"
									...
									<tr>
										...
										<td class="content_02_box_div_table_td" colspan="3">
											<oa:select column="P_TYPES" table="TA_OA_ANSWERS_INFO"
											optionsql="select P_TYPES,typemc from (
													select '1' as  rowcont, '综合管理类' as P_TYPES, '综合管理类' as typemc from dual
													union
													select '2' as rowcont, '技术类' as P_TYPES, '技术类' as typemc from dual ) 
													order by rowcont" style="width: 200px;"
									...
									<tr>
										...
										<td class="content_02_box_div_table_td" colspan="3">
											...
											<oa:input type="hidden" column="ISPUBLISH" table="TA_OA_ANSWERS_INFO" />
											<oa:date column="PUBLISH_TIME" dateFmt="yyyy-MM-dd HH:mm:ss" table="TA_OA_ANSWERS_INFO"/>
										</td>
									</tr>
									<tr>
						...
						<!--审核意见DIV-->
						 <div class="content_02_box" style="width: 100%;"  >
							 <div class="content_02_box_title_bg"  style="width: 100%;"><span>审核</span>
							 </div>
							 <div style="width: 100%;height: 220px;">
								<oa:view column="AUDIT_IDEA" table="TA_OA_ANSWERS_INFO"
											style='width:99%;height: 170px;overflow: auto;' />
							</div>
						</div>
						<!--审核意见DIV END-->
					</div>
...
<script type="text/javascript">
	...
	*/
	var init = function(){
		//隐藏发布时间控件
		jQuery("#dis_TA_OA_ANSWERS_INFO__PUBLISH_TIME").hide();
	}
	...
	*/
	...
	var afterActionSuccessFn=function(){
		...
		if("<%=widowsid%>"!=""){
			...
			//跳转
			window.location.href = path + "/ccapp/xtbg/tjy/answers/jsp/mainAnswers.jsp";
		}
	...
	/*
 	函数:  doCompleteActivity
 	说明:  最后一个环节的特殊处理 
 	参数:   选中的环节
 	返回值: 无
 	*/
 	function doCompleteActivity(action_defid){
 		//自动发布环节 改变 "发布状态"和填充"发布时间"
 		if(!IsSpace(action_defid)&&action_defid=="ywbd_wp1_act10"){
 			jQuery("#TA_OA_ANSWERS_INFO__ISPUBLISH").val("1");
			jQuery("#TA_OA_ANSWERS_INFO__PUBLISH_TIME").val("<%=DateUtil.getCurrentDateTime()%>");
 		} 
 	}
