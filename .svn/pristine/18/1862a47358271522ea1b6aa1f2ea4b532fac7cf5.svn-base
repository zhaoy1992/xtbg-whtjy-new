...
function commandBut2(){
    if(flags==0){  //从菜单进去
       ...
       var num1 = Diagnoses.findCountNumBiz(wft_id);//判断在受理之后调查取证之前
             if(num1>0){
              num4 = Diagnoses.findCountBYLABiz(wft_id);//判断是否是不予立案或移送
                if(num4>0){
                    //当前界面只读，只留下返回按钮可以操作
                       $("button1").style.display = "none" ;
                    $("button2").style.display = "none" ;
      ...
      <TR>
         <TD>
            <TABLE class=Ctable cellSpacing=0 cellPadding=0 width="100%" border=0>
               <TBODY>
                  <TR>
                     <TD class=taobox width="50%">证据信息</TD>
                  </TR>
               </TBODY>
            </TABLE>
         </TD>
