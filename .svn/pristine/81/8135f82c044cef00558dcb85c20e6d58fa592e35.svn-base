...
<%
	...
	String tableName = "V_APP_TOBEWORK_LIST";
...
<html>
	...
	function handle(actInsId,busiId,dbId,DYNAMICPERFORMER,next_acts,andsplitusers,moduleId,userName){
        if(busiId == "")
        {
           alert("流程未绑定表单，不能进行操作！");
           return;
        }
		document.location = "handle_tobe_work.jsp?actInsId="+actInsId+"&busiId="+busiId+"&tobeWorkDBId="+dbId+"&DYNAMICPERFORMER="+DYNAMICPERFORMER+"&next_acts="+next_acts+"&andsplitusers="+andsplitusers+"&moduleId="+moduleId+"&userName="+userName;
	}
	...
	function orderList(ordercon) {
		...
		var url = "tobe_work_list.jsp?ordercondition=" + ordercon + orderType;
	...
	<%@ include file="/epp/css/cssControl.jsp"%><body class="contentbodymargin"  scrolling="no">
		<div id="contentborder" align="center">
			<FORM name="work" id="work" action="tobe_work_list.jsp" method="post" onsubmit="validateForm(this)">			
				...
				<TABLE width="100%" cellpadding="1" cellspacing="1" bordercolor="#EEEEEE" class="thin">
					<pg:listdata dataInfo="com.chinacreator.commonworkflow.list.SHKTobeWorkList" keyName="SHKTobeWorkList" />
					<pg:pager maxPageItems="10" scope="request" data="SHKTobeWorkList" isList="false">
						<tr>
						<%
						...
						<%
						 ...
						 %>
							<tr class="labeltable_middle_tr_01" onMouseOver="this.className='mousestyle1'"
							 onMouseOut="this.className= 'mousestyle2'" ondblclick="handle('<pg:cell colName="actInsId"/>','<pg:cell colName="busiId"/>','<pg:cell colName="dbId"/>','<pg:cell colName="DYNAMICPERFORMER"/>','<pg:cell colName="next_act"/>','<pg:cell colName="andsplitusers"/>','<%=moduleId%>','<pg:cell colName="curUserName"/>')">
								...
								<td class="tablecells" width="15%" nowrap="nowrap">
									<INPUT type="button" class="input" name="handle_ass" value="处理任务"
									 onclick="handle('<pg:cell colName="actInsId"/>','<pg:cell colName="busiId"/>','<pg:cell colName="dbId"/>','<pg:cell colName="DYNAMICPERFORMER"/>','<pg:cell colName="next_act"/>','<pg:cell colName="andsplitusers"/>','<%=moduleId%>','<pg:cell colName="curUserName"/>')">
								</td>
							</tr>
						</pg:list>
