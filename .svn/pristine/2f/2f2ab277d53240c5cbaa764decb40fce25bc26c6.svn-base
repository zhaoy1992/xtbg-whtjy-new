		...
		$("#tabs").tabs({//每个页面只刷新一次
			select : function(event, ui) {
				...
				if ($(iframeitem).attr("id")=="frame2"){
					$(iframeitem).attr("src", "paperlist4draft.jsp");//草稿箱
				}else{
					$(iframeitem).attr("src", "paperlist4reported.jsp");//已上报
				}
			}
		});
				...
				<div>
					<div id="tabs">
						<div id="titleids" style="display:none;" id="_top">
							<ul>
								<li><a href="#tabs-1">已上报信息</a></li>
								<li><a href="#tabs-2">草稿箱</a></li>
							</ul>
						</div>
						<div id="tabs-1">
							<iframe src="paperlist4reported.jsp" id="frame1" frameborder="0"
								width="100%"></iframe>
						</div>
						<div id="tabs-2">
