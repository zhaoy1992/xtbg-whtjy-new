...
/**
 * 
 *<p>Title:FlowStaticTHreadDate.java</p>
 *<p>Description:工作流临时静态线程数据,业务待办中使用</p>
...
public class FlowStaticTHreadData {
	 ...
	 * 流程定义id
	 */
	private static Map<Long, String> DefId = new HashMap<Long, String>();
	/**
	 * 是否暂存
	 */
	 ...
	 * @param def_id
	 */
	public static void setDefId(String def_id) {
		DefId.put(Thread.currentThread().getId(), def_id);
	}
	
	public static String getDefId() {
		if(DefId.containsKey(Thread.currentThread().getId())){
			String def_id = DefId.get(Thread.currentThread().getId());
			return def_id == null ? "" : def_id;
		}else{
			return "";
		}
	}
