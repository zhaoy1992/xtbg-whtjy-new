...
<%
	...
	int showNum = 10;//一次显示页数
	int index = NumberUtils.toInt(request.getParameter("index"), 0);
...
<head>
	...
	<script language="javascript">
		...
		function toNextIndex(){
			var showNum = "<%=showNum%>";
			var currentIndex = parseInt($("index").value)+1;
			$("start").value=currentIndex*showNum+1;
...
<div style="width:100%;text-align:left;">
  <%if(index>0){ %>
 	<a href="javascript:toUpIndex()">&lt;&lt;</a>
 <%} %>
<%
	int totalPage = resultTotal / pageSize + (resultTotal % pageSize>0? 1: 0);
	for(int i= index*showNum+1;i<=totalPage;i++){
		if(i-index*showNum>=showNum+1) break;
		if(startPage!=i){
 ...
 %>
  ...
  <%if(totalPage-(index+1)*showNum>0){ %>
 	<a href="javascript:toNextIndex()">&gt;&gt;</a>
 <%} %>
 </div>
 <!-- 翻页查询表单 -->
