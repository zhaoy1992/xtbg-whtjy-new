					...
					<pg:pager maxPageItems="10" scope="request" data="FormTemplateList"
						...
						<tr height="30" class="labeltable_middle_tr_01">
							<td colspan=100 align='right'>
								...
								<%if(showDelete.equals("true"))
								 {
								   //如果选择了具体的业务类别才允许新增，否则只显示删除按钮(此处用eform_class_name而不用eform_class,后者可能取值为null,长度为4)
								   if(eform_class_name != null)
								   {
								%>
									<input name="newTemplate" type="button" class="input" value="新增" onclick="creatTemplate('<%=eform_class%>')"/>
								<%
								   }
								%>
									<input name="delete" type="button" class="input" value="删除" onclick="deleteTemplate()"/>						
								<%}
								  else
								  {
								%>
