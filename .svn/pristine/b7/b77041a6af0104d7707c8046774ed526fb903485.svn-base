document.write("<div id=meizzCalendarLayer style='position: absolute; z-index: 9999; width: 144; height: 223; display: none'>");
document.write("<iframe name=meizzCalendarIframe scrolling=no frameborder=0 width=100% height=100%></iframe></div>");
function writeIframe()
{
    var strIframe = "<html><head><meta http-equiv='Content-Type' content='text/html; charset=gb2312'><style>"+
	".myButton {height: 10px;width: 15px;font-size: 4px;}"+
	...
	"var objTime,intMax;"+
	"function timeAdd(obj0){if(objTime==null) {objTime=document.all(\"hour\");intMax=23;};var i=parseInt(objTime.value);if(i<intMax)objTime.value=i+1;objTime.select();obj0.focus();}"+
	"function timeRid(obj0){if(objTime==null) {objTime=document.all(\"hour\");intMax=23;};var i=parseInt(objTime.value);if(i>0)objTime.value=i-1;objTime.select();obj0.focus();}"+
    "function onTimeBlur(obj,num,maxnum,next){var strTemp=obj.value;if(isNaN(strTemp)){obj.value=strTemp.substring(0,strTemp.length-1);}else{var intTemp=parseInt(strTemp);if(intTemp>maxnum)obj.value=strTemp.substring(0,strTemp.length-1);else if(intTemp>num){}}}"+
    ...
    "<tr><td><table><tr><td width=\"150\">时间:"+
    "<input name=\"hour\" id=\"hour\" onFocus=\"this.select();objTime=this;intMax=23\" value=\""+WebCalendar.thisHour+"\" size=\"2\" maxlength=\"2\" onblur=\"onTimeBlur(this,2,23,minute)\">:"+
    "<input name=\"minute\" id=\"minute\" onFocus=\"this.select();objTime=this;intMax=59\" value=\""+WebCalendar.thisMinute+"\" size=\"2\" maxlength=\"2\" onblur=\"onTimeBlur(this,2,59,second)\">:"+
    "<input name=\"second\" id=\"second\" onFocus=\"this.select();objTime=this;intMax=59\" value=\""+WebCalendar.thisSecond+"\" size=\"2\" maxlength=\"2\" onblur=\"onTimeBlur(this,2,59,null)\">"+
    "<td width=\"25\"><input name=\"timeup\" type=\"button\" class=\"myButton\" value=\"◆\" onClick=\"timeAdd(this)\">"+
    "<input name=\"timedown\" type=\"button\" class=\"myButton\" value=\"◆\" onClick=\"timeRid(this)\"></td></tr>"+
...
{	
	...
	this.intDate    = new Date();               //当前时间
    this.thisYear   = this.intDate.getFullYear(); //定义年的变量的初始值
    this.thisMonth  = this.intDate.getMonth()+ 1; //定义月的变量的初始值
    this.thisDay    = this.intDate.getDate();     //定义日的变量的初始值
    this.thisHour   = this.intDate.getHours();     //定义小时的变量的初始值
    this.thisMinute = this.intDate.getMinutes();     //定义分钟的变量的初始值
    this.thisSecond = this.intDate.getSeconds();     //定义秒的变量的初始值
    ...
    this.darkColor  = "#85AAEF";//"#0000D0";    //控件的暗色
...
function returnDate() //根据日期格式等返回用户选定的日期
{
    if(WebCalendar.objExport)
    {
		...
		if(WebCalendar.timeShow) {
            returnValue += " " + appendZero(WebCalendar.iframe.document.all("hour").value)+":"+appendZero(WebCalendar.iframe.document.all("minute").value)+":"+appendZero(WebCalendar.iframe.document.all("second").value);
        }
...
function CompareDate(date1,date2)
{
   ...
   try
   {
       t = Date.parse(ConvertDateFormat(date1))-Date.parse(ConvertDateFormat(date2));
       if (t>=0)
         return true;
       else
         return false;
...
{
   var newDate = oldDate.substr(5,5)+"-"+oldDate.substr(0,4)+" "+oldDate.substr(11);
   return newDate;
}
