    ...
    <%if(true){//财政厅
    	...
    	if(flowInfoBeanList!=null&&flowInfoBeanList.size()>0){
    		...
    		for(FlowInfoBean flowInfoBean : flowInfoBeanList){
				...
				//文件上报和信息发布、会议申请流程固化到菜单项中,在这里不展示 FYWJSB XXFBSP hysqsp
				if( !"FYWJSB".equals(flowInfoBean.getBusitype_code()) && !"XXFBSP".equals(flowInfoBean.getBusitype_code()) && !"hysqsp".equals(flowInfoBean.getBusitype_code())){
