...
public class AddOrUpdateOsAppRegObject extends XtDbBaseDao implements FlowBusiDataSaveDao{
	...
	public boolean saveBusiInfo(HttpServletRequest request, Connection conn)
		...
		//物品分类id
	    //String[] P_TYPE_ID_Arr = request.getParameterValues("PTYPE_ID");
	    
		//物品分类名
		//String[] P_TYPE_NAME_Arr = request.getParameterValues("PTYPE_NAME");
		...
		//相关计划表IDs
		String osAppIds = request.getParameter("TA_OA_OSAPPREGYHP__T_APPREGYHP_APPIDS");
		...
		//填写人ID
		String user_id = request.getParameter("TA_OA_OSAPPREGYHP__T_APPREGYHP_USER_ID");
		
		//填写人名称
		String user_name = request.getParameter("TA_OA_OSAPPREGYHP__T_APPREGYHP_USER_NAME");
		
		//领用日期
		String lyDate = request.getParameter("TA_OA_OSAPPREGYHP__T_APPREGYHP_DATE");
		...
		if (!StringUtil.isBlank(busi_id)) {
			...
			for (int i = 0; i < T_OSAPPREG_OB_NO_Arr.length; i++) {
				if ((!T_OSAPPREG_OB_NO_Arr[i].equals("0")) && (!StringUtil.isBlank(T_OSAPPREG_OB_NO_Arr[i])) && (!StringUtil.isBlank(IT_ID_VALUE_Arr[i]))) {
					...
					//osAppRegObjectBean.setP_type_id(P_TYPE_ID_Arr[i]);
					//osAppRegObjectBean.setP_type_name(P_TYPE_NAME_Arr[i]);
					...
					osAppRegObjectBean.setOfustype_name(OFUSTYPE_NAME_VALUE_Arr[i]);
					...
					osAppRegObjectBean.setOrg_id(org_id);
					osAppRegObjectBean.setOrg_name(org_name);
					...
					if ("yhply_wp1_act2".equals(action_defid)) {
						...
						//osHavaLYObjectBean.setP_type_id(P_TYPE_ID_Arr[i]);
						//osHavaLYObjectBean.setP_type_name(P_TYPE_NAME_Arr[i]);
