 ...
 *
 */
public class OsHavaLyOrgExcelToData {
	public static final Logger LOG = Logger
			.getLogger(OsHavaLyOrgExcelToData.class.getName());
	private String exportTemplateFilePath = FileUtil.getAppContextPath()+ CacheCommonManageProxy.getSystemParam("bgyplybm_excelTemplateFilePath");//导出模版所在路径
	...
	* @return
	 */
	public String exportExcelToData(List<OsAppRegObjectBean> list){
		try{
			...
			poiExcelUtil.write(1,2,list.get(0).getOrg_name());
			
			int count = 3;
			for(int i=0;i<list.size();i++){
				OsAppRegObjectBean bean = list.get(i);
				if(!StringUtil.nullOrBlank(bean.getOfustype_name())){
					count++;
					String[] values = new String[5];
					values[0] = Integer.toString(count - 3);
					values[1] = bean.getOfustype_name();
					values[2] = "个";
					values[3] = bean.getT_osappreg_ob_no();
					values[4] = bean.getIt_name();
