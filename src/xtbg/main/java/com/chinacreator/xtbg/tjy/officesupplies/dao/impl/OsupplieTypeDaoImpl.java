...
import com.chinacreator.xtbg.tjy.officesupplies.dao.OsupplieTypeDao;
...
public class OsupplieTypeDaoImpl extends XtDbBaseDao implements OsupplieTypeDao{
	...
	public PagingBean selOsupplieTypeList(String parmjson, String sortName,
		...
		try {
			...
			sql.append("  from ta_oa_osupplietype");
			...
			if(osupplietypebean!=null){
				...
				if(!StringUtil.nullOrBlank(osupplietypebean.getOftype_id())){
					sql.append(" and (oftype_id = '"+osupplietypebean.getOftype_id()+"' or parent_id = '"+osupplietypebean.getOftype_id()+"')");
				}
			}
			//排序
	...
	public boolean deleteOsupplieBeansById(Connection conn, String beanId)
			throws Exception {
		StringBuffer sql = new StringBuffer();
		sql.append(" delete from ta_oa_osupplietype ");
	...
	* @throws Exception
	...
	public String getOsupplieType(String oftype_name) throws Exception{
		PreparedDBUtil pdb = new PreparedDBUtil();
		String id = "";
		StringBuffer sql = new StringBuffer();
		sql.append("select oftype_id");
		sql.append("  from ta_oa_osupplietype t");
		sql.append("   where  t.oftype_name = '"+oftype_name+"'");
		pdb.preparedSelect(sql.toString());
		pdb.executePrepared();
		if(pdb.size() > 0){
			for (int i = 0; i < pdb.size(); i++) {
				id = pdb.getString(i, "oftype_id");
			}
		}
	...
	@Override
	public List<Map<String, String>> getParentTypeList(Connection conn,OsupplieTypeBean bean) throws Exception {
		List<Map<String, String>> list = new ArrayList<Map<String, String>>();
		StringBuffer sql = new StringBuffer();
		PreparedDBUtil db = new PreparedDBUtil();
		sql.append("select a.oftype_id,a.oftype_name from ta_oa_osupplietype a where 1=1");
		if(!StringUtil.isBlank(bean.getParent_id())){
			sql.append(" and a.parent_id = '"+bean.getParent_id()+"'");
		}else{
			sql.append(" and a.parent_id is null");
		}
		if(!StringUtil.isBlank(bean.getUnit_id())){
			sql.append(" and a.unit_id = '"+bean.getUnit_id()+"'");
		}
	...
	@Override
	public List<Map<String, String>> getNameList(Connection conn,
			OsupplieTypeBean bean) throws Exception {
		List<Map<String, String>> list = new ArrayList<Map<String,String>>();
		PreparedDBUtil db = new PreparedDBUtil();
		StringBuffer sql = new StringBuffer();
		sql.append("select distinct a.ofustype_id,a.ofustype_name from TA_OA_HAVA_OBJECT a where 1=1");
		if(!StringUtil.isBlank(bean.getOftype_id())){
			sql.append("and a.oftype_id = '"+bean.getOftype_id()+"'");
		}
		if(!StringUtil.isBlank(bean.getUnit_id())){
			sql.append("and a.unit_id = '"+bean.getUnit_id()+"'");
		}
		//sql.append(" order by a.ofustype_no");
		db.preparedSelect(sql.toString());
		db.executePrepared(conn);
		if(db.size()>0){
			for(int i=0;i<db.size();i++){
