...
public class OsExaminationObjectServiceImpl implements
			...
			for (int i = 0; i < osAppObjectList.size(); i++) {
				for (int j = 0; j < osAppObjectList.size(); j++) {
					if ((!StringUtil.isBlank(osAppObjectList.get(i).get("it_id")))  
						...
						osAppObjectList.get(i).put("t_os_ob_id", osAppObjectList
								       .get(i).get("t_os_ob_id")+","+osAppObjectList.get(j).get("t_os_ob_id"));
	 ...
	 * 
	  ...
	  * @return 
	  * @see com.chinacreator.xtbg.tjy.officesupplies.service.OsExaminationObjectService
	  * #insertOrUpdateOsExaObject(com.chinacreator.xtbg.tjy.officesupplies.entity.OsExaminationBean)
	 */
