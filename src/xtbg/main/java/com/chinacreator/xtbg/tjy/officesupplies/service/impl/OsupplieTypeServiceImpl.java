...
import com.chinacreator.xtbg.tjy.officesupplies.service.OsupplieTypeService;
...
public class OsupplieTypeServiceImpl implements OsupplieTypeService{
	...
	public Map<String, String> getOsupplieTypeBean(String oftype_id,
		...
		try {
			...
			OsupplieTypeDao osupplietypedao=(OsupplieTypeDao)LoadSpringContext.getApplicationContext().getBean("osupplieTypeDaoImp");
	  ...
	  * @param bean
	  * @return 
	  * @see com.chinacreator.xtbg.tjy.officesupplies.service.OsupplieTypeService#insertOrUpdateOsTypeBean(com.chinacreator.xtbg.tjy.officesupplies.entity.OsupplieTypeBean) 
	  */
	@Override
	public String insertOrUpdateOsTypeBean(OsupplieTypeBean bean) {
		...
		try {
			conn = DbManager.getInstance().getTransactionConnection();
			OsupplieTypeDao dao=(OsupplieTypeDao)LoadSpringContext.getApplicationContext().getBean("osupplieTypeDaoImp");
...
//				}
	...
	public List<Map<String, String>> getTypeList(OsupplieTypeBean bean) {
		...
		try {
			conn = DbManager.getInstance().getTransactionConnection();
			OsupplieTypeDao dao=(OsupplieTypeDao)LoadSpringContext.getApplicationContext().getBean("osupplieTypeDaoImp");
			list = dao.getParentTypeList(conn, bean);
			conn.commit();
		} catch (Exception e) {
			DbManager.rollBackNoThrow(conn);
			LOG.error(e.getMessage(), e);
	...
	public List<Map<String, String>> getNameList(OsupplieTypeBean bean) {
		...
		try {
			conn = DbManager.getInstance().getTransactionConnection();
			OsupplieTypeDao dao=(OsupplieTypeDao)LoadSpringContext.getApplicationContext().getBean("osupplieTypeDaoImp");
			list = dao.getNameList(conn, bean);
			conn.commit();
		} catch (Exception e) {
			DbManager.rollBackNoThrow(conn);
			LOG.error(e.getMessage(), e);
