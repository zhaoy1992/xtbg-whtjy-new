...
import com.chinacreator.xtbg.pub.iweboffice.util.Doc2Pdf2SwfUtil;
...
public class IWebOffice {
	...
	public void GetAtByte(InputStream fileinputstream, int vSize) throws IOException {

			byte[] wordbody = new byte[vSize];
			fileinputstream.read(wordbody);
			System.out.println("GetAtBlob,vSize:" + vSize);
			mFileBody = wordbody;
			InputStream instream = fileinputstream;
			instream.read(mFileBody, 0, vSize);
			instream.close();
	...
	private boolean LoadTemplate() {
		...
		String Sql = "SELECT FileBody,FileSize FROM td_octl_template_file WHERE RecordID='"
				+ mTemplate + "'";
		try {
			if (DbaObj.OpenConnection()) {
				try {
		...
		try {
			if (DbaObj.OpenConnection()) {
				...
				try {
					...
					stmt = DbaObj.Conn.createStatement();
					// OracleResultSet
					// update=(OracleResultSet)stmt.executeQuery("select FileBody from Version_File where Fileid="
			...
			if (request.getMethod().equalsIgnoreCase("POST")) {
				...
				if (MsgObj.GetMsgByName("DBSTEP").equalsIgnoreCase("DBSTEP")) { // 如果是合法的信息包
					...
					if (mOption.equalsIgnoreCase("LOADFILE")) { // 下面的代码为打开服务器数据库里的文件
						...
						}else if(MsgObj.MsgFileLoad(mFilePath+"//uploadDocument//"+mFileName)){    //从文件夹调入文档
						...
						boolean saveData = SaveFile();
						boolean saveFile = true;
						// 是否允许将word保存到本地且转换为pdf
						saveFile = MsgObj.MsgFileSave(mFilePath
								+ "//uploadDocument//" + mFileName);
						if (saveData && saveFile) {
							if (CacheCommonManageProxy
									.getBooleanSystemParam("iwebOffice.wordToPdf")) {
								// 将上传的doc转换成pdf文件
								String sourFilePath = mFilePath
										+ "//uploadDocument//" + mFileName;
								String pdfFilePath = mFilePath
										+ "//uploadDocument//"
										+ mFileName.replace(".doc", ".pdf");
								Doc2Pdf2SwfUtil doc2swf = new Doc2Pdf2SwfUtil(
										mFilePath, sourFilePath, pdfFilePath,
										"");
								doc2swf.converDocToPdf();
							}
						} else {
						...
						MsgObj.MsgTextClear();
						if (mDirectory.trim().equalsIgnoreCase("")) {
							mFilePath = mFilePath +"//ccapp//xtbg//resources//plug//iWebOffice2009//HTMLIMAGE//";
						} else {
							mFilePath = mFilePath +"//ccapp//xtbg//resources//plug//iWebOffice2009//HTMLIMAGE//"
									+ mDirectory;
						}
						...
						mFilePath = mFilePath + "\\Document\\images\\"
								+ mImageName; // 图片在服务器的完整路径
