 ...
 *2014-3-9
 */
public class DirectoryExcelToData {
	public static final Logger LOG = Logger
			.getLogger(DirectoryExcelToData.class.getName());
	private String exportTemplateFilePath = FileUtil.getAppContextPath()+ CacheCommonManageProxy.getSystemParam("directry_excelTemplateFilePath");//导出模版所在路径
		...
		try{
			...
			for(int i=0 ; i < useridArry.length; i++){
				...
				if(!StringUtil.nullOrBlank(directoryBean.getDirectoryname())){
					...
					values[48] = directoryBean.getLeavetime();//离职时间
	...
	public String importExcelToData(String filepath){
		...
		try {
			...
			for(int i = 2 ; i < datas.length; i++){//从第二行开始读取
				...
				userinfoBean.setLeavetime(data[48]);//离职日期
