 ...
 *Sep 16, 2013
 */
public class FileFilesDevolveList extends DataInfoImpl {
	private static final long serialVersionUID = 1L;
	private static final Logger LOG = Logger.getLogger(FileFilesDevolveList.class);
	...
	public PagingBean getDataList(String parmjson, String sortName,
		...
		try {
			if (!StringUtil.isBlank(parmjson)) {
				FileFilesDevolveBean bean = (FileFilesDevolveBean) JSONObject
						.parseObject(parmjson, FileFilesDevolveBean.class);
				FileFilesDevolveDao dao = (FileFilesDevolveDao) LoadSpringContext
						.getApplicationContext().getBean(
								"FileFilesDevolveDaoImpl");
				pb = dao.queryFileFilesDevolveList(bean, sortName,
