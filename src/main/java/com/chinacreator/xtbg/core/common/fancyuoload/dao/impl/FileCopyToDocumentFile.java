...
import com.chinacreator.xtbg.core.common.fancyuoload.dao.FileCopy;
...
public class FileCopyToDocumentFile extends FileCopy {
	...
	public boolean copyOtherToFileBean(Map<String, String> params) {
		...
		try {
			/*1.delete file*/
			sql.append("delete oa_fc_attach where djbh=? and filename=?");//首先清空原先的文档附件
			...
			sql.append("insert into oa_fc_attach(attachid,djsn,djbh,filename,extend,attachcontent,filepos,creator) ");
			sql.append("select '"+ft.getAttachId()+"','"+uuid+"','"+djbh+"','"+filename+"'||f.filetype,f.filetype,f.filebody,f.filepath,f.username from oa_document_file f where f.recordid = ?");
